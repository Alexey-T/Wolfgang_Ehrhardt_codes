{Part 3c of regression test for SPECFUND unit  (c) 2011-2014  W.Ehrhardt}
...
begin
  ...
  y := poisson_pmf(mu,k);
  testrel(12, 2*NE2, y, f, cnt,failed);   {!!64-bit}
...
begin
  ...
  testrel(4, NE2+1, y, f, cnt,failed);  {!!fpc64}  {+1 for ARM}
...
const
  NE = 2;
  NE1 = 6;
  NE2 = 10;  {tests cdf(inv(y))=y, y = 1/NTEST .. 1.00}
begin
  cnt := 0;
  failed := 0;
  ...
  for n:=1 to NTEST do begin
    ...
    x := kolmogorov_inv(f);
    y := kolmogorov_cdf(x);
    testrel(10+n, NE2, y, f, cnt,failed);
  end;
