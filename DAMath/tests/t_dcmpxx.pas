{Test program to compare precision of DAMCmplx functions with mpc routines}
...
uses
  ...
  {$ifdef MPC_Diagnostic}
    mp_supp,
  {$endif}
  taus88, btypes, mp_types, mp_base, mp_real, mp_cmplx, damath, damcmplx;
...
{---------------------------------------------------------------------------}
function reldevx(const a,b: mp_complex): double;
...
{---------------------------------------------------------------------------}
procedure testcfun(const s: mp_string; xf: TCProc; mpf: TMProc; x1,x2,y1,y2: double);
var
  x,y,d,u: double;
  f,z,zmax: complex;
...
begin
  ...
  for i:=1 to n do begin
    ...
    z.im := y;
    xf(z,f);
    mpc_set_dbl(a,f.re,f.im);  {a=xf(z)}
    mpc_set_dbl(mx,x,y);
    mpf(mx,mf);                {mf = mpf(z)}
    d := reldevx(a,mf);
    {$ifdef use_eps}
...
begin
  ...
  testcfun('cot', {$ifdef FPC_ProcVar}@{$endif}ccot, {$ifdef FPC_ProcVar}@{$endif}mpc_cot, -a, a, -15, 15);
end;
...
begin
  ...
  testcfun('coth', {$ifdef FPC_ProcVar}@{$endif}ccoth, {$ifdef FPC_ProcVar}@{$endif}mpc_coth, -15, -15, -a, a);
end;
...
begin
  ...
  testcfun('tan', {$ifdef FPC_ProcVar}@{$endif}ctan, {$ifdef FPC_ProcVar}@{$endif}mpc_tan, -a, a, -15, 15);
end;
...
begin
  ...
  testcfun('tanh', {$ifdef FPC_ProcVar}@{$endif}ctanh, {$ifdef FPC_ProcVar}@{$endif}mpc_tanh, -15, -15, -a, a);
end;
...
begin
  ...
  testcfun('sqrt1mz2', {$ifdef FPC_ProcVar}@{$endif}csqrt1mz2, {$ifdef FPC_ProcVar}@{$endif}mpc_sqrt1mz2,
           -1000, 1000, -1e200, 1e200);
  testcfun('sqrt1mz2', {$ifdef FPC_ProcVar}@{$endif}csqrt1mz2, {$ifdef FPC_ProcVar}@{$endif}mpc_sqrt1mz2,
           -1e200, 1e200, -1000, 1000);
end;
...
begin
  ...
  writeln('Test DAMCmplx V', DAMCmplx_Version,' with MPArith V', MP_VERSION, '  (c) 2014-2016 W.Ehrhardt');
