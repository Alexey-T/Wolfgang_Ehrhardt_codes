...
{---------------------------------------------------------------------------}
function bulirsch_el2(x,kc,a,b: double): double;
...
begin
  if x=0.0 then bulirsch_el2 := 0.0
    ...
    {bulirsch_el2 := x*(a-b)/sqrt(1+x*x) + b*arcsinh(x)}
...
{$ifdef FPC}
  ...
  NABS = 20;     {will pass with smaller values}
...
{$ifdef FPC}
  testrele(15, eps2, y, f, cnt,failed);  {FPC 32-bit / SSE2}
{$else}
...
{$endif}
  ...
  testrel(31, NE1+1, y, f, cnt,failed);  {+1 for ARM}
  ...
  testrel(35, NE1+1, y, f, cnt,failed);

  y := el2(1e-4, 0,1,1);
  f := 0.9999999983333333408e-4;
  testrel(36, NE1+1, y, f, cnt,failed);
...
begin
  ...
  *)
  ...
  testrel(35, NE+1, y, f, cnt,failed);  {+1 for ARM}
  ...
  y := comp_ellint_1(1+ldexpd(1,-30));
  f := 11.43692847414619341;
  testrel(40, NE, y, f, cnt,failed);

  y := comp_ellint_1(1+ldexpd(1,-40));
  f := 14.902664382032274579;
  testrel(41, NE, y, f, cnt,failed);

  y := comp_ellint_1(1+ldexpd(1,-50));
  ...
  y := comp_ellint_2(1+ldexpd(1,-30));
  f := 0.9999999898141916125;
  testrel(46, NE, y, f, cnt,failed);

  y := comp_ellint_2(1+ldexpd(1,-50));
  ...
  y := comp_ellint_b(1+ldexpd(1,-20));
  f := 1.000006171399459128;
  testrel(52, NE, y, f, cnt,failed);

  y := comp_ellint_d(1+ldexpd(1,-20));
  f := 6.971182842499741989;
  testrel(53, 300, y, f, cnt,failed);         {!!!!}
  ...
  testrel(59, NE+1, y, f, cnt,failed);  {SSE}
  ...
  testrel(64, NE1, y, f, cnt,failed);   {SSE}
  ...
  testrel(68, 200, y, f, cnt,failed);   {!!! k^2 > nu and k^2 ~ nu, x87: 80}

  y := comp_ellint_3(4, 1000);
  f := -0.0002430031307463734391;
  testrel(69, NE1, y, f, cnt,failed);   {SSE}

  y := comp_ellint_3(2, 2);
  f := -0.3639605746942992989;
  testrel(70, NE1, y, f, cnt,failed);   {SSE}
...
begin
  ...
  testrel(14, NE+11, y, f, cnt,failed);   {+1 for ARM}
...
begin
  ...
  testrel(19, NE+1, y, f, cnt,failed);   {ARM}
  ...
  testrel(30, NE+1, y, f, cnt,failed);    {SSE}
