{Part 3 of regression test for SPECFUND unit  (c) 2010-2013  W.Ehrhardt}
...
begin
  ...
  testrel( 4, NE1+1, y, f, cnt,failed);
  ...
  testrel(21, NE+1, y, f, cnt,failed);
  ...
  testrel(23, NE+1, y, f, cnt,failed);

  y := normstd_cdf(10.0);
  f := 1;
  testrel(24, NE, y, f, cnt,failed);

  y := normstd_cdf(PosInf_d);
  f := 1;
  testabs(25, NE, y, f, cnt,failed);
...
begin
  ...
  y := normstd_inv(1-ldexpd(1,-20));
  f := 4.7630010342678139570;
  testrel(16, NE, y, f, cnt,failed);

  y := normstd_inv(1-ldexpd(1,-50));
...
begin
  ...
  testrel(13, NE+2, y, f, cnt,failed); {+2 for ARM}
...
begin
  ...
  mu := 5;
  sd := 2.0;
  y := normal_inv(mu, sd, ldexpd(1,-20));;
  ...
  y := normal_inv(mu, sd, 1-ldexpd(1,-20));
...
begin
  ...
  testrele(51, 1e-11, y, f, cnt,failed);

  y := t_cdf(120000000, -1.0);
  f := 0.15865525493965227849;
  testrele(52, 1e-8, y, f, cnt,failed);
  ...
  testrel(55, NE4, y, f, cnt,failed);

  y := t_cdf(nu, -1.5);
  f := 0.668135152014062988e-1;
  testrel(56, NE4, y, f, cnt,failed);

  y := t_cdf(nu, -1.0);
  f := 0.1586600932975529668;
  testrel(57, NE4, y, f, cnt,failed);

  y := t_cdf(nu, -0.5);
  f := 0.308539739120663701;
  testrel(58, NE4, y, f, cnt,failed);
...
begin
  ...
  y := t_inv(nu, 1-ldexpd(1,-20)); {0.99999904632568359375}
  ...
  testrel(49, NE+1, y, f, cnt,failed);       {+1 for ARM}

  {extreme cases}
  y := t_inv(10000, 0.9921875);
  f := 2.4179727636644442344;
  testrele(50, 2e-13, y, f, cnt,failed);

  y := t_inv(120000, 0.9921875);
  f := 2.4175934900459174302;
  testrele(51, 2e-12, y, f, cnt,failed);
...
begin
  ...
  y := f_inv(nu1, nu2, 1-ldexpd(1,-20)); {0.99999904632568359375}
