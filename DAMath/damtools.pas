...
{Accurate Double Math Tools: Root finding, function minimization, numerical
...
(*************************************************************************
 ...
 DESCRIPTION   :  Accurate Double Math Tools: Root finding, function minimization,
                  numerical integration, convergence acceleration, quadratic equations
 ...
 0.27     23.01.13  we          DAMath and double changes
 0.28     02.03.13  we          Removed repeated TFuncD type definition
 0.29     27.08.13  we          levinu1, wynneps1
 0.30     01.09.13  we          Change type of lsx to word (for 16-bit limit > 3200)
 0.31     23.09.13  we          cubsolve
 0.32     06.10.13  we          Quadpack fix: sizeof(extended) -> sizeof(double)
 0.33     16.04.14  we          am_getmem/freemem replaced with memh.malloc/mfree
 0.34     25.08.15  we          Use THREE & SIXX to avoid 'optimization'
 0.35     05.10.16  we          zbrenty for zeros of f(x)-y
 0.36     30.06.17  we          Fix return type of zbrent/y
 0.37     01.07.17  we          PolyRoots functions
 0.38     01.07.17  we          PolyRootPolish uses PolEvalCHEDer
 0.39     09.06.17  we          PolyRoots polishes complex roots
 0.40     02.12.17  we          Suppress warnings: Local variable does not seem to be initialized
 0.41     09.01.18  we          intde_p and intdei_p
 0.42     09.01.18  we          Avoid non-sense hint from Delphi 64-bit
 0.43     05.02.18  we          Removed ier=2 for intde* routines
 0.44     05.02.18  we          PolyRootsOA (open array version)
 0.45     12.02.18  we          Avoid non-sense warning for FPC304 -O4
 0.46     19.04.18  we          Fixes for FPC311
 0.47     26.10.18  we          zridders (Ridders' root finding method)
...
{---------------------------------------------------------------------------}
{#Z-}
procedure localmin(f: TFuncD; a,b,eps,t: double; var x,fx: double; var ic: integer);
...
procedure mbrent(f: TFuncD; a,b,t: double; var x,fx: double; var ic: integer);
...
function fmin(f: TFuncD; a,b,tol: double): double;
  ...
  { procedure localmin using eps = 0.5*sqrt(eps_d) and t = tol/3.}
...
{---------------------------------------------------------------------------}
{#Z-}
function zbrenty(f: TFuncD; y,a,b,t: double; var ic,err: integer): double;
  ...
  { 6*eps_d*|x|+2*t, where t is a positive tolerance, i.e. it solves f(x)=y.}
...
function zbrent(f: TFuncD; a,b,t:double; var ic,err: integer): double;
  {-Brent/Dekker algorithm with guaranteed convergence for finding a zero  }
  { of a function: Return a zero x of the function f in the interval [a,b],}
  { function zbrenty with y=0.0 is used.                                   }

function zeroin(f: TFuncD; a,b,t: double): double;
...
function zridders(f: TFuncD; a,b,t: double): double;
  {-Find a zero x of f with Ridders' method in the interval [a,b] with}
  { a tolerance 2*eps_d*|x| + 0.5*t, returns NaN if f(a)*f(b) > 0.    }
...
{---------------------------------------------------------------------------}
{#Z-}
procedure quanc8(fun: TFuncD; a,b,abserr,relerr: double;
...
type
  TQXLimArr = array[1..QMAXLIM] of double;  {double vector for subinterval lists}
  TQILimArr = array[1..QMAXLIM] of integer;   {Integer  vector for subinterval lists}

procedure quagk(f: TFuncD; a,b,epsabs: double; var result,abserr: double; var ier: integer);
  ...
  {        ier.                                                            }
  {                                                                        }
  {    = 6 The input is invalid, because epsabs <= 0 and epsrel < 50*eps_d.}
  ...
  {        result, abserr, last are set to NaN_d.                          }
  {#F}

procedure qags(f: TFuncD; a, b, epsabs, epsrel: double; limit: integer;
               var result, abserr: double; var neval: longint; var ier: integer);
...
procedure qagi(f: TFuncD; bound: double;inf: integer; epsabs, epsrel: double; limit: integer;
               var result, abserr: double; var neval: longint; var ier: integer);
...
procedure qagse(f: TFuncD; a, b, epsabs, epsrel: double;
...
procedure qagie(f: TFuncD; bound: double; inf: integer;epsabs, epsrel: double; limit: integer;
                var result, abserr: double;
                var neval: longint;
                var ier: integer;
                var alist, blist, elist, rlist: TQXLimArr;
                var iord: TQILimArr; var last: integer);
...
procedure qk21(f: TFuncD; a,b: double; var result,abserr,resabs,resasc: double);
...
procedure qawce(f: TFuncD; a, b, c, epsabs, epsrel: double;
                limit: integer; var result, abserr: double;
                var neval: longint; var ier: integer;
                var alist, blist, elist, rlist: TQXLimArr;
                var iord: TQILimArr; var last: integer);
...
procedure qawc(f: TFuncD; a, b, c, epsabs, epsrel: double;
               limit: integer; var result, abserr: double;
               var neval: longint; var ier: integer);
  {-Adaptive quadrature of the function f(x)/(x-c) over the finite interval}
  { (a,b) with the singularity at c with c<>a, c<>b. The routine calculates}
...
procedure intde(f: TFuncD; a, b, eps: double; var result, abserr: double;
                var neval: longint; var ier: integer);
...
procedure intde_p(f: TFuncDP; p: pointer; a, b, eps: double; var result, abserr: double;
                var neval: longint; var ier: integer);
  {-Automatic quadrature of f(x) over the finite interval (a,b)}
  { using Double Exponential (DE) transformation. Parameters:  }
...
procedure intdei(f: TFuncD; a, eps: double; var result, abserr: double;
                var neval: longint; var ier: integer);
...
procedure intdei_p(f: TFuncDP; p: pointer; a, eps: double; var result, abserr: double;
                var neval: longint; var ier: integer);
  {-Automatic quadrature of f(x) over (a,INF) using Double Exponential}
  { transformation when f(x) has no oscillatory factor. Parameters:   }
...
procedure intdeo(f: TFuncD; a, omega, eps: double; var result, abserr: double;
                var neval: longint; var ier: integer);
  {-Automatic quadrature of f(x) over (a,INF) using Double Exponential}
  ...
  { abserr - estimate of the absolute error, if ier=0 or 3 }
  { neval  - number of integrand evaluations               }
  { ier    - 0: OK, 1: eps < eps_d/32, eps>1, or omega=0   }
  {          3: max. iterations, result/abserr have values }
  {#F}
...
{---------------------------------------------------------------------------}
{#Z-}
procedure levinu1(an: double; n,nmax: integer;
                  var qnum, qden: array of double;
                  var extsum, sn: double; var ierr: integer);
...
procedure wynneps1(an: double; n,nmax: integer; sum: boolean;
                   var e: array of double;
                   var extlim, sn: double; var ierr: integer);
...
{#Z+}
procedure qpsrt(limit: integer; var last, maxerr: integer; var ermax: double;
...
const
  ...
  ITMax = 5000;
var
  d,e,m,p,q,r,tol,t2,u,v,w: double;
  fu,fv,fw: double;
  it: integer;
begin
  ...
  {Force t >= 0}
  t := abs(t);
  {eps shall not be < 2*eps_d, preferably not smaller than sqrt(eps_d)}
  if eps < 2.0*eps_d then eps := 2.0*eps_d;
...
procedure mbrent(f: TFuncD; a,b,t: double; var x,fx: double; var ic: integer);
  ...
  { ic is the iteration count, -1 if a=b, 0 if max.=5000 exceeded.}
begin
  localmin(f,a,b,0.5*sqrt(eps_d),t,x,fx,ic);
end;
...
var
  ...
  ic: integer;
begin
  localmin(f,a,b,0.5*sqrt(eps_d),tol/3.0,x,fx,ic);
  fmin := x;
end;
...
const
  ITMax = 5000;
var
  c,d,e,fa,fb,fc,tol,m,p,q,r,s: double;
  it: integer;
begin

  {Ref: Brent [28] ch.3/4, [29] ch.7; see also [13] ch.9.2. This algorithm }
...
function zridders(f: TFuncD; a,b,t: double): double;
  ...
  { a tolerance 2*eps_d*|x| + 0.5*t, returns NaN if f(a)*f(b) > 0.    }
var
  x1,x2,x3,xm,f1,f2,f3,fm,tol: double;
  s1,s2,s3,sm: integer;
  icnt: integer;
const
  MAXIT = 100;
...
begin
  if d=0 then getexpd := 0
    ...
    e := (THexDblW(d)[3] and $7FF0) shr 4;
    if e=$7FF then getexpd := $7FFF
    else if e=0 then getexpd := -$7FFF
    else getexpd := e-$3FF;
  end;
end;
...
procedure cubeval(x,a,b,c,d: double; var q,q1,b1,c2: double);
  {-Interal use: return q = q(x) = ax^3 + bx^2 + cx + d,}
...
begin
  if IsInfOrNAN(a,b,c) or IsNaNOrInfD(d) then begin
    x  := NaN_d;
    x1 := NaN_d;
    x2 := NaN_d;
    y1 := 0.0;
  ...
  if r/maxba >= t then begin
    ...
    s := isign(t);
    t := -q1/a;
    if t>0.0 then r := 1.324718*maxd(r, sqrt(t));
    x0 := x - s*r;
    if x<>x0 then begin
      s1 := succd(1.0);
      repeat
...
begin
  {last assignment before sqrdx, otherwise there is a non-sense}
  {hint from Delphi 64-bit: Value assigned to 'last' never used}
  last := 0;
  sqrdx := sqr(radix);
...
begin
  ...
  {Note that no extended is available, so use compensated evaluation of p(x)}
  PolEvalCHEDer(x,p,n+1,px,dp,z);
...
begin
  ...
  f1 := k+1.0;
  t  := an*sqr(f1);
  if abs(t)<Sqrt_MinDbl then begin
    qden[k] := 1.0;
    qnum[k] := 1.0;
  end
  else begin
  ...
  t := abs(qden[0]);
  if (t>1.0) or (t*Maxdouble > abs(qnum[0])) then begin
...
begin
  ...
  if n=0 then extlim := sn
    ...
    for j:=n downto 1 do begin
       ...
       aux2 := e[j-1];
       diff := e[j] - aux2;
       if abs(diff) <= Sqrt_MinDbl then e[j-1] := aux1
...
var
  area, stone, step, cor11, temp: double;
  qprev, qnow, qdiff, qleft, esterr, tolerr: double;
  levmax, lev, i, j: integer;
  nim, nofin: longint;
  qright: array[1..31] of double;
  f, x: array[0..16] of double;
  fsave, xsave: array[1..8,1..30] of double;
...
const
  w0: double =   3956 / 14175; {Newton-Cotes weights multiplied by 8}  {Fix311}
  w1: double =  23552 / 14175;
  w2: double =  -3712 / 14175;
  w3: double =  41984 / 14175;
  w4: double = -18160 / 14175;
...
stage3: {*** central calculation ****}
  ...
  tolerr := maxd(abserr,relerr*abs(area)) * (step/stone);
...
const
  ...
  xgk: array[1..11] of double = (              {abscissae of the 21-point Gauss/Kronrod rule}
         ...
         0.0);
  wgk: array[1..11] of double = (              {weights of the 10-point gauss rule}
...
var
  hlgth: double; {half-length of the interval}
  centr: double; {mid point of the interval}
  dabsc: double; {abscissa}
  resg : double; {result of the 10-point Gauss formula}
  resk : double; {result of the 21-point Kronrod formula}
  reskh: double; {approximation to the mean value of f over (a,b)}
  fval1,
  fval2: double; {function values}
  fc,fsum,dhlgth, t: double;
...
begin
  ...
  if (resasc<>0.0) and (abserr<>0.0) then begin
    ...
    abserr := resasc*mind(1.0,200.0*t*t*t)
  end;
  t := 50.0*eps_d;
  if resabs > MinDouble/t then abserr := maxd(t*resabs,abserr);
end;


{---------------------------------------------------------------------------}
procedure qk15i(f: TFuncD; bound: double; inf: integer; a, b: double;
                var result, abserr, resabs, resasc: double);
  {-Integration with 15-point transformed Gauss-Kronrod rule}
const
  wg : array[1..8] of double = ( {weights of the 7-point Gauss rule}
  ...
  xgk: array[1..8] of double = (   {abscissae of the 15-point Kronrod rule}
         ...
         0.0);
  wgk: array[1..8] of double = ( {weights of the 15-point Kronrod rule}
...
var
  ...
  absc,
  absc1,
  absc2: double;  {abscissa}
  fval1,
  fval2: double;  {function value}
  hlgth: double;  {half-length of the interval}
  resg:  double;  {result of the 7-point gauss formula}
  resk:  double;  {result of the 15-point Kronrod formula}
  reskh: double;  {approximation to the mean value of the transformed integrand}
  tabsc1,
  tabsc2: double; {transformed abscissa}
  dinf, fc, fsum, t: double;
...
type
  TEpsArray = array[1..52] of double;
  TEps3Res  = array[1..3]  of double;
...
procedure qelg(var n: integer;        {index of the new element in the 1. column of the epsilon table}
               ...
               var result: double;  {resulting approximation to the integral}
               var abserr: double;  {estimate of the absolute error}
...
var
  e0, e1, e2, e3: double;   {the 4 elements on which the computation of a}
                            {new element in the epsilon table is based}
  error: double;            {abs(e1-e0)+abs(e2-e1)+abs(new-e2)}
  delta1, delta2, delta3: double;
  err1, err2, err3: double;
  tol1, tol2, tol3: double;
  e1abs, res, ss: double;
...
l50:
  ...
  if nres<4 then begin
    ...
    res3la[2] := res3la[3];
    res3la[3] := result;
    abserr    := maxd(abserr, 5.0*eps_d*abs(result));
  end
end;
...
var
  ...
  small: double;     {length of the smallest interval considered up to now, multiplied by 1.5}
  area1, area12, area2, a1, a2, b1, b2, correc, defabs,
  defab1, defab2, dres, ertest, resabs,reseps,
  error1, error2, erro12: double;
...
begin
  ...
  result := 0.0;
  abserr := 0.0;
  if (epsabs <= 0.0) and (epsrel < 50.0*eps_d) then begin
    ier := 6;
    neval := 0;
    exit;
  end;
  ...
  {test on accuracy}
  dres := abs(result);
  errbnd := maxd(epsabs, epsrel*dres);
  alist[1] := a;
  blist[1] := b;
  rlist[1] := result;
  elist[1] := abserr;
  iord[1] := 1;
  if (abserr <= 100.0*eps_d*defabs) and (abserr>errbnd) then ier := 2;
  ...
  correc := 0;
  ertest := 0;
  if dres >= (1.0-50.0*eps_d)*defabs then ksgn := 1;
  ...
  while last < limit do begin
    ...
    rlist[maxerr] := area1;
    rlist[last] := area2;
    errbnd := maxd(epsabs, epsrel*abs(area));
    ...
    if maxd(abs(a1),abs(b2)) <= (1.0+100.0*eps_d)*(abs(a2)+1000.0*MinDouble) then ier := 4;
    ...
    if last=2 then begin
      ...
      if abseps < abserr then begin
        ...
        result := reseps;
        correc := erlarg;
        ertest := maxd(epsabs,epsrel*abs(reseps));
...
l100:
  ...
  if abserr=MaxDouble then goto l115;
  ...
  {test on divergence}
  if (ksgn=-1) and (maxd(abs(result),abs(area)) <= defabs*0.01) then goto l130;
...
begin
  ...
  if dres>=(1.0-50.0*eps_d)*defabs then ksgn := 1 else ksgn := -1;
...
begin
  ...
  if epsabs=0.0 then epsrel := 50.0*eps_d else epsrel := 0.0;
  infa := IsInfD(a);
  infb := IsInfD(b);
  if IsNanD(a) or IsNanD(b) or (infa and (a=b)) then begin
    ier := 9;
    result := NaN_d;
    abserr := NaN_d;
    exit;
...
{---------------------------------------------------------------------------}
procedure qk15c(f: TFuncD; a,b,c: double; var result, abserr, resabs, resasc: double);
  {-Integration of f(x)/(x-c) over (a,b) with 15-point Gauss-Kronrod rule}
const
  wg : array[1..4] of double = ( {weights of the 7-point Gauss rule}
...
{---------------------------------------------------------------------------}
procedure qc25gcc(f: TFuncD; a,b,c: double; var result,abserr: double);
  {-Integration of f(x)/(x-c) over (a,b) using the generalized Clenshaw-Curtis method}
const
  x: array[1..11] of double = ( {x contains the values cos(k*pi/24)}
...
var
  v,cheb12: array[1..13] of double;
  fval,cheb24: array[1..25] of double;
var
  ak22,amom0,amom1,amom2,cc,centr,hlgth: double;
  res12,res24,u: double;
...
{---------------------------------------------------------------------------}
procedure qc25c(f: TFuncD; a,b,c: double; var result,abserr: double; var krul,neval: integer);
...
procedure qawce(f: TFuncD; a, b, c, epsabs, epsrel: double;
  ...
  {        epsrel < 50*eps_d. result, abserr, neval, last are set to zero. }

var
  aa,bb,a1,a2,b1,b2,area,area1,area2,area12,
  error1,error2,erro12,errbnd,errmax,errsum: double;
...
begin
  ...
  result := 0.0;
  abserr := 0.0;
  if (c=a) or (c=b) or ((epsabs<=0.0) and (epsrel < 50.0*eps_d)) then begin
    ier := 6;
    neval := 0;
    exit;
  end;
  ...
  {test on accuracy}
  errbnd := maxd(epsabs,epsrel*abs(result));
  if limit=1 then ier := 1;
  if (abserr<mind(0.01*abs(result),errbnd)) or (ier=1) then begin
  ...
  repeat
    ...
    if errsum > errbnd then begin
      ...
      {set error flag in the case of bad integrand}
      {behaviour at a point of the integration range}
      if maxd(abs(a1),abs(b2)) <= (1.0+100.0*eps_d)*(abs(a2)+1000.0*MinDouble) then ier := 3;
    end;
    {append the newly-created intervals to the list}
    if error2 <= error1 then begin
      alist[last]   := a2;
...
var
  ...
  irback,t,ep,em,tk,xw,wg,xa,fp,fm,err,errh,tn,errd: double;
  done: boolean;
begin
  ...
  if (eps < eps_d/32) or (eps > 1.0) or (omega=0.0) then begin
    ier := 1;
    exit;
  end
  else ier := 0;
  ...
  ehm   := 1.0/ehp;
  xw    := exp(pp - pi_2);
  xa    := a + sqrt(0.5*xw*per2);  if xa=a then exit;
  iv    := f(xa);
  neval := 1;
  ir    := iv*xw;
  iv    := 0.5*per2*iv;
