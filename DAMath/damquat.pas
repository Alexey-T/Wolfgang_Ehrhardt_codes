...
{DAMath based quaternion routines}
...
(*************************************************************************
 ...
 DESCRIPTION   :  DAMath based quaternion routines
 ...
 0.19     22.10.17  we          Ported from AMQuat
...
function  qdot(const a,b: quaternion): double;
 {-Compute the scalat (dot) product a.b}
...
procedure qmulx(const a: quaternion; b: double; var c: quaternion);
  {-Compute the product c = a * b}

procedure qneg(const a: quaternion; var b: quaternion);
  {-Compute the negative of a, b = -a}

function  qnorm(const a: quaternion): double;
...
procedure qpowx(const a: quaternion; b: double; var c: quaternion);
  {-Return the real power of w = a^b = exp(b*ln(a))}

procedure qsec(const a: quaternion; var b: quaternion);
  {-Return the circular secant  b = sec(a) = 1/cos(z)}
...
{---------------------------------------------------------------------------}
procedure c2qm(ai: double; usei: boolean; const u: complex; const a: quaternion; var b: quaternion);
...
{---------------------------------------------------------------------------}
procedure use_complex_ex(func: tcproc; ai: double; usei: boolean; const a: quaternion; var b: quaternion);
