...
(*************************************************************************
 ...
 0.14     22.12.17  we          aesr_rangel/w
 0.15     25.12.17  we          aesr_rangel for old non-basm compilers
**************************************************************************)
...
function  aesr_rangew(var ctx: aesr_ctx; range: word): word;
  {-Next random word in range 0..range-1}

function  aesr_rangel(var ctx: aesr_ctx; range: longint): longint;
...
begin
  aesr_next(ctx);
  aesr_rangew := rangew(ctx.nr, range);
end;
...
begin
  aesr_next(ctx);
  aesr_rangel := rangel(ctx.nr, range);
end;
