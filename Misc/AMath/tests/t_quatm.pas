{Test/dev program for AMQuat  (c) W.Ehrhardt 2017}
unit t_quatm;
...
type
  tq = array[0..3] of extended;

var
  eps, eps_f, maxerr, maxerr_f: extended;

var
  cnt, failed: integer;

{---------------------------------------------------------------------------}
function reldevq(const a: quaternion; const b: TQ): extended;
var
  t: quaternion;
  r: extended;
begin
...
{---------------------------------------------------------------------------}
function reldevx(x,y: extended): extended;
var
  r: extended;
begin
  inc(cnt);
...
{Test cases computed with Mathematica package Quaternions}
...
{---------------------------------------------------------------------------}
function reldev2q(const a,b: quaternion): extended;
var
  t: quaternion;
  r: extended;
begin
...
begin
  writeln;
  writeln('Test program for AMQuat  (c) W.Ehrhardt 2017');
  eps := 6*eps_x;
  maxerr := -1;
  eps_f  := 32*eps_x;
  maxerr_f := -1;
  ...
  writeln(' - max. error = ', maxerr/eps_x:1:2,',  max. error func(inv(q)) = ', maxerr_f/eps_x:1:2, ' [eps_x]');
end;
