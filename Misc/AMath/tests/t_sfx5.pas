{Part 5 of regression test for SPECFUNX unit  (c) 2010-2016  W.Ehrhardt}
...
procedure test_chebyshev_tx;
procedure test_chebyshev_ux;
procedure test_chebyshev_vx;
procedure test_chebyshev_wx;
procedure test_chebfunx;
procedure test_gegenbauer_cx;
procedure test_hermite_hx;
procedure test_hermite_hex;
procedure test_jacobi_px;
procedure test_laguerrex;
procedure test_zernike_rx;
procedure test_orthopolyx;
...
begin
  ...
  y := chebyshev_tx(20,1e-7);
  f := 0.999999999998;
  testrel(16, NE, y, f, cnt,failed);

  y := chebyshev_tx(21,1e-10);
...
var
  ...
  cnt,failed,i: integer;
const
  f12: THexExtW = ($0000,$0000,$3CB0,$924D,$BFFC);  {-1.4287276100367307663E-1}
const
  NE = 8;
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','chebyshev_ux');
  ...
  y := chebyshev_ux(20,1e-7);
  f := 0.9999999999978;
  testrel(21, NE, y, f, cnt,failed);

  y := chebyshev_ux(21,1e-10);
...
begin
  ...
  y := chebyshev_vx(1,-0.75);
  f := -2.5;
  testrel(9, NE, y, f, cnt,failed);

  y := chebyshev_vx(3,-1.75);
  f := -47.125;
  testrel(10, NE, y, f, cnt,failed);

  y := chebyshev_vx(3,0.75);
  f := -0.875;
  testrel(11, NE, y, f, cnt,failed);

  y := chebyshev_vx(20,1e-10);
  f := 1.000000001999999998;
  testrel(12, NE, y, f, cnt,failed);

  y := chebyshev_vx(21,1e-10);
  f := -0.4999999988999999989*2;
  testrel(13, NE, y, f, cnt,failed);

  y := chebyshev_vx(30,1.75);
  f := 0.953672986334881337e15;
  testrel(14, NE, y, f, cnt,failed);

  y := chebyshev_vx(31,-0.75);
  f := 1.979055979754775763;
  testrel(15, NE, y, f, cnt,failed);

  y := chebyshev_vx(65,-1.75);
  f := -0.7512911534276803238e33;
  testrel(16, NE, y, f, cnt,failed);

  y := chebyshev_vx(66,0.75);
  f := -0.6322890154096164170;
  testrel(17, NE, y, f, cnt,failed);

  y := chebyshev_vx(129,-1.75);
  f := -0.1215527424708477387e66;
  testrel(18, NE, y, f, cnt,failed);

  y := chebyshev_vx(130,0.75);
  f := 1.066498186308875424;
  testrel(19, NE, y, f, cnt,failed);

  y := chebyshev_vx(259,-1.75);
  f := -0.3230037956135866602e131;
  testrel(20, NE, y, f, cnt,failed);

  y := chebyshev_vx(262,0.75);
  f := 0.3651824569614297354;
  testrel(21, NE2, y, f, cnt,failed);

  y := chebyshev_vx(519,-1.75);
  f := -0.2280832294625592145e262;
  testrel(22, NE3, y, f, cnt,failed);

  y := chebyshev_vx(520,-1.5);
  f := 0.3598599375293868940e218;
  testrel(23, NE3, y, f, cnt,failed);

  y := chebyshev_vx(520,0.75);
  f := 0.7386964251018360491;
  testrel(24, NE3, y, f, cnt,failed);

  x := 1+ldexp(1,-30);
  y := chebyshev_vx(250,x);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','chebyshev_f1x');
  ...
  testrel(11, NE1, y, f, cnt,failed);     {NE1 for AMD}
  ...
  y := chebyshev_f1x(v, 2);
  f := 5749928.954608412337;
  testrel(19, NE1, y, f, cnt,failed);     {NE1 for AMD}
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','gegenbauer_cx');

  y := gegenbauer_cx(1, -0.2, 1.0);
  f := -0.4;
  testrel( 1, NE, y, f, cnt,failed);

  y := gegenbauer_cx(1, 0.0, 1.0);
  f := 2.0;
  testrel( 2, NE, y, f, cnt,failed);

  y := gegenbauer_cx(1, 1.0, 1.0);
  f := 2.0;
  testrel( 3, NE, y, f, cnt,failed);

  y := gegenbauer_cx(1, 1.0, 0.5);
  f := 1.0;
  testrel( 4, NE, y, f, cnt,failed);

  y := gegenbauer_cx(1, 5.0, 1.0);
  f := 10.0;
  testrel( 5, NE, y, f, cnt,failed);

  y := gegenbauer_cx(1, 100.0, 0.5);
  f := 100.0;
  testrel( 6, NE, y, f, cnt,failed);

  y := gegenbauer_cx(2, -0.2, 0.5);
  f := 0.12;
  testrel( 7, NE, y, f, cnt,failed);

  y := gegenbauer_cx(2, 0.0, 1.0);
  f := 1.00;
  testrel( 8, NE, y, f, cnt,failed);

  y := gegenbauer_cx(2, 1.0, 1.0);
  f := 3.00;
  testrel( 9, NE, y, f, cnt,failed);

  y := gegenbauer_cx(2, 1.0, 0.1);
  f := -0.96;
  testrel(10, NE, y, f, cnt,failed);

  y := gegenbauer_cx(2, 5.0, 1.0);
  f := 55.0;
  testrel(11, NE, y, f, cnt,failed);

  y := gegenbauer_cx(2, 100.0, 0.5);
  f := 4950.0;
  testrel(12, NE, y, f, cnt,failed);

  y := gegenbauer_cx(3, -0.2, 0.5);
  f := 0.112;
  testrel(13, NE, y, f, cnt,failed);

  y := gegenbauer_cx(3, 0.0, 1.0);
  f := 2/3; {Fix311}
  testrel(14, NE, y, f, cnt,failed);

  y := gegenbauer_cx(3, 1.0, 1.0);
  f := 4.000;
  testrel(15, NE, y, f, cnt,failed);

  y := gegenbauer_cx(3, 1.0, 0.1);
  f := -0.392;
  testrel(16, NE, y, f, cnt,failed);

  y := gegenbauer_cx(3, 5.0, 1.0);
  f := 220.0;
  testrel(17, NE, y, f, cnt,failed);

  y := gegenbauer_cx(3, 100.0, 0.5);
  f := 161600.0;
  testrel(18, NE, y, f, cnt,failed);

  y := gegenbauer_cx(10,-0.2, 0.5);
  f := 0.9934389248e-2;
  testrel(19, NE, y, f, cnt,failed);

  y := gegenbauer_cx(10, 0.0, 1.0);
  f := 0.2;
  testrel(20, NE, y, f, cnt,failed);

  y := gegenbauer_cx(10, 1.0, 1.0);
  f := 11.0;
  testrel(21, NE, y, f, cnt,failed);

  y := gegenbauer_cx(10, 1.0, 0.1);
  f := -0.4542309376;
  testrel(22, NE, y, f, cnt,failed);

  y := gegenbauer_cx(10, 5.0, 1.0);
  f := 9.23780e+4;
  testrel(23, NE, y, f, cnt,failed);

  y := gegenbauer_cx(10, 100.0, 0.5);
  f := 1.5729338392690000e+13;
  testrel(24, NE, y, f, cnt,failed);

  y := gegenbauer_cx(21, -0.2, 0.5);
  f := 0.9991386973302926030e-2;
  testrel(25, NE, y, f, cnt,failed);

  y := gegenbauer_cx(21, 0.0, 1.0);
  f := 2/21;
  testrel(26, NE, y, f, cnt,failed);

  y := gegenbauer_cx(21, 1.0, 1.0);
  f := 22.0;
  testrel(27, NE, y, f, cnt,failed);

  y := gegenbauer_cx(21, 1.0, 0.1);
  f := 0.8103854265403986412;
  testrel(28, NE, y, f, cnt,failed);

  y := gegenbauer_cx(21, 5.0, 1.0);
  f := 14307150.0;
  testrel(29, NE, y, f, cnt,failed);

  y := gegenbauer_cx(21, 100.0, 0.5);
  f := 0.826932426083121018e20;
  testrel(30, NE2, y, f, cnt,failed);

  y := gegenbauer_cx(42, -0.2, 0.5);
  f := -0.4324011590989000524e-2;
  testrel(31, NE, y, f, cnt,failed);

  y := gegenbauer_cx(42, 0.0, 1.0);
  f := 1/21;
  testrel(32, NE, y, f, cnt,failed);

  y := gegenbauer_cx(42, 1.0, 1.0);
  f := 43.0;
  testrel(33, NE, y, f, cnt,failed);

  y := gegenbauer_cx(42, 1.0, 0.1);
  f := 0.3961791364804087116;
  testrel(34, NE, y, f, cnt,failed);

  y := gegenbauer_cx(42, 5.0, 1.0);
  f := 3042312350.0;
  testrel(35, NE, y, f, cnt,failed);

  y := gegenbauer_cx(42, 100.0, 0.5);
  f := -0.2728803040216282769e30;
  testrel(36, NE, y, f, cnt,failed);

  y := gegenbauer_cx(1000, 100.0, 1.0);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','hermite_hex');

  y := hermite_hex(11,-0.5);
  f := 3240.81005859375;
  testrel(1, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','jacobi_px');

  y := jacobi_px(2,1,0.75,0.5);
  f := 0.291015625;
  testrel(1, NE, y, f, cnt,failed);

  y := jacobi_px(2,1,0.75,0.1);
  f := -0.647109375;
  testrel(2, NE, y, f, cnt,failed);

  y := jacobi_px(8,1,0.75,0.1);
  f := 0.2039343077673979243;
  testrel(3, NE, y, f, cnt,failed);

  y := jacobi_px(8,1,0.75,2.5);
  f := 178708.9906612954856;
  testrel(4, NE, y, f, cnt,failed);

  y := jacobi_px(8, 1.5, -0.5, -1);
  f := 0.196380615234375;
  testrel(5, NE, y, f, cnt,failed);

  y := jacobi_px(8,1.5, -0.5, -2);
  f := 8899.118499755859377;
  testrel(6, NE, y, f, cnt,failed);

  y := jacobi_px(8,1.5, -0.5, 0);
  f := 0.21820068359375e-1;
  testrel(7, NE, y, f, cnt,failed);

  y := jacobi_px(8,1.5, -0.5, 0.9375);
  ...
  y := jacobi_px(8,1.5, -0.5, 0.25);
  f := -0.4237014055252075195;
  testrel(10, NE, y, f, cnt,failed);

  y := jacobi_px(8,1.5, -0.5, -0.75);
  f := 0.2199906110763549806;
  testrel(11, NE, y, f, cnt,failed);

  y := jacobi_px(25,1.5, -0.5, 0.75);
  f := -0.8993370671575232300;
  testrel(12, NE, y, f, cnt,failed);

  y := jacobi_px(100,1.5, -0.5, -0.25);
  f := 0.3442708728724186509e-1;
  testrel(13, NE, y, f, cnt,failed);

  y := jacobi_px(100,1.5, -0.5, -2);
  f := 0.1094511122764460740e57;
  testrel(14, NE, y, f, cnt,failed);

  y := jacobi_px(200,1.5, -0.5, 0.25);
  f := -0.5283109520288860554e-1;
  testrel(15, NE, y, f, cnt,failed);

  y := jacobi_px(200,1.5, -0.5, 2.5);
  f := 0.1562423735191907416e136;
  testrel(16, NE, y, f, cnt,failed);

  y := jacobi_px(8, 0.5, 1.5, 2);
  f := 21852.069671630859375;
  testrel(17, NE, y, f, cnt,failed);

  y := jacobi_px(8, 0.5, 1.5, -0.875);
  f := -0.8486263942904770374;
  testrel(18, NE, y, f, cnt,failed);

  y := jacobi_px(100, 9.5, -1.5, 2);
  f := 0.4661975827107720642e60;
  testrel(19, NE, y, f, cnt,failed);

  y := jacobi_px(100, 9.5, -1, 2);
  f := 0.5731010730126711304e60;
  testrel(20, NE, y, f, cnt,failed);

  y := jacobi_px(10, 9.5, -1., 0.5);
...
begin
  ...
  y := laguerrex(1, -2.0, 1.0);
  f := -2.0;
  testrel( 1, NE, y, f, cnt,failed);

  y := laguerrex(1, 0.5, 1.0);
  f := 0.5;
  testrel( 2, NE, y, f, cnt,failed);

  y := laguerrex(2, -2.0, 1.0);
  f := 0.5;
  testrel( 3, NE, y, f, cnt,failed);

  y := laguerrex(2, 0.5, -1.0);
  f := 4.875;
  testrel( 4, NE, y, f, cnt,failed);
  ...
  y := laguerre_assx(2, 1,  1.0);
  f := 0.5;
  testrel( 6, NE, y, f, cnt,failed);

  y := laguerrex(2,-1.0,  1.0);
  f := -0.5;
  testrel( 7, NE, y, f, cnt,failed);
  ...
  y := laguerrex(2,-3.0,  1.0);
  f := 2.5;
  testrel( 9, NE, y, f, cnt,failed);

  y := laguerrex(3, -2.0, 1.0);
  f := 1/3; {Fix311}
  testrel(10, NE, y, f, cnt,failed);

  y := laguerrex(3, 0.5, -1.0);
  ...
  y := laguerre_assx(3, 1,  1.0);
  f := -1/6;
  testrel(13, NE, y, f, cnt,failed);

  y := laguerre_assx(3, 2,  1.0);
  f := 7/3;
  testrel(14, NE, y, f, cnt,failed);

  y := laguerre_assx(3, 2, 0);
  f := 10.0;
  testrel(15, NE, y, f, cnt,failed);

  y := laguerre_assx(3,2,0.0009765625);
  f := 64361610239.0/6442450944.0;
  testrel(16, NE, y, f, cnt,failed);

  y := laguerre_assx(3,2,0.125);
  f := 26999/3072;
  testrel(17, NE, y, f, cnt,failed);

  y := laguerre_assx(3,2,1.5);
  f := 1/16;
  testrel(18, NE, y, f, cnt,failed);

  y := laguerre_assx(3,2,100);
  f := -427970/3;
  testrel(19, NE, y, f, cnt,failed);
  ...
  y := laguerrex(3,-2,0.125);
  f := 0.7486979166666666667e-2;
  testrel(22, NE, y, f, cnt,failed);

  y := laguerrex(3,-2,0.0009765625);
  f := 0.4766819377740224203e-6;
  testrel(23, NE, y, f, cnt,failed);

  y := laguerrex(3,-3.0,  1.0);
  f := -1/6; {Fix311}
  testrel(24, NE, y, f, cnt,failed);

  y := laguerrex(3,-4.0,  1.0);
  f := -8/3;
  testrel(25, NE, y, f, cnt,failed);

  y := laguerre_assx(4, 2, 0.5);
  f := 6.752604166666666667;
  testrel(26, NE, y, f, cnt,failed);

  y := laguerrex(10,-0.5,0);
  f := 46189/262144;
  testrel(27, NE, y, f, cnt,failed);

  y := laguerrex(10,-0.5,0.125);
...
begin
  ...
  for i:=1 to 100 do begin
    x := -0.9;
    c := sqrtpi/Gammax(i+0.5)*facx(i);
    while x<1 do begin
      {HMF[1], 22.5.31}
      y := jacobi_px(i,-0.5,-0.5,x)*c;
      f := chebyshev_tx(i,x);
      t := abs(y-f)/eps_x;
      if t > NE then begin
        writeln('   i/x/t = ',i:6, x:10:4, t:30:2);
  ...
  for i:=1 to 100 do begin
    x := -0.9;
    c := (2*i+1)*facx(i)/pochhammerx(1.5,i);
    while x<1 do begin
      y := jacobi_px(i,0.5,-0.5,x)*c;
      f := chebyshev_wx(i,x);
      t := abs(y-f)/eps_x;
  ...
  for i:=1 to 100 do begin
    x := -0.9;
    c := 0.5*sqrtpi/Gammax(i+1.5)*facx(i+1);
    while x<1 do begin
      {HMF[1], 22.5.32}
      y := jacobi_px(i,0.5,0.5,x)*c;
      f := chebyshev_ux(i,x);
  ...
  for i:=1 to 25 do begin
    ...
    while x<=100 do begin
      n := i div 2;
      if odd(i) then begin
        y := x*laguerrex(n,0.5,x*x)*facx(n);
        if odd(n) then y := -y;
        y := ldexp(y,i);
      end
      else begin
        y := laguerrex(n,-0.5,x*x)*facx(n);
        if odd(n) then y := -y;
        y := ldexp(y,i);
      end;
      f := hermite_hx(i,x);
      if f=0 then t := abs(y-f)/eps_x
      else t := abs(1-y/f)/eps_x;
      if t > 40 then begin
        writeln('   i/x/t = ',i:6, x:10:4, t:30:2);
        inc(failed);
      end;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','zernike_rx');

  {Wolfram Alpha: ZernikeR[3,1,0.5] etc}

  y := zernike_rx(3,1,0.5);
  f := -0.625;
  testrel( 1, NE, y, f, cnt,failed);

  y := zernike_rx(6,2,0.75);
  f := -0.283447265625;
  testrel( 2, NE, y, f, cnt,failed);

  y := zernike_rx(19,5,0.25);
  f := -0.2671170888206688687;
  testrel( 3, NE, y, f, cnt,failed);

  y := zernike_rx(19,5,0.4287525723551651222);
  f := 0;
  testabs( 4, NA, y, f, cnt,failed);

  y := zernike_rx(19,5,0.5);
  f := 0.28446197509765625;
  testrel( 5, NE, y, f, cnt,failed);

  y := zernike_rx(19,5,2.5);
  f := 8.6222178509933948516845703125e11;
  testrel( 6, NE, y, f, cnt,failed);

  y := zernike_rx(14,4,0.75);
  f := -0.298682145774364471435546875;
  testrel( 7, NE, y, f, cnt,failed);

  y := zernike_rx(14,4,0.8246570394661101917);
  f := 0;
  testabs( 8, NA, y, f, cnt,failed);

  y := zernike_rx(14,4,0.875);
  f := 0.3172262174016395875;
  testrel( 9, NE, y, f, cnt,failed);

  y := zernike_rx(42,16,0.75);
  f := -0.1378478860818820635;
  testrel(10, NE, y, f, cnt,failed);

  y := zernike_rx(42,16,0.8597234728144440095);
  f := 0;
  testabs(11, NA, y, f, cnt,failed);

  y := zernike_rx(42,16,0.875);
  f := 0.1826304987525820983;
  testrel(12, NE, y, f, cnt,failed);

  y := zernike_rx(42,16,0.984375);
  f := 0.3032452839343570153;
  testrel(13, NE, y, f, cnt,failed);

  y := zernike_rx(42,16,2.5);
  f := 0.2842735123784586302e27;
  testrel(14, NE, y, f, cnt,failed);

  y := zernike_rx(120,42,0.125);
  f := -0.7251933738244693132e-15;
  testrel(15, NE, y, f, cnt,failed);

  y := zernike_rx(120,42,0.25);
  f := -0.6271346168560805383e-4;
  testrel(16, NE, y, f, cnt,failed);

  y := zernike_rx(120,42,0.5);
  f := 0.9992364238854123563e-1;
  testrel(17, NE, y, f, cnt,failed);

  y := zernike_rx(120,42,0.75);
  f := -0.1509886728548490631e-1;
  testrel(18, NE, y, f, cnt,failed);

  y := zernike_rx(120,42,0.875);
  f := 0.1087275784879156651;
  testrel(19, NE, y, f, cnt,failed);

  y := zernike_rx(120,42,7.5);
