{Part 4 of regression test for SPECFUN unit  (c) 2018  W.Ehrhardt}
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','Complete elliptic integrals (Mathematica)');
  ...
  {Some of the next case would produce large errors or return NaN, because }
  {the range reduction is tuned to extended, with a double rounding error  }
  {of about eps_d the extended argument is equivalent to about 2048 eps_x)}
  {Former versions gave no errors, but have type extended in Specfun, this }
  {has been changed, and the range reduction becomes critical near some    }
  {multiples of Pi/2 or Pi/4}
...
{** Bad red}
  ...
  testrel(108, 4, y, f, cnt,failed);

{** Bad red}
  y := M_EllipticPi(2, 1-4*Pi, 0.5);
  f := 3.212941214490270606;           {Maple/MPMath}
  testrel(109, 4, y, f, cnt,failed);
...
begin
  ...
  testrel(40, 12, y, f, cnt,failed);   {!!! inaccurate 3.2???}
