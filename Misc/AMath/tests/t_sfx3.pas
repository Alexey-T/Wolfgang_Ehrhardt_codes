{Part 3 of regression test for SPECFUNX unit  (c) 2010  W.Ehrhardt}
...
procedure test_normstd_pdfx;
procedure test_normstd_cdfx;
procedure test_normstd_invx;

procedure test_normal_pdfx;
procedure test_normal_cdfx;
procedure test_normal_invx;

procedure test_beta_cdfx;
procedure test_beta_invx;
procedure test_beta_pdfx;

procedure test_t_cdfx;
procedure test_t_invx;
procedure test_t_pdfx;

procedure test_f_pdfx;
procedure test_f_cdfx;
procedure test_f_invx;
...
begin
  ...
  writeln('Function: ','normstd_pdfx');
  {Maple: statevalf[pdf,normald](x)}

  y := normstd_pdfx(NegInf_x);
  f := 0;
...
begin
  ...
  writeln('Function: ','normstd_cdfx');
  {Maple: statevalf[cdf,normald](x)}

  y := normstd_cdfx(NegInf_x);
  f := 0;
  testabs( 1, 0, y, f, cnt,failed);
  ...
  y := normstd_cdfx(-0.125);
  f := 0.45026177516988710702;
  testrel(12, NE, y, f, cnt,failed);

  y := normstd_cdfx(-0.0078125);
  ...
  testrele(26, 4*eps, y, f, cnt,failed);
...
begin
  ...
  y := normstd_invx(0.5009765625);
  f := 0.24478816191106774544e-2;
  testrel( 11, NE, y, f, cnt,failed);

  y := normstd_invx(0.625);
  f := 0.31863936396437516302;
  testrel(12, NE, y, f, cnt,failed);

  y := normstd_invx(0.750);
  f := 0.67448975019608174320;
  testrel(13, NE, y, f, cnt,failed);

  y := normstd_invx(0.9375);
  f := 1.5341205443525463117;
  testrel(14, NE, y, f, cnt,failed);

  y := normstd_invx(0.9990234375);
  f := 3.0972690781987844624;
  testrel(15, NE, y, f, cnt,failed);

  y := normstd_invx(1-ldexp(1,-20));
  f := 4.7630010342678139570;
  testrel(16, NE, y, f, cnt,failed);

  y := normstd_invx(1-ldexp(1,-50));
  ...
  {extended only}
  y := normstd_invx(1-ldexp(1,-60));
  f := 8.7733211690275516819;
  testrel(20, NE, y, f, cnt,failed);

  y := normstd_invx(1e-4900);
  f := -150.17813523043735394;
  testrel(21, NE, y, f, cnt,failed);

  y := normstd_invx(1e-1000);
  f := -67.785685596602619841886;
  testrel(22, NE, y, f, cnt,failed);
...
begin
  ...
  writeln('Function: ','normal_pdfx');
  {Maple: statevalf[pdf,normal[mu,sd]](x)}

  mu := 5;
  sd := 2.0;
  y := normal_pdfx(mu, sd, -70);
  f := 0.8641168661420526104e-306;
  testrel(1, NE, y, f, cnt,failed);

  y := normal_pdfx(mu, sd, -1);
  f := 0.2215924205969003588e-2;
  testrel(2, NE, y, f, cnt,failed);

  y := normal_pdfx(mu, sd, 0);
  f := 0.8764150246784268681e-2;
  testrel(3, NE, y, f, cnt,failed);

  y := normal_pdfx(mu, sd, 2);
  f := 0.6475879783294586381e-1;
  testrel(4, NE, y, f, cnt,failed);

  y := normal_pdfx(mu, sd, 5);
  f := 0.1994711402007163390;
  testrel(5, NE, y, f, cnt,failed);

  y := normal_pdfx(mu, sd, 10);
  f := 0.8764150246784268681e-2;
  testrel(6, NE, y, f, cnt,failed);

  y := normal_pdfx(mu, sd, 80);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','normal_cdfx');

  {statevalf[cdf,normald[mu,sd]](x);}
  mu := 5;
  sd := 2.0;

  y := normal_cdfx(mu, sd, -70);
  f := 0.4605353009581954842e-307;
  testrel(1, NE, y, f, cnt,failed);

  y := normal_cdfx(mu, sd, -1);
  f := 0.1349898031630094527e-2;
  testrel(2, NE, y, f, cnt,failed);

  y := normal_cdfx(mu, sd, 0);
  f := 0.62096653257761351670e-2;
  testrel(3, NE, y, f, cnt,failed);

  y := normal_cdfx(mu, sd, 2);
  f := 0.668072012688580660e-1;
  testrel(4, NE, y, f, cnt,failed);

  y := normal_cdfx(mu, sd, 5);
  f := 0.5;
  testrel(5, NE, y, f, cnt,failed);

  y := normal_cdfx(mu, sd, 10);
  f := 0.993790334674223865;
  testrel(6, NE, y, f, cnt,failed);

  y := normal_cdfx(mu, sd, 20);
  f := 0.9999999999999680911;
  testrel(7, NE, y, f, cnt,failed);

  y := normal_cdfx(mu, sd, 24);
  f := 1.0;
  testrel(8, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','normal_invx');

  mu := 5;
  sd := 2.0;
  y := normal_invx(mu, sd, ldexp(1,-20));;
  f := -4.526002068535627914;
  testrel(1, NE, y, f, cnt,failed);

  y := normal_invx(mu, sd, 0.125);
  f := 2.699301239247983643;
  testrel(2, NE, y, f, cnt,failed);

  y := normal_invx(mu, sd, 0.5);
  f := 5.0;
  testrel(3, NE, y, f, cnt,failed);

  y := normal_invx(mu, sd, 0.75);
  f := 6.348979500392163486;
  testrel(4, NE, y, f, cnt,failed);

  y := normal_invx(mu, sd, 0.875);
  f := 7.300698760752016357;
  testrel(5, NE, y, f, cnt,failed);

  y := normal_invx(mu, sd, 0.9);
  f := 7.563103131089200934;
  testrel(6, NE, y, f, cnt,failed);

  y := normal_invx(mu, sd, 0.99);
  f := 9.652695748081682202;
  testrel(7, NE, y, f, cnt,failed);

  y := normal_invx(mu, sd, 0.9990234375);
  f := 11.19453815639756892;
  testrel(8, NE, y, f, cnt,failed);

  y := normal_invx(mu, sd, 1-ldexp(1,-20));
...
begin
  ...
  writeln('Function: ','beta_cdfx');

  y := beta_cdfx(1.2, 1.3, 0);
  f := 0;
  testabs(1, NE, y, f, cnt,failed);

  y := beta_cdfx(1.2, 1.3, 1e-10);
  f := 1.3443494465428975103e-12;
  testrel( 2, NE, y, f, cnt,failed);

  y := beta_cdfx(1.2, 1.3, 0.001);
  f := 3.3763004250453581348e-4;
  testrel( 3, NE, y, f, cnt,failed);

  y := beta_cdfx(1.2, 1.3, 0.1);
  f := 8.3399782830674834581e-2;
  testrel( 4, NE, y, f, cnt,failed);

  y := beta_cdfx(1.2, 1.3, 0.5);
  f := 5.2978142945129908088e-1;
  testrel( 5, NE, y, f, cnt,failed);

  y := beta_cdfx(1.2, 1.3, 0.9);
  f := 9.3852939722443065921e-1;
  testrel( 6, NE, y, f, cnt,failed);

  y := beta_cdfx(1.2, 1.3, 0.99);
  f := 9.9688643834125438042e-1;
  testrel( 7, NE, y, f, cnt,failed);

  y := beta_cdfx(1.2, 1.3, 0.999);
  f := 9.9984379283306763424e-1;
  testrel( 8, NE, y, f, cnt,failed);

  y := beta_cdfx(1.5, 4.3, 0.9);
  f := 0.99987810769284942730980; {qcalc}
  testrel( 9, NE, y, f, cnt,failed);

  y := beta_cdfx(5, 20, 0.2);
  f := 5.4012267024242087821312e-1;
  testrel(10, NE, y, f, cnt,failed);

  y := beta_cdfx(8, 10, 0.2);
  f := 0.1093431523409920000e-1;
  testrel(11, NE, y, f, cnt,failed);

  y := beta_cdfx(8, 10, 0.8);
  f := 0.9995067502624768;
  testrel(12, NE, y, f, cnt,failed);

  y := beta_cdfx(1, 0.5, 0.9);
  f := 6.8377223398316206680e-1;
  testrel(13, NE, y, f, cnt,failed);

  y := beta_cdfx(0.1, 0.5, 0.99);
  f := 9.82283678979602092297725e-1;
  testrel(14, NE, y, f, cnt,failed);

  y := beta_cdfx(0.1, 0.5, 1);
  f := 1;
  testrel(15, NE, y, f, cnt,failed);

  y := beta_cdfx(1e-4, 1e4, 1e-4);
  f := 9.99978059362107134278786e-1;
  testrel(16, NE, y, f, cnt,failed);

  y := beta_cdfx(1e-4, 1e4, 1e-3);
  f := 9.99999999586219825547e-1;
  testrel(17, NE, y, f, cnt,failed);

  y := beta_cdfx(1e-4, 1e4, 1e-10);
  f := 9.9867703308020004431e-1;
  testrel(18, NE, y, f, cnt,failed);

  {Extreme cases, reduced accuracy}
  y := beta_cdfx(1.2, 1.3, 1e-100);
  f := 1.34434944656489595584e-120;
  testrel(19, 100, y, f, cnt,failed);

  y := beta_cdfx(800, 0.5, 0.9);
  f := 1.5538647445875693462265e-38;
  testrel(20, 200, y, f, cnt,failed);

  y := beta_cdfx(900, 900, 0.515625);
  f := 9.0757434037800433419883e-1;
  testrel(21, 40, y, f, cnt,failed);

  {Case < 0 and >1, not allowed for ibetax}
  y := beta_cdfx(1.2, 1.3, -1);
  f := 0;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','beta_invx');

  y := beta_invx(2, 1, 0.1);
  f := 3.162277660168379332e-1;
  testrel( 1, NE, y, f, cnt,failed);

  y := beta_invx(1, 2, 0.01);
  f := 5.012562893380045266e-3;
  testrel( 2, NE, y, f, cnt,failed);

  y := beta_invx(2, 1, 0.01);
  f := 10e-2;
  testrel( 3, NE, y, f, cnt,failed);

  y := beta_invx(2, 1, 0.05);
  f := 2.236067977499789696e-1;
  testrel( 4, NE, y, f, cnt,failed);

  y := beta_invx(2, 1, 0.99);
  f := 1.989974874213239909*0.5;
  testrel( 5, NE, y, f, cnt,failed);

  y := beta_invx(5, 10, 0.2);
  ...
  y := beta_invx(1.5, 4.25, 0.99);
  f := 0.7194720681939346506;
  testrel(10, NE, y, f, cnt,failed);

  y := beta_invx(5, 20, 0.5);
  f := 1.919240597664992704e-1;
  testrel(11, NE, y, f, cnt,failed);

  y := beta_invx(8, 10, 0.2);
  f := 3.449977502489844803e-1;
  testrel(12, NE, y, f, cnt,failed);

  y := beta_invx(8, 10, 0.8);
  f := 5.427825084949223437e-1;
  testrel(13, NE, y, f, cnt,failed);

  y := beta_invx(1, 0.5, 0.9);
  f := 0.99;
  testrel(14, NE, y, f, cnt,failed);

  y := beta_invx(1800, 0.5, 0.9);
  f := 9.999956130742314815e-1;
  testrel(15, NE, y, f, cnt,failed);

  y := beta_invx(0.1, 0.5, 0.98);
  f := 9.872767968062899295e-1;
  testrel(16, NE, y, f, cnt,failed);

  y := beta_invx(0.5, 0.125, 0.9);
  f := 9.999999663947204350e-1;
  testrel(17, NE, y, f, cnt,failed);

  y := beta_invx(900, 900, 0.9);
  f := 5.151018817560778887e-1;
  testrel(18, NE, y, f, cnt,failed);

  y := beta_invx(1800, 0.5, 1e-4);
  f := 9.958036054195801287e-1;
  testrel(19, NE, y, f, cnt,failed);

  y := beta_invx(4500,40,0.125);
  f := 0.9895767941483560347;
  testrel(20, NE, y, f, cnt,failed);

  y := beta_invx(100,102,0.5);
  f := 0.4950331343781250235;
  testrel(21, NE, y, f, cnt,failed);

  y := beta_invx(100,102,0.875);
  f := 0.5355056613331838346;
  testrel(22, NE, y, f, cnt,failed);

  y := beta_invx(100,300,0.5);
  f := 0.2495830001077997259;
  testrel(23, NE, y, f, cnt,failed);

  y := beta_invx(100,300,0.625);
  f := 0.2565236334554090243;
  testrel(24, NE, y, f, cnt,failed);

  y := beta_invx(1000,200,0.125);
...
begin
  cnt := 0;
  failed := 0;
  {Maple: f:= (a,b,x) -> x^(a-1)*(1-x)^(b-1) / Beta(a,b);}

  writeln('Function: ','beta_pdfx');

  y := beta_pdfx(2, 3, 0.1);
  f := 0.972;
  testrel( 1, NE, y, f, cnt,failed);

  y := beta_pdfx(3, 2, 0.01);
  f := 0.1188e-2;
  testrel( 2, NE, y, f, cnt,failed);

  y := beta_pdfx(2, 3, 0.01);
  f := 0.117612;
  testrel( 3, NE, y, f, cnt,failed);

  y := beta_pdfx(2, 3, 0.05);
  f := 0.541500;
  testrel( 4, NE, y, f, cnt,failed);

  y := beta_pdfx(2, 3, 0.99);
  f := 0.1188e-2;
  testrel( 5, NE, y, f, cnt,failed);

  y := beta_pdfx(5, 10, 0.2);
  f := 2.1496311316480;
  testrel( 6, NE, y, f, cnt,failed);
...
begin
  ...
  testrel(58, 4500, y, f, cnt,failed);   {!!!!!!!!!!!!}
...
begin
  ...
  y := t_invx(nu, 0.9990234375);
  f := 325.94830079770134898;
  testrel(11, NE, y, f, cnt,failed);

  y := t_invx(nu, 1-ldexp(1,-20)); {0.99999904632568359375}
...
begin
  ...
  testrel(11, 2*NE2, y, f, cnt,failed);
  ...
  testrel(18, 350, y, f, cnt,failed);          {Intel NE2}
  ...
  testrel(20, 2*NE2, y, f, cnt,failed);
...
begin
  ...
  y := f_cdfx(nu1, nu2, 1/1024);
  f := 0.70796121838458865266e-13;
  testrel( 1, NE1, y, f, cnt,failed);

  y := f_cdfx(nu1, nu2, 0.125);
  f := 0.10696408932185717088e-2;
  testrel( 2, NE1, y, f, cnt,failed);

  y := f_cdfx(nu1, nu2, 0.5);
  f := 0.13538507460415484644;
  testrel( 3, NE1, y, f, cnt,failed);

  y := f_cdfx(nu1, nu2, 0.75);
  ...
  y := f_cdfx(nu1, nu2, 100);
  f := 0.99999999999278539289;
  testrel( 9, NE1, y, f, cnt,failed);

  y := f_cdfx(nu1, nu2, 500);
...
begin
  ...
  y := f_invx(nu1, nu2, 1e-6);
  f := 0.27250601258372329764e-1;
  testrel( 1, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.001953125);
  f := 0.14449168758699289717;
  testrel( 2, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.03125);
  f := 0.30351965339193473178;
  testrel( 3, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.125);
  f := 0.48469389512563697769;
  testrel( 4, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.25);
  f := 0.65198482424610119623;
  testrel( 5, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.5);
  f := 0.97731605405255776796;
  testrel( 6, NE+2, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.75);
  f := 1.4490744742928857851;
  testrel( 7, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.9);
  f := 2.0593194961568176081;
  testrel( 8, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.95);
  f := 2.5437185496928079783;
  testrel( 9, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.99);
  f := 3.8049397459502750704;
  testrel(10, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 0.9990234375);
  f := 6.1078009092810849030;
  testrel(11, NE, y, f, cnt,failed);

  y := f_invx(nu1, nu2, 1-ldexp(1,-20)); {0.99999904632568359375}
  ...
  y := f_invx(nu1, nu2, 0.001953125);
  (* Slightly edited due to Delphi/FPC bug:
  if 0.15050733921182494928 > 0.150507339211824949284791727242 then begin
    writeln('bug');
  end;
  *)
  f := 0.1505073392118249492;
  testrel(13, NE2, y, f, cnt,failed);
