{T_AMATHM - regression test main unit for AMATH  (c) 2009-2018  W.Ehrhardt}
...
procedure test_amath_main;
  {-AMath regression test via single procedure call}

implementation

uses
  amath, t_amath0, t_amath1, t_amath2;

const
  rmTest: TFPURoundingMode = rmNearest;  {rmNearest, rmDown, rmUp, rmTruncate}
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e4000;    ty: 9211.0335191567426814)
         );
const
  name = 'arccosh';
var
...
const
  NT=9;
  TData: array[1..NT] of TPair = (
           (tx: 1e-1000; ty: 2303.2782401746056293),
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e1000;  ty: 2303.2782401746056293)
         );
const
  name = 'arcsinh';
var
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e4;     ty: 0.44034091128314607936e4343)
         );
const
  name = 'cosh';
var
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e19;      ty: -0.3749051695507178302),
           (tx: 1e26;      ty: -0.5211662981646172019),
           (tx: 0.0;       ty:  0.9872460775989134842),
           (tx: 1.0;       ty:  0.7148812462333444532)
         );
const
  name = 'cos';
var
...
begin
  ...
  for i:=1 to NT do with TData[i] do begin
    x := tx;
    if x=0.0 then x := ldexp(1.0,1000)
    else if x=1.0 then x := ldexp(1.0,16000);
    y := ty;
    z := cos(x);
    inc(cnt);
    if reldevx(z,y) > EPS then begin
...
begin
  ...
  for i:=1 to NT do with TData[i] do begin
    x := tx;
    if x=-1.0 then x := ldexp(1.0,40)+0.125
    else if x=-2.0 then x := ldexp(1.0,66);
    y := ty;
    z := cosPi(x);
    inc(cnt);
    if reldevx(z,y) > EPS then begin
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e26;    ty: -0.6106545837616196399),
           (tx: 1.0;     ty: -6.2012281179184650510),   {2^1000}
           (tx: 2.0;     ty:  1.0223603233481935071));  {2^16000}
const
  name = 'cot';
var
  i, cnt, failed: integer;
...
begin
  ...
  for i:=1 to NT do with TData[i] do begin
    x := tx;
    if x=1.0 then x := ldexp(1.0,1000)
    else if x=2.0 then x := ldexp(1.0,16000);
    y := ty;
    z := cot(x);
    inc(cnt);
    if reldevx(z,y) > EPS then begin
    ...
    if abs(x)<1e4 then begin
      ...
      inc(cnt);
      {y > Pi, Use SEPS here}
      if reldevx(z,y) > SEPS then begin
        inc(failed);
        writeln(x:19,' ',y:19, ' ',z:19,' ',reldevx(z,y):18);
      end;
      x := tx-Pi;
...
const
  ...
  TData: array[1..NT] of TPair = (
           (tx: 0.0;     ty: 1.0),
           (tx: 1e-10;   ty: 1.0),
           (tx: 1e-7;    ty: 1.000000000000001667),
           (tx: 1e-5;    ty: 1.000000000016666667),
           ...
           (tx: 10000.0; ty: 0.44034091128314607936e4339),
           (tx: 11360.0; ty: 0.1693971478842983140e4930)
         );
const
  name = 'sinhc';
var
...
begin
  ...
  with TData[NT] do begin
    ...
    z := sinhc(x);
    inc(cnt);
    if reldevx(z,y) > 200*EPS then begin
      inc(failed);
      writeln(x:19,' ',y:19, ' ',z:19,' ',reldevx(z,y):18);
    end;
  end;
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e4;     ty: 0.4403409112831460794e4343)
         );
const
  name = 'sinhmx';
var
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e26;     ty:  0.8534551480056754105),
           (tx: 0;        ty: -0.1592017030862424382),  {2^1000}
           (tx: 1;        ty:  0.6992458822072964750)   {2^16000}
         );
const
  name = 'sin';
var
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e-8;     ty:  0.99999999999999998333),
           (tx: 2e-10;    ty:  1.0000000000000000000),
           (tx: 1e-20;    ty:  1.0000000000000000000),
           (tx: 1e-4000;  ty:  1.0000000000000000000),
           (tx: 0;        ty:  1)
         );
const
  name = 'sinc';
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e26;    ty: -1.6375869871311218646),
           (tx: 1.0;     ty: -0.1612583799506580567),   {2^1000}
           (tx: 2.0;     ty:  0.9781287254233769093));  {2^16000}
const
  name = 'tan';
var
  i, cnt, failed: integer;
...
begin
  ...
  inc(cnt);
  z := tanPi(0.5);
  if not isinf(z) then begin
    inc(failed);
    writeln(x:19,' ',PosInf_x:19, ' ',z:19);
  end;
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 22.0;    ty: 0.99999999999999999984),
           (tx: 24.0;    ty: 1.0),
           (tx: 1e1000;  ty: 1.0)
         );
const
...
var
  ...
  begin
    ...
    if reldevx(a,b) > SEPS then begin
      inc(failed);
      writeln('  power: ',x:5:2,' ',y:5:2, ' ',a:19,' ',b:19,' ',reldevx(a,b):16);
    end;
    if x>0.0 then begin
      a := logbase(x,b);
      inc(cnt);
      if reldevx(a,y) > SEPS then begin
        inc(failed);
        writeln('logbase: ',x:5:2,' ',y:5:2, ' ',a:19,' ',b:19,' ',reldevx(a,y):16);
      end;
      a := logN(x,b);
      inc(cnt);
      if reldevx(a,y) > SEPS then begin
        inc(failed);
        writeln('   logN: ',x:5:2,' ',y:5:2, ' ',a:19,' ',b:19,' ',reldevx(a,y):16);
      end;
    end;
  end;
begin
...
var
  ...
  begin
    ...
    if reldevx(a,b) > SEPS then begin
      inc(failed);
      writeln('  powm1: ',x:5:2,' ',y:5:2, ' ',a:19,' ',b:19,' ',reldevx(a,b):16);
    end;
  end;
...
var
  ...
  n: longint;
const
  vs: THexExtW = ($0000,$0000,$2000,$0000,$0000);  {1.2825405666778921151E-4937}
  ...
  begin
    ...
    if reldevx(a,b) > SEPS then begin
      inc(failed);
      writeln('  !!! ',x:6:2,' ',n:2, ' ',a:19,' ',b:19,' ',reldevx(a,b):16);
    end;
  end;
...
var
  ...
  procedure itest(tx,ty,b: extended);
  var
    a,x,y: extended;
  begin
    x := tx;
    ...
    a := hypot(x,y);
    inc(cnt);
    if reldevx(a,b) > EPSX then begin
      inc(failed);
      writeln(x:8:2,' ',y:8:2, ' ',a:19,' ',b:19,' ',reldevx(a,b):16);
    end;
...
begin
  ...
  itest(1e-20,1.0,1.0);
  itest(1,1e-20,1.0);
  itest(0,MinExtended,MinExtended);
  itest(MinExtended,0,MinExtended);
  itest(MinExtended,MinExtended,MinExtended*sqrt(2));
...
var
  ...
  procedure itest(n: integer; x,y,z,b: extended);
  var
    a: extended;
  begin
    a := hypot3(x,y,z);
    ...
    if reldevx(a,b) > EPSX then begin
      inc(failed);
      writeln(' Test ',n, ' failed; rel. dev. = ', reldevx(a,b):16);
    end;
  end;
...
begin
  ...
  itest( 6, MinExtended,0.0,0.0, MinExtended);
  itest( 7, 0.0,MinExtended,0.0, MinExtended);
  itest( 8, 0.0,0.0,MinExtended, MinExtended);
  itest( 9, MaxExtended,0.0,0.0, MaxExtended);
  itest(10, 0.0,MaxExtended,0.0, MaxExtended);
  itest(11, 0.0,0.0,MaxExtended, MaxExtended);
  itest(12, MinExtended,MinExtended,MinExtended, sqrt(3)*MinExtended);
...
var
  ...
  xx,yx: array[0..20] of extended;
  i,j,f: integer;
  cnt, failed: integer;
  dd,dx,e,ee,s,x,nx,nd,rd,rx,sd,sx,q,sa: extended;
begin
  ...
  while (i<=1000) and (failed=0) do begin
    ...
    if abs(s-dd) > 2*EPSD*sa  then begin
      ...
      writeln(reldevx(dd,s)/EPSD);
    end;
    if abs(s-dx) > 2*EPSX*sa then begin
      inc(failed);
      writeln('random dot2x failed. i=',i,':', dx:24,' ',s:24);
      writeln(reldevx(dx,s)/EPSX);
    end;
    ...
    rx := rmsx(xx,21);
    x  := sqrt(dx/21);
    if abs(sx-dx)>6*EPSX*abs(dx) then begin
      inc(failed);
      writeln('random sumsqrx failed. i=',i,':', sx:24,' ',dx:24);
    end;
    if abs(x-rx)>6*EPSX*abs(x) then begin
      inc(failed);
      writeln('random rmsx failed.  i=',i,':', rx:24,' ',x:24);
    end;

    dx := sqrt(dx);
    nx := norm2x(xx,21);
    if abs(nx-dx)>6*EPSX*abs(dx) then begin
      inc(failed);
      writeln('random norm2x failed. i=',i,':', nx:24,' ',dx:24);
    end;
    inc(i);
  end;
  ...
  while (e>EPSD) and (f=0) do begin
    ...
    yx[2] := -1;
    dx := dot2x(xx,yx,3);
    if abs(dx-ee)>EPSX*ee then begin
      inc(failed);
      f := 1;
      writeln('eps dot2x failed. i=',i,':', dx:24,' ',e:24);
    end;
    e := e/1024;
    inc(i);
  end;
  ...
  while (abs(q)<=0.99) and (f=0) do begin
    ...
    if abs(sx-x)>4*EPSX*abs(x) then begin
      inc(failed);
      f := 1;
      writeln('sum2x failed. q=',q:15:12,' ', sx:20,' ',x:20);
    end;
    sd := sumsqr(xd,21);
    sx := sumsqrx(xx,21);
    ...
    if abs(sx-x)>8*EPSX*abs(x) then begin
      inc(failed);
      f := 1;
      writeln('sumsqrx failed. q=',q:15:12,' ', sx:20,' ',x:20);
    end;
    q := -q*1.125;
  end;
  if failed>0 then writeln('*** failed: ',failed,' of ',cnt)
...
const
  NT=14;
  TData: array[1..NT] of TPair = (
           (tx: 0.0;       ty: 0.71434350759724481535e-1650),  {succx(0)=0.36451995318824746025e-4950}
           (tx: 0.0;       ty: 0.0),
           (tx: 1e-4900;   ty: 0.46415888336127788930e-1633),
           ...
           (tx: 1e4931;    ty: 0.46415888336127788927e1644)
        );
const
  name = 'cbrt';
var
...
begin
  ...
  failed := 0;
  for i:=1 to NT do with TData[i] do begin
    if i=1 then x := succx(0) else x := tx;
    y := ty;
    z := cbrt(x);
    inc(cnt);
    if reldevx(z,y) > EPS then begin
...
begin
  ...
  if reldevx(z,y) > EPS then begin
    inc(failed);
    writeln(n:4,' ',x:16,' ',y:18, ' ',z:18,' ',reldevx(z,y):18);
  end;
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: 1e24;     ty:  4.796410966144272776)
         );
const
  name = 'rem_2pi';
var
...
const
  ...
  TData: array[1..NT] of TPair = (
           ...
           (tx: -1e12;    ty:   0.657624759136786467),
           (tx: -1e19;    ty:   1.955090807925410481),
           (tx: 1e24;     ty:  -1.486774341035313701)
         );
const
  name = 'rem_2pi_sym';
var
...
const
  RMS: array[TFPURoundingMode] of string[10] =
        ('rmNearest', 'rmDown', 'rmUp', 'rmTruncate');
begin
  total_cnt    := 0;
  total_failed := 0;
  writeln('T_AMATH - main test program for AMATH unit V',AMath_Version,'   (c) 2009-2018  W.Ehrhardt');

  if SetRoundMode(rmTest)=rmTest then {nothing};
  if rmTest<>rmNearest then begin
    EPS    := 2E-18;
    SEPS   := 2E-17;
    EPSX   := 2*eps_x;
    EPSD   := 2*eps_d;
    writeln('*** Using rounding mode ',RMS[rmTest]);
  end
  else begin
    EPS    := 1E-18;
    SEPS   := 1E-17;
...

{
  ...
  writeln;
  writeln('Tests double-extended functions:');
  test_xdivrem;
  test_fma_x;
  test_misc2x;
  ...
  if GetRoundMode<>rmTest then writeln('!! Rounding mode changed')
  else writeln;
