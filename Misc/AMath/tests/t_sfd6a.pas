{Part 6a of regression test for SPECFUN unit  (c) 2011  W.Ehrhardt}
...
begin
  ...
  {double(1e-10) would be = 1.0000000000000000364e-10, use exact 0.5^35}
  y := sph_bessel_kne(25,ldexpd(1,-35));  {x=0.2910383045673370361328125e-10}
  f := 0.7945024363300855607e306;
  testrel(14, NE, y, f, cnt,failed);

  y := sph_bessel_kne(-25,ldexpd(1,-35));
  f := 0.4718992694778096800e294;
  testrel(15, NE, y, f, cnt,failed);
...
begin
  ...
  y := sph_bessel_kn(25,ldexpd(1,-35));  {x=0.2910383045673370361328125e-10}
  f := 0.7945024363069624965e306;
  testrel(14, NE, y, f, cnt,failed);

  y := sph_bessel_kn(-25,ldexpd(1,-35));
  f := 0.4718992694640756036e294;
  testrel(15, NE, y, f, cnt,failed);
...
begin
  ...
  testrel(12, 2*NE, y, f, cnt,failed);
...
begin
  ...
  testrel(25, NE, berp, fber, cnt,failed);

  fbei := 0.1859891445884795168e30;
  testrel(26, NE, beip, fbei, cnt,failed);

  fker := -0.8766246185882256455e-32;
  testrel(27, NE, kerp, fker, cnt,failed);

  fkei := 0.2292564824625330302e-31;
  testrel(28, NE, keip, fkei, cnt,failed);

  {----------------------------------}
  x := 500.0;
  kelvin_der(x, berp, beip, kerp, keip);
  fber := -0.3103065329961790330e152;
  testrel(29, NE, berp, fber, cnt,failed);

  fbei := 0.5451844726091405450e152;
  testrel(30, NE, beip, fbei, cnt,failed);

  fker := -0.4220529386541760937e-155;
  testrel(31, NE, kerp, fker, cnt,failed);

  fkei := 0.1537225557166260039e-154;
  testrel(32, NE, keip, fkei, cnt,failed);

  {--------------------------------------------------}
  x := 5.0;
  fber := -3.845339473262154524;
  berp := kelvin_berp(x);
  testrel(33, NE, berp, fber, cnt,failed);
  ...
  kerp := kelvin_kerp(x);
  testrel(35, NE, kerp, fker, cnt,failed);
  ...
  berp := kelvin_berp(x);
  testrel(37, NE, berp, fber, cnt,failed);
  ...
  beip := kelvin_beip(x);
  testrel(42, NE, beip, fbei, cnt,failed);
  ...
  berp := kelvin_berp(x);
  testrel(45, NE, berp, fber, cnt,failed);
...
{$endif}
  ...
  {negative nu}
  y := struve_l(-4.125, 10);
  f := 0.1164501893528164086e4;
  testrel(39, NE, y, f, cnt,failed);

  y := struve_l(-4.125, 30);           {AE}
  ...
  y := struve_l(-0.125, 30);           {AE}
  f := 0.7814652427883314765e12;
  testrel(43, NE, y, f, cnt,failed);

  y := struve_l(-0.125, 600);          {AE}
  f := 0.6146225307628329318e259;
  testrel(44, NE, y, f, cnt,failed);

  y := struve_l(-31.25, 1.0);
  ...
  y := struve_l(-31.25, 20.0);
  f := 3.108364052786170322;
  testrel(47, NE, y, f, cnt,failed);

  y := struve_l(-31.25, 40.0);
  f := 0.1120269549800670326e12;
  testrel(48, NE, y, f, cnt,failed);

  y := struve_l(-31.25, 47);           {AE}
...
begin
  ...
  y := struve_h(-31.25, 40);        {int}
  f := 0.1077952026367204399;
  testabs(66, NA, y, f, cnt,failed);

  y := struve_h(-31.25, 48);        {AE}
  f := 0.9127208388797704561e-2;
  testabs(67, NA, y, f, cnt,failed);

  y := struve_h(-10, 30);           {int}
