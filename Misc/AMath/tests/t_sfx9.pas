{Part 9 of regression test for SPECFUNX unit  (c) 2013-2018  W.Ehrhardt}
...
procedure test_hyperg_1f1x;
procedure test_hyperg_1F1rx;
procedure test_hyperg_2f1x;
procedure test_hyperg_2F1rx;
procedure test_hyperg_2F0x;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','hyperg_1F1x');

  {GSL integer a,b}
  y := hyperg_1F1x(1, 1, 0.5);
  f := 1.6487212707001281468;
  testrel(1, NE, y, f, cnt,failed);

  y := hyperg_1F1x(1, 2, 500.0);
  ...
  testrel(60, NE4, y, f, cnt,failed);  {!!!!}

  y := hyperg_1F1x(-10, -20, -100.0);
  {GSL 1.64284868563391159e-35; ??? Totally wrong}
  ...
  y := hyperg_1F1x(-100, 20.1, 1);
  f := 0.0021267655527278456412;
  testrel(110, NE3, y, f, cnt,failed);

  y := hyperg_1F1x(-100, 20.1, 10);
  f := 2.0908665169032186979e-11;
  testrel(111, NE1, y, f, cnt,failed);

  y := hyperg_1F1x(-100, 20.1, 100);
  ...
  y := hyperg_1F1x(10,8000.0,11500.0);
  f := 0.1081895784081134874e289;
  testrel(146, NE1, y, f, cnt,failed);

  {b = 2a case}
  y := hyperg_1F1x(2000,4000,10);
  f := 148.8775590092748498;
  testrel(147, NE, y, f, cnt,failed);

  y := hyperg_1F1x(0.125,0.25,5);
  ...
  testrel(158, NE2, y, f, cnt,failed);   {!!}

  y := hyperg_1F1x(-1000.5, 1, -2);
  f := 0.1143992271759473033e38;
  testrel(159, NE, y, f, cnt,failed);

  y := hyperg_1F1x(-1000.5, -1.5, 2);
  f := -563.1980796259134972;
  testrel(160, NE3, y, f, cnt,failed);   {!!}
  ...
  testrel(162, NE4, y, f, cnt,failed);

  y := hyperg_1F1x(-1000.5, -1.5, -2);
  f := 0.3301330588826002370e42;
  testrel(163, NE, y, f, cnt,failed);

  y := hyperg_1F1x(-1000, -1000.1, 200.0);
  f := 7.066514294896245043e+86;
  testrel(164, NE2, y, f, cnt,failed);

  {Related to igammat}
  y := hyperg_1F1x(1,-9.5, -234.5);
  f := -0.4709619719604298796e-1;
  testrel(165, NE, y, f, cnt,failed);

  {GSL, 'fixed' in AMath 1.29.01}
  y := hyperg_1F1x(-1000, -1000.1, 10.0);
  f := 22004.341698908631636;
  testrel(166, NE, y, f, cnt,failed);

  {very small a and b}
  y := hyperg_1F1x(1e-20, 1e-22, 1.0);
  f := 172.8281828459045235;
  testrel(167, NE, y, f, cnt,failed);

  y := hyperg_1F1x(1e-19, -1e-20, -2.5);
  f := 10.17915001376101205;
  testrel(168, NE, y, f, cnt,failed);

  {extended only}
  y := hyperg_1F1x(0.125,0.25,10000);
  f := 0.1340240102785329774e4343;
  testrel(200, NE, y, f, cnt,failed);

  y := hyperg_1F1x(0.125,0.25,11340);
  f := 0.1188395957794066052e4925;
  testrel(201, NE, y, f, cnt,failed);

  y := hyperg_1F1x(-3.25, -6.5, 10000);
  f := -0.2753393409793698243e4354;
  testrel(202, NE, y, f, cnt,failed);

  y := hyperg_1F1x(3,6,10000);
  f := 0.5280921114927426548e4333;
  testrel(203, NE, y, f, cnt,failed);

  y := hyperg_1F1x(100,201,11500);
  f := 6.386981289548717729e+4802;
  testrel(204, 3200, y, f, cnt,failed);
...
(*
  ...
  y := hyperg_1F1x(10, 1.1, -500);
  f := -3.400379216707701408e-23;    {!!!!!!!!!}
  writeln(1-y/f);

  y := hyperg_1F1x(50, 1.1, -100);
  f := 4.632883869540640460e-24;     {!!!!!}
  writeln(1-y/f);

  y := hyperg_1F1x(50, 1.1, -110.0);     {!!!!!}
  ...
  y := hyperg_1F1x(-26.1, 2.0, 100.0);
  {f := 1.341557199575986995e+19;}
  f := 1.341557199575986910e19;
  writeln(1-y/f);

  {GSL: 'Horrible', AMath too}
  y := hyperg_1F1x(100, 10.1, -220);
  f := -4.296130300021696573e-64;
  writeln(1-y/f);

  {After disallowing Kummer for negative integer b err is ~ 1.369E-11}
  y := hyperg_1F1x(-100, -200, -100.0);
  f := 4.341117950073364570e-25; {wolfram}
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','hyperg_1F1rx');

  {functions.wolfram.com}
  y := hyperg_1F1rx(-6, -4, -0.5);
  f := 0.203125;
  testrel(1, NE, y, f, cnt,failed);
  ...
  y := hyperg_1F1rx(-6, -4+ldexp(1,-40), 0.5);
  f := -0.1718749999519580651;
  testrel(3, NE1, y, f, cnt,failed);

  y := hyperg_1F1rx(-6, -6, 0.5);
  f := 0;
  testrel(4, NE, y, f, cnt,failed);

  y := hyperg_1F1rx(-6, -6-ldexp(1,-32), 0.5);
  f := -2.763881640260390814e-7;
  testrel(5, NE2, y, f, cnt,failed);

  y := hyperg_1F1rx(5, -1, 0.5);
...
begin
  ...
  for n:=1 to 20 do begin
    y := hyperg_2F1x(-n,b,c,1);
    f := pochhammerx(c-b,n)/pochhammerx(c,n);
    testrel(n, NE, y, f, cnt,failed);
  end;

  {-- GSL -----------------------}
  y := hyperg_2F1x(1, 1, 1, 0.5);
  f := 2.0;
  testrel(21, NE, y, f, cnt,failed);

  y := hyperg_2F1x(8, 8, 1, 0.5);
  f := 12451584.0;
  testrel(22, NE, y, f, cnt,failed);
  ...
  y := hyperg_2F1x(25, 25, 1, -0.5);
  f := -2.9995530823639545027e-06;
  testrele(40, 4e-15, y, f, cnt,failed);

  y := hyperg_2F1x(1.5, 0.5, 2.0, 1.0-1.0/64.0);
  f := 3.17175539044729373926;
  testrel(41, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1.5, 0.5, 2.0, 1.0-1.0/1024.0);
  f := 4.90784159359675398250;
  testrel(42, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1.5, 0.5, 2.0, 1.0-1.0/65536.0);
  f := 7.552266033399683914;
  testrel(43, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1.5, 0.5, 2.0, 1.0-1.0/16777216.0);
  ...
  y := hyperg_2F1x(1.5, 0.5, 2.0, -1.0+1.0/1048576.0);
  f := 0.7627599110890647380;
  testrel(47, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1.5, 0.5, 3.0, 1.0);
  f := 1.697652726313550248;
  testrel(48, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1.5, -4.2, 3.0, 1.0);
  f := 0.15583601560025710650;
  testrel(49, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-7.4, 0.7, -1.5, 1.0);
  f := -0.3447886695924658500;
  testrel(50, NE, y, f, cnt,failed);

  y := hyperg_2F1x(0.1, -2.7, -1.5, 1.0);
  f := 1.059766766063610123;
  testrel(51, NE, y, f, cnt,failed);

  y := hyperg_2F1x(0, -2, -4, 0.5);
  f := 1.0;
  testrel(52, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-10.34, 2.05, 3.05, 0.1725);
  f := 0.3104735522131300104;
  testrel(53, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-9.99999999999, 2.05, 3.05, 0.1725);
  f := 0.3214193463019748754;
  testrel(54, NE, y, f, cnt,failed);

  y := hyperg_2F1x(11, -1, 11.0/2.0, 0.125 );
  f := 0.75;
  testrel(55, NE, y, f, cnt,failed);

  {--- Forrey --------------------------}
  x := 0.2;
  y := hyperg_2F1x(0.5,1.0,1.5,-x*x);
  f := arctan(x)/x;
  testrel(56, NE, y, f, cnt,failed);
  ...
  y := hyperg_2F1x(1.1,0,-1,1);
  f := 1;
  testrel(98, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1.2,-1,0.5,1);
  f := -1.4;
  testrel(99, NE, y, f, cnt,failed);

  y := hyperg_2F1x(0.1,0.2,0.3,0.5);
  ...
  y := hyperg_2F1x(10,5,-300.5,0.5);
  f := 0.9211827166328477894;
  testrel(114, NE, y, f, cnt,failed);

  y := hyperg_2F1x(2.25, 3.75, -0.5, -1);
  f := -0.6312206769497026561;
  testrel(115, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1.5,3,3,-2);
  f := 0.1924500897298752548;
  testrel(116, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1.5,3,3,0.75);
  f := 8;
  testrel(117, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-1,2,3.5,-1000);
  f := 572.4285714285714286;
  testrel(118, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-1,2,3.5,10);
  f := -4.714285714285714286;
  testrel(119, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-1,2,1,10);
  f := -19;
  testrel(120, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-1,2,3.5,-1e20);
  f := 0.5714285714285714286e20;
  testrel(121, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-1.125,2.75,3.5,-1e20);
  f := 0.2422430230688187783e23;
  testrel(122, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-1,2.75,3.5,-1e50);
  ...
  y := hyperg_2F1x(1.5, 1000, 2.25, 0.5);
  f := 0.7705550948378541170e299;
  testrel(125, NE1, y, f, cnt,failed);

  y := hyperg_2F1x(1.125,2.75,3.5,-1e50);
  ...
  y := hyperg_2F1x(1,2,0.5,0.75);
  f := 65.53118474162122841;
  testrel(128, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1,2,3.5,0.75);
  ...
  y := hyperg_2F1x(1,0.5,3.5,-0.75);
  f := 0.9130746471820776254;
  testrel(131, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1,0.5,3.5,0.9375);
  ...
  y := hyperg_2F1x(1,2,3.5,0.9375);
  f := 2.962465226592878248;
  testrel(134, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1,0.5,3.5,-1.75);
  f := 0.8343569941403365761;
  testrel(135, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1,-1,3.5,-1.75);
  f := 1.5;
  testrel(136, NE, y, f, cnt,failed);

  y := hyperg_2F1x(1,2,3.5,-2.5);
  f := 0.4408735093563970956;
  testrel(137, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-50,6.25,-51,-2);
  f := 0.6449742538251207026e20;
  testrel(138, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-50,6.25,-51.5,-2);
  f := 0.9886773025921599926e19;
  testrel(139, NE, y, f, cnt,failed);

  y := hyperg_2F1x(-80,2.75,3.5,1e50);
  f := 0.7539779180460486919e3999;
{$ifdef FPC}
  testrel(140, NE2, y, f, cnt,failed);
{$else}
  testrel(140, NE1, y, f, cnt,failed);
{$endif}

(*
  {Problem cases}

  y := hyperg_2F1x(-500,6.25,-600,-1.5);
  f := 0.627841772681655481946316866475e-2;
  writeln(1-y/f);

  y := hyperg_2F1x(-500, 6.25,-600,-1.25);
  f := 0.115277178300449647772660141090e-1;
  writeln(1-y/f);

  y := hyperg_2F1x(-500,6.25,-600,-2);
  f := 5.59957326424345415374046211996e28;
  writeln(1-y/f);

  y := hyperg_2F1x(-125,6.25,-150,-2);
  f := -0.506691674075524839352943058984e14;
  writeln(1-y/f);

  y := hyperg_2F1x(-500,6.25,-600,-5);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','hyperg_2F1rx');

  {functions.wolfram.com}
  y := hyperg_2F1rx(0, 5, 0, 0.5);
  f := 0.0;
  testrel(1, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(1, 5, 0, 0.5);
  f := 160.0;
  testrel(2, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(1, 5, ldexp(1,-40), 0.5);
  f := 159.9999999997519665;
  testrel(3, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(-6, 5, -2, 1);
  f := 840.0;
  testrel(4, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(-6, 5, -5, 0.5);
  f := 2362.50;
  testrel(5, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(-6, 5, -6, 0.5);
  f := 0;
  testrel(6, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(1.25, -6, -6, 0.5);
  f := 0;
  testrel(7, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(-6, 5, -4, 0.5);
  f := -472.5;
  testrel(8, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(-6, 5, -4+ldexp(1,-40), 0.5);
  f := -472.5000000001784741;
  testrel(9, NE1, y, f, cnt,failed);

  y := hyperg_2F1rx(-6, 5, -4, -2);
  f := 12579840.0;
  testrel(10, NE, y, f, cnt,failed);
  ...
  y := hyperg_2F1rx(1, 1, 1, 0.5);
  f := 2.0;
  testrel(13, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(8, 8, 1, 0.5);
  f := 12451584.0;
  testrel(14, NE, y, f, cnt,failed);
  ...
  y := hyperg_2F1rx(8, 8, 5, 0.5);
  f := 175.2380952380952381;
  testrel(19, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(9, 9, -1.5, 0.99);
  ...
  y := hyperg_2F1rx(5, 5, -1, 0.5);
  f := 1237440;
  testrel(22, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(5, 5, -10, 0.5);
  f := 6.8070553334784e16;
  testrel(23, NE, y, f, cnt,failed);

  y := hyperg_2F1rx(5, 5, -100, 0.5);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','hyperg_2F0x');

  {Computed with MPMath 0.17}
  y := hyperg_2F0x(1.5, 2.5, -3.5);
  f := 0.06232362765198486065;
  testrel(1, NE1, y, f, cnt,failed);

  y := hyperg_2F0x(-2, 1.5, 3.5);
  f := 36.4375;
  testrel(2, NE, y, f, cnt,failed);

  y := hyperg_2F0x(1.5, -2, 3.5);
  f := 36.4375;
  testrel(3, NE, y, f, cnt,failed);

  y := hyperg_2F0x(1.5, 0, 3.5);
  f := 1;
  testrel(4, NE, y, f, cnt,failed);

  y := hyperg_2F0x(1.5, 2.5, -100);
  f := 0.7092776453693892794e-3;
  testrel(5, NE, y, f, cnt,failed);

  y := hyperg_2F0x(15, 2.5, -100);
  ...
  y := hyperg_2F0x(-2.5, 1.5, 0);
  f := 1;
  testrel(9, NE, y, f, cnt,failed);

  y := hyperg_2F0x(1.5, 2.5, -0.01);
  f := 0.9640594689959897311;
  testrel(10, NE1, y, f, cnt,failed);

  y := hyperg_2F0x(50, 50, -0.03125);
  f := 2.367887757397116595e-17;
  testrel(11, NE, y, f, cnt,failed);

  y := hyperg_2F0x(50, 30, -0.03125);
  f := 1.519977201318170588e-11;
  testrel(12, NE, y, f, cnt,failed);

  y := hyperg_2F0x(5, 10, -0.03125);
  f := 0.2778878346657118121;
  testrel(13, NE, y, f, cnt,failed);

  {Asyptotic sum}
  y := hyperg_2F0x(1.5, 0.5, 0.01);
  ...
  y := hyperg_2F0x(1.5, 0.5, 0.01953125);
  f := 1.015218046373990184;
  testrel(16, NE, y, f, cnt,failed);

  y := hyperg_2F0x(1.5, -2.5, 0.01953125);
