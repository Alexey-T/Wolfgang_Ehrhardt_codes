{Part 6a of regression test for SPECFUND unit  (c) 2011-2017  W.Ehrhardt}
...
begin
  ...
  testrel(17, NE+1, y, f, cnt,failed);  {+1 for ARM}
...
begin
  ...
  testrel(15, NE+3, y, f, cnt,failed);   {+3 for ARM}
...
begin
  ...
  testrel(25, NE2+1, y, f, cnt,failed);  {+1 for ARM}
...
begin
  ...
  testrele(11, EPS2, kerp, fker, cnt,failed);
  ...
  testrele(15, EPS3, kerp, fker, cnt,failed);

  fkei := 0.7381496295976999158e-4;
  testrele(16, EPS3, keip, fkei, cnt,failed);
  ...
  testrel(23, NE1, kerp, fker, cnt,failed);
  ...
  testrel(25, NE2, berp, fber, cnt,failed);
  ...
  kerp := kelvin_kerp(x);
  testrel(35, NE2, kerp, fker, cnt,failed);   {64-bit}
  ...
  berp := kelvin_berp(x);
  testrel(37, NE2, berp, fber, cnt,failed);   {64-bit}
  ...
  kerp := kelvin_kerp(x);
  testrele(43, EPS2, kerp, fker, cnt,failed);

  fkei := 0.2056476542120472207e-4;
  keip := kelvin_keip(x);
  testrele(44, EPS2, keip, fkei, cnt,failed);
...
begin
  ...
  y := struve_h1(x);
  {$ifdef FPC}
    testrel(8, NE+1, y, f, cnt,failed);   {64-bit -O4 fastmath}
  {$else}
    testrel(8, NE, y, f, cnt,failed);
  {$endif}
...
{$endif}
  ...
  y := struve_l(-30, 20.77734375);     {!! near zero, use abs errors}
