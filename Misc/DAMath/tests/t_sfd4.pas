...
{$ifdef CPUARM}
  ...
  testrel( 9, 2, y, f, cnt,failed);

  x := 26.5;
  y := erfi(x);
  f := 2.0501652832248793153e303;
  testrel(10, 3, y, f, cnt,failed);

  x := 26.6417236328125;
  y := erfi(x);
  f := 0.380479407656224048888685423243e307;
  testrel(11, 2, y, f, cnt,failed);
{$else}
  x := -10;
  y := erfi(x);
  f := -0.1524307422708669699e43;
...
begin
  ...
  testrel(10, NE+1, y, f, cnt,failed);   {+1 for CPUARM}
  ...
  y := erfce_inv(MinDouble);
  f := 0.2535599352761576228e308;
  testrel(16, NE, y, f, cnt,failed);

  y := erfce_inv(MaxDouble);
  f := -26.62873571375148955;
  testrel(16, NE, y, f, cnt,failed);

  y := erfce_inv(MaxDouble/53);
  f := -26.55408205529371956;
  testrel(16, NE, y, f, cnt,failed);

  y := erfce_inv(MaxDouble/54);
  f := -26.55373008950455867;
  testrel(16, NE, y, f, cnt,failed);
...
{$ifdef CPUARM}
  ...
  testrel(21, 2, y, f, cnt,failed);

  y := erf_z(20.0);
  f := 0.5520948362159763190e-87;
  testrel(22, 3, y, f, cnt,failed);
...
begin
  ...
  testrel(13, NE1, y, f, cnt,failed);  {ARM}
