{Part 3b of regression test for SPECFUND unit  (c) 2011+  W.Ehrhardt}
...
procedure test_laplace_cdf;
procedure test_laplace_inv;
procedure test_laplace_pdf;

procedure test_logistic_cdf;
procedure test_logistic_inv;
procedure test_logistic_pdf;

procedure test_lognormal_cdf;
procedure test_lognormal_inv;
procedure test_lognormal_pdf;

procedure test_moyal_cdf;
procedure test_moyal_inv;
procedure test_moyal_pdf;

procedure test_pareto_cdf;
procedure test_pareto_inv;
procedure test_pareto_pdf;

procedure test_triangular_pdf;
procedure test_triangular_inv;
procedure test_triangular_cdf;

procedure test_uniform_cdf;
procedure test_uniform_inv;
procedure test_uniform_pdf;

procedure test_weibull_cdf;
procedure test_weibull_inv;
procedure test_weibull_pdf;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','laplace_cdf');

  {statevalf[cdf,laplace[0,2]](x);}
  a := 0.0;
  b := 2.0;
  y := laplace_cdf(a, b, -100);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','laplace_inv');
  a := 0.0;
  b := 2.0;
  y := laplace_inv(a, b, ldexpd(1,-20));
  f := -26.33959286127792176;
  testrel(1, NE, y, f, cnt,failed);

  y := laplace_inv(a, b, 0.125);
  f := -2.772588722239781238;
  testrel(2, NE, y, f, cnt,failed);

  y := laplace_inv(a, b, 0.5);
  f := 0.0;
  testrel(3, NE, y, f, cnt,failed);

  y := laplace_inv(a, b, 0.75);
  f := 1.386294361119890619;
  testrel(4, NE, y, f, cnt,failed);

  y := laplace_inv(a, b, 0.875);
  f := 2.772588722239781238;
  testrel(5, NE, y, f, cnt,failed);

  y := laplace_inv(a, b, 0.9);
  f := 3.218875824868200749;
  testrel(6, NE, y, f, cnt,failed);

  y := laplace_inv(a, b, 0.99);       {not exact}
  f := 7.824046010856292117;
  testrel(7, 2, y, f, cnt,failed);    {!!!!!!!!}

  y := laplace_inv(a, b, 0.9990234375);
  f := 12.47664925007901557;
  testrel(8, NE, y, f, cnt,failed);

  y := laplace_inv(a, b, 1-ldexpd(1,-20));
  f := 26.33959286127792176;
  testrel(9, NE, y, f, cnt,failed);
...
begin
  ...
  y := laplace_pdf(a, b, ldexpd(1,-20));
  f := 0.2499998807907388709;
  testrel(2, NE, y, f, cnt,failed);

  y := laplace_pdf(a, b, -0.125);
  f := 0.2348532657033689465;
  testrel(3, NE, y, f, cnt,failed);

  y := laplace_pdf(a, b, 0.5);
  ...
  y := laplace_pdf(a, b, -10);
  f := 0.1684486749771366774e-2;
  testrel(7, NE, y, f, cnt,failed);

  y := laplace_pdf(a, b, 100);
  f := 0.4821874619909794458e-22;
  testrel(8, NE, y, f, cnt,failed);

  y := laplace_pdf(a, b, -1000);
  ...
  y := laplace_pdf(a, b, 0.9990234375);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','logistic_cdf');

  {statevalf[cdf,logistic[a,b]](x);}
  a := 0;
  b := 1;
  y := logistic_cdf(a, b, -100);
  f := 0.3720075976020835963e-43;
  testrel(1, NE, y, f, cnt,failed);

  y := logistic_cdf(a, b, -10);
  f := 0.4539786870243439450e-4;
  testrel(2, NE, y, f, cnt,failed);

  y := logistic_cdf(a, b, -1);
  f := 0.2689414213699951207;
  testrel(3, NE, y, f, cnt,failed);

  y := logistic_cdf(a, b, 0);
  f := 0.5;
  testrel(4, NE, y, f, cnt,failed);
  ...
  y := logistic_cdf(a, b, 2);
  f := 0.8807970779778824441;
  testrel(6, NE, y, f, cnt,failed);

  y := logistic_cdf(a, b, 5);
  ...
  y := logistic_cdf(a, b, 20);
  f := 0.9999999979388463818;
  testrel(9, NE, y, f, cnt,failed);

  y := logistic_cdf(a, b, 50);
  f := 1.0;
  testrel(10, NE, y, f, cnt,failed);
  ...
  y := logistic_cdf(a, b, 3);
  f := 0.8807970779778824441;
  testrel(14, NE, y, f, cnt,failed);

  y := logistic_cdf(a, b, 4);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','logistic_inv');

  a := 0.0;
  b := 1.0;
  y := logistic_inv(a, b, ldexpd(1,-20));
  f := -13.86294265752413503;
  testrel(1, NE, y, f, cnt,failed);

  y := logistic_inv(a, b, 0.125);
  f := -1.945910149055313305;
  testrel(2, NE, y, f, cnt,failed);

  y := logistic_inv(a, b, 0.5);
  f := 0;
  testrel(3, NE, y, f, cnt,failed);

  y := logistic_inv(a, b, 0.75);
  f := 1.098612288668109691;
  testrel(4, NE, y, f, cnt,failed);

  y := logistic_inv(a, b, 0.875);
  f := 1.945910149055313305;
  testrel(5, NE, y, f, cnt,failed);

  y := logistic_inv(a, b, 0.9);
  f := 2.197224577336219383;
  testrel(6, NE, y, f, cnt,failed);

  y := logistic_inv(a, b, 0.99);
  f := 4.595119850134589927;
  testrel(7, NE, y, f, cnt,failed);

  y := logistic_inv(a, b, 0.9990234375);
  f := 6.930494765951626481;
  testrel(8, NE, y, f, cnt,failed);

  y := logistic_inv(a, b, 1-ldexpd(1,-20));
  f := 13.86294265752413503;
  testrel(9, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','logistic_pdf');

  a := 0.0;
  b := 1.0;
  y := logistic_pdf(a, b, 0);
  f := 0.25;
  testrel(1, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, 0.5);
  f := 0.2350037122015944891;
  testrel(2, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, -2);
  f := 0.1049935854035065174;
  testrel(3, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, 5);
  f := 0.6648056670790154914e-2;
  testrel(4, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, -10);
  f := 0.4539580773595167103e-4;
  testrel(5, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, 50);
  f := 0.1928749847963917783e-21;
  testrel(6, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, -100);
  f := 0.3720075976020835963e-43;
  testrel(7, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, 500);
  f := 0.7124576406741285532e-217;
  testrel(8, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, 10000);
  ...
  y := logistic_pdf(a, b, 300);
  f := 0.2894134972965153792e-258;
  testrel(18, NE, y, f, cnt,failed);

  y := logistic_pdf(a, b, 5000);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','lognormal_cdf');
  {statevalf[cdf,lognormal[a,b]](x);}

  a := 2;
  b := 1.5;
  y := lognormal_cdf(a, b, 0.0009765625);
  f := 0.1305820500266527804e-8;
  testrel(1, 2*NE, y, f, cnt,failed);

  y := lognormal_cdf(a, b, 0.125);
  f := 0.3267772762571824551e-2;
  testrel(2, NE, y, f, cnt,failed);

  y := lognormal_cdf(a, b, 1);
  f := 0.9121121972586786984e-1;
  testrel(3, NE, y, f, cnt,failed);

  y := lognormal_cdf(a, b, 5);
  f := 0.3972873686243666920;
  testrel(4, NE, y, f, cnt,failed);

  y := lognormal_cdf(a, b, 10);
  f := 0.5799335141280535526;
  testrel(4, NE, y, f, cnt,failed);

  y := lognormal_cdf(a, b, 50);
  f := 0.8987890904238633564;
  testrel(6, NE, y, f, cnt,failed);

  y := lognormal_cdf(a, b, 100);
  f := 0.9587870056652280963;
  testrel(7, NE, y, f, cnt,failed);

  y := lognormal_cdf(a, b, 1000);
  f := 0.9994657439207799539;
  testrel(8, NE, y, f, cnt,failed);

  y := lognormal_cdf(a, b, 10000);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','lognormal_inv');

  a := 2;
  b := 1.5;

  y := lognormal_inv(a, b, ldexpd(1,-20));
  f := 0.5831380283049865408e-2;
  testrel(1, NE, y, f, cnt,failed);

  y := lognormal_inv(a, b, 0.125);
  f := 1.315840900655271756;
  testrel(2, NE, y, f, cnt,failed);

  y := lognormal_inv(a, b, 0.5);
  f := 7.389056098930650227;
  testrel(3, NE, y, f, cnt,failed);

  y := lognormal_inv(a, b, 0.75);
  f := 20.32262150159026255;
  testrel(4, NE, y, f, cnt,failed);

  y := lognormal_inv(a, b, 0.875);
  f := 41.49297229319674307;
  testrel(5, NE, y, f, cnt,failed);

  y := lognormal_inv(a, b, 0.9);
  f := 50.51788077261473522;
  testrel(6, NE, y, f, cnt,failed);

  y := lognormal_inv(a, b, 0.99);       {!!! not exact}
  f := 242.141389834721616;
  testrel(7, 2*NE, y, f, cnt,failed);     {!!!}

  y := lognormal_inv(a, b, 0.9990234375);
  f := 769.6251801436839498;
  testrel(8, NE+1, y, f, cnt,failed);    {+1 for ARM}

  y := lognormal_inv(a, b, 1-ldexpd(1,-20));
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','lognormal_pdf');

  a := 2.0;
  b := 1.5;
  y := lognormal_pdf(a, b, 0.0009765625);
  f := 0.5450176094613075364e-5;
  testrel(1, NE2, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 0.125);
  f := 0.5269949016070822002e-1;
  testrel(2, NE, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 1);
  f := 0.1093400497839957465;
  testrel(3, NE, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 2);
  f := 0.9098358075364976163e-1;
  testrel(4, NE, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 5);
  f := 0.5141943565196794751e-1;
  testrel(5, NE, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 10);
  f := 0.2606049016368246096e-1;
  testrel(6, NE, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 100);
  f := 0.5885925220711086559e-3;
  testrel(7, NE, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 1000);
  f := 0.1259724999118361604e-5;
  testrel(8, NE, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 1e9);
  ...
  y := lognormal_pdf(a, b, 0.25);
  f := 0.1507955442895558942e-1;
  testrel(12, NE, y, f, cnt,failed);

  y := lognormal_pdf(a, b, 0.5);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','pareto_pdf');

  {pareto_pdf := (k,a,x) -> a*k^a/x^(a+1);}
  k := 0.8;
  a := 0.5;
  y := pareto_pdf(k, a, 0.8);
  f := 0.625;
  testrel(1, NE, y, f, cnt,failed);
  ...
  y := pareto_pdf(k, a, 100);
  f := 0.4472135954999579393e-3;
  testrel(6, NE, y, f, cnt,failed);

  y := pareto_pdf(k, a, 10000);
  f := 0.4472135954999579393e-6;
  testrel(7, NE, y, f, cnt,failed);

  y := pareto_pdf(k, a, 1e9);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','pareto_inv');

  {pareto_inv := (k,a,x) -> 1-(k/x)^a}
  k := 0.8;
  a := 0.5;
  y := pareto_inv(k, a, 0);
  f := 0.8;
  ...
  y := pareto_inv(k, a, ldexpd(1,-20));
  f := 0.80000152588108904006;
  testrel(2, NE, y, f, cnt,failed);

  y := pareto_inv(k, a, 0.125);
  f := 1.044897959183673469;
  {WA: 1.0448979591836734693877551020408163265306}
  testrel(3, NE, y, f, cnt,failed);

  y := pareto_inv(k, a, 0.5);
  f := 3.2;
  testrel(4, NE, y, f, cnt,failed);

  y := pareto_inv(k, a, 0.75);
  f := 12.8;
  testrel(5, NE, y, f, cnt,failed);

  y := pareto_inv(k, a, 0.875);
  f := 51.2;
  testrel(6, NE, y, f, cnt,failed);

  y := pareto_inv(k, a, 0.9);             {not exact}
  f := 80.0;
  testrel(7, NE2, y, f, cnt,failed);

  y := pareto_inv(k, a, 0.99);            {not exact}
  f := 8000.0;
  testrel(8, NE2, y, f, cnt,failed);      {!!!!!!!!!!!}

  y := pareto_inv(k, a, 0.9990234375);
  f := 838860.8;
  testrel(9, NE, y, f, cnt,failed);

  y := pareto_inv(k, a, 1-ldexpd(1,-20));
  f := 879609302220.8;
  testrel(10, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','pareto_cdf');

  {pareto_cdf := (k,a,x) -> 1-(k/x)^a}
  k := 0.8;
  a := 0.5;
  y := pareto_cdf(k, a, 0.80078125);
  f := 0.4879239129211806457e-3;
  testrel(1, 512, y, f, cnt,failed);  {!!!!!!!!!!!!!!!??????????????????}
  {The reason for large relative error: k is not extact. If we calculate}
  {f1=pareto_cdf(k,a, 0.80078125) and pareto_cdf(k+0.4e-17,a,0.80078125)}
  {with Maple using 30 digits we get f1=0.000487923912921180645741631279}
  {f2=0.487923912921178146961441061e-3 and (1-f2/f1)/1e-17 = 512.12497 !}
  ...
  y := pareto_cdf(k, a, 100);
  f := 0.9105572809000084121;
  testrel(7, NE, y, f, cnt,failed);

  y := pareto_cdf(k, a, 1000);
  f := 0.9717157287525380990;
  testrel(8, NE, y, f, cnt,failed);

  y := pareto_cdf(k, a, 50000);
  f := 0.996;
  testrel(9, NE, y, f, cnt,failed);

  y := pareto_cdf(k, a, 1e12);
  f := 0.9999991055728090001;
  testrel(10, NE, y, f, cnt,failed);

  k := 2;
  a := 2.5;
  y := pareto_cdf(k, a, 2.1);
  f := 0.1148298658063191116;
  testrel(11, 2*NE, y, f, cnt,failed);       {!!!!!}

  y := pareto_cdf(k, a, 2.25);
  f := 0.2550644609721968467;
  testrel(12, NE, y, f, cnt,failed);

  y := pareto_cdf(k, a, 2.5);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','triangular_cdf');

  a := 0.0;
  b := 1.0;
  c := 0.25;

  y := triangular_cdf(a,b,c,-1);
  f := 0;
  testrel(1, NE, y, f, cnt,failed);

  y := triangular_cdf(a,b,c,0);
  f := 0;
  testrel(2, NE, y, f, cnt,failed);

  y := triangular_cdf(a,b,c,0.1);
  f := 0.04;
  testrel(3, NE, y, f, cnt,failed);

  y := triangular_cdf(a,b,c,0.25);
  f := 0.25;
  testrel(4, NE, y, f, cnt,failed);

  y := triangular_cdf(a,b,c,0.5);
  f := 2/3;
  testrel(5, NE, y, f, cnt,failed);

  y := triangular_cdf(a,b,c,0.8);
  f := 71/75;
  testrel(6, NE, y, f, cnt,failed);
  ...
  y := triangular_cdf(a,b,c,0.75);
  f := 0.5625;
  testrel(11, NE, y, f, cnt,failed);

  y := triangular_cdf(a,b,c,0.9);
  f := 0.81;
  testrel(12, NE, y, f, cnt,failed);

  y := triangular_cdf(a,b,c,1.25);
  f := 1;
  testrel(13, NE, y, f, cnt,failed);
  ...
  y := triangular_cdf(a,b,c,-2);
  f := 0;
  testrel(14, NE, y, f, cnt,failed);

  y := triangular_cdf(a,b,c,-1.5);
  f := 0.0625;
  testrel(15, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','triangular_pdf');

  a := 0.0;
  b := 1.0;
  c := 0.25;

  y := triangular_pdf(a,b,c,-1);
  f := 0;
  testrel(1, NE, y, f, cnt,failed);

  y := triangular_pdf(a,b,c,0);
  f := 0;
  testrel(2, NE, y, f, cnt,failed);

  y := triangular_pdf(a,b,c,0.1);
  f := 0.8;
  testrel(3, NE, y, f, cnt,failed);

  y := triangular_pdf(a,b,c,0.25);
  f := 2.0;
  testrel(4, NE, y, f, cnt,failed);

  y := triangular_pdf(a,b,c,0.5);
  f := 4/3;
  testrel(5, NE, y, f, cnt,failed);

  y := triangular_pdf(a,b,c,0.8);
  f := 8/15;
  testrel(6, NE, y, f, cnt,failed);
  ...
  y := triangular_pdf(a,b,c,0.75);
  f := 1.5;
  testrel(11, NE, y, f, cnt,failed);

  y := triangular_pdf(a,b,c,0.9);
  f := 1.8;
  testrel(12, NE, y, f, cnt,failed);

  y := triangular_pdf(a,b,c,1.25);
  f := 0;
  testrel(13, NE, y, f, cnt,failed);
  ...
  y := triangular_pdf(a,b,c,-2);
  f := 0;
  testrel(14, NE, y, f, cnt,failed);

  y := triangular_pdf(a,b,c,-1.5);
  f := 0.25;
  testrel(15, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','triangular_inv');

  a := 0.0;
  b := 1.0;
  c := 0.25;

  y := triangular_inv(a,b,c,0.01);
  f := 0.05;
  testrel(1, NE, y, f, cnt,failed);

  y := triangular_inv(a,b,c,0.1);
  f := 0.1581138830084189666;
  testrel(2, NE, y, f, cnt,failed);

  y := triangular_inv(a,b,c,1/3);
  f := 0.2928932188134524756;
  testrel(3, NE, y, f, cnt,failed);

  y := triangular_inv(a,b,c,0.5);
  f := 0.3876275643042054755;
  testrel(4, NE, y, f, cnt,failed);

  y := triangular_inv(a,b,c,2/3);
  f := 0.5;
  testrel(5, NE, y, f, cnt,failed);

  y := triangular_inv(a,b,c,0.9);
  f := 0.7261387212474169433;
  testrel(6, NE, y, f, cnt,failed);

  y := triangular_inv(a,b,c,0.99);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','uniform_inv');

  a := -1.0;
  b := 3.0;
  y := uniform_inv(a, b, ldexpd(1,-20));
  f := -0.999996185302734375;
  testrel(1, NE, y, f, cnt,failed);

  y := uniform_inv(a, b, 0.125);
  f := -0.5;
  testrel(2, NE, y, f, cnt,failed);

  y := uniform_inv(a, b, 0.5);
  f := 1.0;
  testrel(3, NE, y, f, cnt,failed);

  y := uniform_inv(a, b, 0.75);
  f := 2.0;
  testrel(4, NE, y, f, cnt,failed);

  y := uniform_inv(a, b, 0.875);
  f := 2.5;
  testrel(5, NE, y, f, cnt,failed);

  y := uniform_inv(a, b, 0.9);
  f := 2.6;
  testrel(6, NE, y, f, cnt,failed);

  y := uniform_inv(a, b, 0.99);
  f := 2.96;
  testrel(7, NE, y, f, cnt,failed);

  y := uniform_inv(a, b, 0.9990234375);
  f := 2.99609375;
  testrel(8, NE, y, f, cnt,failed);

  y := uniform_inv(a, b, 1-ldexpd(1,-20));
...
begin
  ...
  y := uniform_cdf(a, b, 0.0009765625);
  f := 0.250244140625;
  testrel(4, NE, y, f, cnt,failed);
  ...
  y := uniform_cdf(a, b, 2.5);
  f := 0.875;
  testrel(6, NE, y, f, cnt,failed);

  y := uniform_cdf(a, b, 2.99);
  f := 0.9975;
  testrel(7, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','weibull_cdf');

  {statevalf[cdf,weibull[0.5,1]](x);}
  a := 0.5;
  b := 1.0;
  y := weibull_cdf(a, b, ldexpd(1,-20));
  f := 0.9760858180243377653e-3;
  testrel(1, NE, y, f, cnt,failed);

  y := weibull_cdf(a, b, 0.015625);
  f := 0.1175030974154045971;
  testrel(2, NE, y, f, cnt,failed);

  y := weibull_cdf(a, b, 0.125);
  f := 0.2978114986734404038;
  testrel(3, NE, y, f, cnt,failed);

  y := weibull_cdf(a, b, 0.5);
  ...
  y := weibull_cdf(a, b, 10);
  f := 0.9576707803767950023;
  testrel(7, NE, y, f, cnt,failed);

  y := weibull_cdf(a, b, 100);
  f := 0.9999546000702375151;
  testrel(8, NE, y, f, cnt,failed);

  y := weibull_cdf(a, b, 1000);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','weibull_inv');

  a := 0.5;
  b := 1.0;
  y := weibull_inv(a, b, ldexpd(1,-20));
  f := 0.9094955691354244759e-12;
  testrel(1, NE, y, f, cnt,failed);

  y := weibull_inv(a, b, 0.125);
  f := 0.1783063281624441480e-1;
  testrel(2, NE, y, f, cnt,failed);

  y := weibull_inv(a, b, 0.5);
  f := 0.48045301391820142467;
  testrel(3, NE, y, f, cnt,failed);

  y := weibull_inv(a, b, 0.75);
  f := 1.921812055672805699;
  testrel(4, NE, y, f, cnt,failed);

  y := weibull_inv(a, b, 0.875);
  f := 4.324077125263812822;
  testrel(5, NE, y, f, cnt,failed);

  y := weibull_inv(a, b, 0.9);
  f := 5.301898110478398011;
  testrel(6, NE, y, f, cnt,failed);

  y := weibull_inv(a, b, 0.99);      {!!! not exact}
  f := 21.20759244191359204;
  testrel(7, NE, y, f, cnt,failed);

  y := weibull_inv(a, b, 0.9990234375);
  f := 48.04530139182014247;
  testrel(8, NE, y, f, cnt,failed);

  y := weibull_inv(a, b, 1-ldexpd(1,-20));
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','weibull_pdf');

  a := 0.5;
  b := 1.0;
  y := weibull_pdf(a, b, ldexpd(1,-20));
  f := 511.50024406117153906;
  testrel(1, NE, y, f, cnt,failed);

  y := weibull_pdf(a, b, 0.125);
  f := 0.9930445019184586250;
  testrel(2, NE, y, f, cnt,failed);

  y := weibull_pdf(a, b, 0.5);
  ...
  y := weibull_pdf(a, b, 10);
  f := 0.6692837279341107373e-2;
  testrel(6, NE, y, f, cnt,failed);

  y := weibull_pdf(a, b, 100);
  f := 0.2269996488124242577e-5;
  testrel(7, NE, y, f, cnt,failed);

  y := weibull_pdf(a, b, 10000);
...
begin
  ...
  y := moyal_inv(0, 2, 0.75);
  f := 4.574781344821882738;
  testrel(4, NE, y, f, cnt,failed);

  y := moyal_inv(0, 2, 0.875);
  f := 7.398130184703767302; {alpha}
  testrel(5, NE, y, f, cnt,failed);

  y := moyal_inv(0, 2, 0.95);
  f := 11.07714294407235304;
  testrel(6, NE, y, f, cnt,failed);

  y := moyal_inv(0, 2, 0.99);
  f := 17.51741060923158137;
  testrel(7, NE, y, f, cnt,failed);

  y := moyal_inv(1, 1, 0.125);
  f := 0.1440854368523699718;
  testrel(8, NE2, y, f, cnt,failed);

  y := moyal_inv(1, 1, 0.25);
  ...
  y := moyal_inv(1, 1, 0.75);
  f := 3.287390672410941369;
  testrel(11, NE, y, f, cnt,failed);

  y := moyal_inv(1, 1, 0.875);
  f := 4.699065092351883651;
  testrel(12, NE, y, f, cnt,failed);

  y := moyal_inv(1, 1, 0.95);
  f := 6.538571472036176521;
  testrel(13, NE, y, f, cnt,failed);

  y := moyal_inv(1, 1, 0.99);
