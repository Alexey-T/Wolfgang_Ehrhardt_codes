{Main test unit for quadsolve/cubsolve/polyroots,  (c) W.Ehrhardt 2013-2018}
unit t_drootm;
...
begin
  ...
  if (i <> 2) or (x1<>1.0) or (abs(x2-1.0000000210734241) > 2*2E-16) then begin  {2* for ARM}
    squad_selftest := 8;
    exit;
  end;
...
var
  ...
  function fail(x,y: double): boolean;
  begin
    if IsInfd(y) then fail := x<>y
    else fail := abs(x-y) > 4*eps_d * abs(y); {4* for ARM}
  end;

begin
  n := fsolve(a,b,c,x1,y1,x2,y2);
  inc(totalcnt);
  if (n<>en) or fail(x1,ex1) or fail(x2,ex2) or fail(y1,ey1) or fail(y2,ey2) then begin
...
begin
  ...
  cubsolve(a,b,c,d,x,x1,y1,x2,y2);
  if ccheck(x,x1,y1,x2,y2,1e-17,15,-y,15,y,4*tol1) then begin {4* for ARM}
    cubsolve_selftest := 3;
    exit;
  end;
...
begin
  writeln;
  writeln('Test program for DAMTools/PolyRoots(A)   (c) W.Ehrhardt 2017');
  failed := 0;
