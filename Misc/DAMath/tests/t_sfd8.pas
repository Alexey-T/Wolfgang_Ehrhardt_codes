...
begin
  ...
  testrel( 3, NE1, y, f, cnt,failed);   {NE1 for ARM}
  ...
  x := 35.9990234375;
  y := zeta(x);
  f := 1.000000000014561775;
  testrel(18, NE, y, f, cnt,failed);

  x := 36.0009765625;
  y := zeta(x);
  f := 1.000000000014542075;
  testrel(19, NE, y, f, cnt,failed);

  x := 51.0;
  y := zeta(x);
  f := 1.000000000000000444;
  ...
  y := zeta(-3e-9);
  f := -0.4999999972431844094;
  testrel(28, NE, y, f, cnt,failed);

  y := zeta(3e-9);
  f := -0.5000000027568156086;
...
begin
  ...
  testrel( 2, NE+1, y, f, cnt,failed);  {+1 for ARM}
...
begin
  ...
  for i:=-trunc(MAXGAM)+2 to 100 do if i<>1 then begin
    if i<0 then x := abs(i*eps_d*10)
    else x := 10*eps_d;
    y := zetaint(i);
    if (i<0) and (i and 1 = 0) then f := 0.0
    else f := zeta(succd(i));
    if IsInfD(y) or IsInfD(f) then begin
      if f<>y then begin
...
begin
  ...
  x := 1.5;
  y := etam1(x);
  f := -0.23485297537459205;
  testrel( 5, 3, y, f, cnt,failed);    {!!!!}
...
const
  ...
  NE1 = 4;
  NE2 = 10;
  NE3 = 40; {case ZetaH(-s,..) see test_zetah, 40 for SSE/64 bit, 30 for 387/32 bit}
begin
  cnt := 0;
  failed := 0;
  ...
  f := 32e-200;
  testrel(32, NE1, y, f, cnt,failed);  {NE1 for ARM}
  ...
  testrel(47, NE+1, y, f, cnt,failed);   {+1 for ARM}
  ...
  f := 1e160;
  testrel(49, NE1+1, y, f, cnt,failed);  {+1 for ARM}
  ...
  testrel(51, NE1+1, y, f, cnt,failed);  {+1 for ARM}
  ...
  testrel(54, NE+1, y, f, cnt,failed); {+1 for ARM}
  ...
  testrel(68, NE+1, y, f, cnt,failed);  {+1 for ARM}
  ...
  testrel(95, NE1, y, f, cnt,failed);     {CPUARM}

  y := LerchPhi(-2,1,10000);
  f := 0.3333555562962222098e-4;
  testrel(96, NE1, y, f, cnt,failed);     {CPUARM}
  ...
  testrel(106, NE2, y, f, cnt,failed);
...
{$endif}
  ...
  testrel( 9, NE+1, y, f, cnt,failed);  {+1 for ARM}
  ...
  testrel(41, NE+1, y, f, cnt,failed);  {+1 for ARM}
  ...
  testrel(85, NE3, y, f, cnt,failed);

  y := polylog(-16, -12);
  f := 2208.471352806776057;
  testrel(86, NE3, y, f, cnt,failed);
  ...
  testrel(102, NE2, y, f, cnt,failed);
  ...
  testrel(106, 1200, y, f, cnt,failed);  {!!! Two Recursion levels, inexact square root, cancellation}
...
begin
  ...
  f := -1.460354508809586813;
  testrel(28, NE1, y, f, cnt,failed);    {change: polylog(s,1)=zeta(s)}
