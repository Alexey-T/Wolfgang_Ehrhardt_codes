{Part 9a of regression test for SPECFUND unit  (c) 2013-2015  W.Ehrhardt}
...
begin
  ...
  testrele(22,1e-12, y, f, cnt,failed);  {!!!!!!!!!!!!}
...
{$ifdef FPC}
  testrel(69, 4600, y, f, cnt,failed);    {!!!!!!!!!!!!!!!!!!!!!}
{$else}
  testrel(69, NE3, y, f, cnt,failed);
{$endif}

(*
  {Overflow in 'Evaluate the finite sum'}
  y := hyperg_u(1, -1000, 1010);
  f := 0.0004971408839859245170;
  testrel(70, NE, y, f, cnt,failed);
*)

  y := hyperg_u(8, 1, 0.001);
  f := 0.0007505359326875706975;
  testrel(71, NE1, y, f, cnt,failed);        {FPC/SSE2}

  y := hyperg_u(8, 1, 0.5);
  f := 6.449509938973479986e-06;
  testrel(72, NE1, y, f, cnt,failed);        {FPC/SSE2}
...
{$ifdef FPC}
  testrel(116, NE1, y, f, cnt,failed);
{$else}
  testrel(116, NE1, y, f, cnt,failed);
{$endif}
...
{$endif}
  ...
  y := hyperg_u(-2.0, 0.5, 0.0);      {WE: via gamma_delta_ration}
  f := 0.75;
  ...
  testrel(141, NE1, y, f, cnt,failed);
  ...
  testrel(145, NE1, y, f, cnt,failed);
  ...
  testrel(150, NE1, y, f, cnt,failed);
  ...
  testrel(156, NE1, y, f, cnt,failed);
...
*)
  ...
  testrel(172, NE1, y, f, cnt,failed);
  ...
  testrele(173, 3e-12 , y, f, cnt,failed);    {!!!!!!!!!!!!!!!!}
...
begin
  ...
  {--------------------------------------------}
(*
  {The next two AMath test case cannot be used with DAMath:}
  {Intermediate values for sfd_1f1 will produce overflows!}
...
*)
  ...
  y := WhittakerM(1,2,500);
  f := 0.2022044105877972979e108;
  testrel(33, NE2, y, f, cnt,failed);

  y := WhittakerM(2,-0.25,500);
  f := 0.9736001484358642973e103;
...
begin
  ...
  testrel(7, NE2, y, f, cnt,failed);   {32 Bit}
  ...
  testrel(9, NE1, y, f, cnt,failed);   {32 Bit}
  ...
  testrel(31, NE+1, y, f, cnt,failed);    {+1 for ARM}
...
begin
  ...
  testrel(15, NE+1, y, f, cnt,failed);   {+1 for ARM}
...
begin
  ...
  y := CylinderV(1,0);
  f := 0.3280019486668764664;
  testrel(35, NE, y, f, cnt,failed);

  y := CylinderV(1,1e-8);
  f := 0.3280019520979396206;
  testrel(36, NE, y, f, cnt,failed);
  ...
  y := CylinderV(0,1e-4);
  f := 0.6862454277541928463;
  testrel(41, NE, y, f, cnt,failed);

  y := CylinderV(0,1e-8);
  f := 0.6862126308393456439;
...
{$ifdef FPC}
  testrel(49, NE1, y, f, cnt,failed);   {NE1 for FPC/SSE2}
{$else}
  testrel(49, NE, y, f, cnt,failed);
{$endif}
  ...
  testrel(57, NE1, y, f, cnt,failed);         {NE1 for ARM}

  y := CylinderV(200, -2.0);
  f := 3.590716405717152808e173;   {alpha}
  testrel(58, NE4, y, f, cnt,failed);   {ln(Gamma)}
