{Part 5 of regression test for SPECFUND unit  (c) 2010-2013  W.Ehrhardt}
...
{$endif}
  ...
  y := chebyshev_t(10,x);
  f := 0.0;
  testabs(13, NE2, y, f, cnt,failed);

  x := cosPi(-15/42);
  y := chebyshev_t(21,x);
  f := 0.0;
  testabs(14, NE3, y, f, cnt,failed);    {!!!!}

  x := cosPi(31/100);
  y := chebyshev_t(50,x);
  f := 0.0;
  testabs(15, NE2, y, f, cnt,failed);
...
{$ifdef CPUARM}
  testrel( 3, 230, y, f, cnt,failed);   {!!!!!!??}
{$else}
  testrel( 3, 90, y, f, cnt,failed);   {!!!!!!??}
{$endif}
  ...
  testrel( 8,  40, y, f, cnt,failed);    {!!! 64-bit}

  x := 1 - ldexpd(1,-48);
  y := chebyshev_u(i,x);
  f := 64.9999999996748556;
  testrel( 9, 150, y, f, cnt,failed);    {!!! 64-bit}

  x := 1.25;
  y := chebyshev_u(70,x);
  f := 0.1574122160956548406e22;
  testrel(10, NE2+1, y, f, cnt,failed);  {+1 for ARM}
...
begin
  ...
  testrel(23, NE2, y, f, cnt,failed);         {Delphi64!!, FPC64:NE}
...
begin
  ...
  testrel(18, NE+2, y, f, cnt,failed);    {+2 for ARM}
...
begin
  ...
  testrel(19, 16, y, f, cnt,failed);    {!!! 64-bit}
...
begin
  ...
  testrel(41, 220, y, f, cnt,failed);   {!!! 64-bit}

  y := laguerre(99,1.5,1);
  f := -0.323309770429035454e1;
  testrel(42, 140, y, f, cnt,failed);   {!!! 64-bit}
  ...
  testrel(57, NE+1, y, f, cnt,failed);      {+1 for ARM}
  ...
  testrel(60, NE+1, y, f, cnt,failed);      {+1 for ARM}
...
begin
  ...
  for i:=1 to 100 do begin
    ...
    while k<10 do begin
      ...
      f := chebyshev_v(i,x);
      t := abs(y-f)/eps_d;
      if t > 128 then begin  {32 if not 64 bit}
        writeln('   i/x/t = ',i:6, x:10:4, t:30:2);
        inc(failed);
      end;
      inc(k);
...
begin
  ...
  testrel(20, NE+2, y, f, cnt,failed);  {+2 for ARM}
