...
(*************************************************************************
 ...
 1.24.00  30.09.16  we          sfd_chebf1
 1.24.01  07.10.16  we          sfd_hermite_he

 1.29.00  14.02.18  we          fixed parameter/return types in sfd_hermite_he
 ***************************************************************************)
...
function sfd_chebf1(v,x: double): double;
...
function sfd_hermite_he(n: integer; x: double): double;
...
begin
  ...
  w := abs(v);
  if w=0.0 then sfd_chebf1 := 1.0
  else if w=1.0 then sfd_chebf1 := x
  else begin
    if x > 1.0 then sfd_chebf1 := cosh(w*arccosh(x))
    else if x =  1.0 then sfd_chebf1 := 1.0
    else if x =  0.0 then sfd_chebf1 := cospi(0.5*w)
    else if x = -1.0 then sfd_chebf1 := cospi(w)
    else if x > -1.0 then sfd_chebf1 := cos(w*arccos(x))
    else begin
      {On the branch cut x < -1 the real part of the function}
      {value is returned: cosh(v*[arccosh(|x|)+Pi*i])}
      z := cospi(w);
      if z=0.0 then sfd_chebf1 := z
      else begin
...
begin
  ...
  if n<=1 then begin
    if n=1 then sfd_hermite_he := h1
    else if n=0 then sfd_hermite_he := h0
    else sfd_hermite_he := 0.0;
    exit;
  end;
