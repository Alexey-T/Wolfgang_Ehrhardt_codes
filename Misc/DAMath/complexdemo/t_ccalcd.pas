...
2018-11-20: Fix format now 1:15
*)

uses
  BTypes, DAMath, DFPU, DAMCmplx, DAMCcalc;
...
begin
  ...
  writeln('            ln, ln1p, lngamma, log10, psi, re, rstheta, sec, sech, sin,');
  writeln('            sinh, sn, sqr, sqrt, tan, tanh, wk, zeta');
  writeln;
...
begin
  if use_sci then begin
    ...
    writeln(abs(evr.Res.im):24, ' *I');
  end
  else begin
    write(evr.Res.re:1:15);
    if evr.res.im < 0 then write(' - ')
    else write('  + ');
    writeln(abs(evr.Res.im):1:15, ' * i');
  end;
end;
...
begin
  ...
  writeln('Test of DAMCmplx V', DAMCmplx_Version, ' / DAMCcalc   [t_ccalcd]   (c) W.Ehrhardt 2018');
  ...
  repeat
    ...
    if (cmd='HEX') then begin
      writeln('Hex result = ', Dbl2Hex(evr.Res.re), ' + ', Dbl2Hex(evr.Res.im) + ' * i');
      continue;
    end;
