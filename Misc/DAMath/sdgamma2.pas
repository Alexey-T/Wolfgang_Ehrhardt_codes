...
(*************************************************************************
 ...
 1.24.00  23.08.16  we          sfd_invgam (inverse of gamma)

 1.35.00  23.08.18  we          NanOrInf check in sfd_incgamma_ex, sfd_ibeta, sfd_nnbeta
 1.35.01  28.08.18  we          sfd_igammap_der
 1.35.02  07.09.18  we          sfd_igamman
 1.35.03  07.09.18  we          improved sfd_igamma

 1.36.00  29.09.18  we          sfd_igamma with Laguerre expansion
 1.36.01  02.10.18  we          removed sfd_igamman, sfd_igamma for x<0, a<>-1,-2,...,
 1.36.02  02.10.18  we          sfd_igammal for x < 0
...
function sfd_igammap_der(a,x: double): double;
...
function sfd_invgam(y: double): double;
...
begin
  dax := 0.0;
  if IsNanOrInfD(x) or IsNanOrInfD(a) or (x<0.0) or (a<0.0)  then begin
    {$ifopt R+}
...
{---------------------------------------------------------------------------}
function igamlsum(a, x: double): double;
...
begin
  ...
  t := abs(s);
  if (t<=1.0) or (t < MaxDouble/f) then igamlsum := f*s
  else igamlsum := copysignd(PosInf_d,t);
end;
...
begin
  ...
  else if a=0.0 then sfd_igamma := sfd_e1(x)
  else if x<0.0 then begin
...
var
  r,t,p: double;
begin
  if IsNanOrInfD(x) or IsNanOrInfD(a) or ((a<=0.0) and (frac(a)=0.0)) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfd_igammal := NaN_d;
  ...
  if x=0.0 then sfd_igammal := 0.0
    ...
    if frac(a)=0.5 then sfd_igammal := 0.0
    else begin
      {NIST, 8.5.1}
      t := sfd_1f1(a, a+1, -x);
      {Real part of x^a}
...
begin
  ...
  s2  := s*s;
  rta := sqrt(a);
  t   := a + s*rta + (s2 - 1.0)/THREE + s*(s2 - 7.0)/(36.0*rta)
...
function sfd_igammap_der(a,x: double): double;
  ...
  { lower incomplete gamma function P(a,x), x >= 0, a <> 0,-1,-2 ...}
begin
  if IsNanOrInfD(x) or IsNanOrInfD(a) or (x<0.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfd_igammap_der := NaN_d;
    exit;
  end;
  if x=0.0 then begin
    if a=1.0 then sfd_igammap_der := 1.0
    else if a>1.0 then sfd_igammap_der := 0.0
    else sfd_igammap_der := PosInf_d
  end
  else begin
    {compute result x^(a-1)*exp(-x)/Gamma(a) with igprefix function}
    sfd_igammap_der := sfd_igprefix(a,x)/x;
  end;
end;
...
begin
  if IsNanOrInfD(y) or (y<y0) then sfd_invgam := NaN_d
  else sfd_invgam := sfd_ilng(ln(y));
end;
...
begin
  ...
  if IsNanOrInfD(x) or IsNanOrInfD(a) or IsNanOrInfD(b) or (a <= 0.0) or (b <= 0.0) or (x < 0.0) or (x > 1.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfd_ibeta := NaN_d;
...
begin
  {Refs:  Cephes[7], function incbetl in ldouble/incbetl.c}
  {and  Boost [19], beta.hpp; [37] Alg.708, function BASYM}
  if IsNanOrInfD(x) or IsNanOrInfD(a) or IsNanOrInfD(b) or (x < 0.0) or (x > 1.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfd_nnbeta := NaN_d;
...
begin
  ...
  if IsNanOrInfD(y) or IsNanOrInfD(a) or IsNanOrInfD(b) or (a <= 0.0) or (b <= 0.0) or (y < 0.0) or (y > 1.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfd_ibeta_inv := NaN_d;
