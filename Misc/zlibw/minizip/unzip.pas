...
function unzStringFileNameCompare(const fileName1: PChar8;
                                  const fileName2: PChar8;
...
function unzOpen(const path: PChar8): unzFile;
...
function unzOpenCurrentFile(afile: unzFile): int;

{Open for reading data the current file in the zipfile.
  If there is no error, the return value is UNZ_OK.}


function unzCloseCurrentFile(afile: unzFile): int;
...
const
  unz_copyright: PChar8 = ' unzip 0.15 Copyright 1998 Gilles Vollant ';
...
type
  file_in_zip_read_info_s = record
    read_buffer: PChar8;  {internal buffer for compressed data}
...
begin
  ...
  if err=1 then begin
    ...
    if feof(fin)=1 then unzlocal_getByte := UNZ_ERRNO
    else unzlocal_getByte := UNZ_EOF;
  end;
end;
...
begin
  ...
  if err=UNZ_OK then err := unzlocal_getByte(fin,i);
  inc(x, uLong(i) shl 8);

  if err=UNZ_OK then pX := x else pX := 0;
...
{---------------------------------------------------------------------------}
function strcmpcasenosensitive_internal(fileName1, fileName2: PChar8): int;
  {-My own strcmpi / strcasecmp}
var
  c1, c2: char8;
begin
  repeat
    c1 := upcase(fileName1^); inc(fileName1);
    c2 := upcase(fileName2^); inc(fileName2);
    if c1=#0 then begin
      if c2=#0 then strcmpcasenosensitive_internal := 0
      else strcmpcasenosensitive_internal := -1;
      exit;
    end;
    if c2=#0 then begin
...
begin
  if iCaseSensitivity=0 then iCaseSensitivity := CASESENSITIVITYDEFAULTVALUE;
  if iCaseSensitivity=1 then unzStringFileNameCompare := strComp(fileName1,fileName2)
  else unzStringFileNameCompare := strcmpcasenosensitive_internal(fileName1,fileName2);
end;
...
begin
  ...
  if fseek(fin,0,SEEK_END)<>0 then begin
  ...
  if uMaxBack>uSizeFile then uMaxBack := uSizeFile;
  ...
  uBackRead := 4;
  while uBackRead<uMaxBack do begin
    if uBackRead+BUFREADCOMMENT>uMaxBack then uBackRead := uMaxBack
    else inc(uBackRead, BUFREADCOMMENT);
    uReadPos := uSizeFile-uBackRead ;

    if BUFREADCOMMENT+4 < uSizeFile-uReadPos then uReadSize := BUFREADCOMMENT+4
    else uReadSize := uSizeFile-uReadPos;

    if fseek(fin,uReadPos,SEEK_SET)<>0 then break;
    if fread(buf, uInt(uReadSize), 1, fin)<>1 then break;

    i := int(uReadSize)-3;
    while i>0 do begin
      dec(i);
      if (buf^[i]=$50) and (buf^[i+1]=$4b) and (buf^[i+2]=$05) and (buf^[i+3]=$06) then begin
        {ENDHEADERMAGIC}
        uPosFound := uReadPos+uInt(i);
        break;
      end;
    end;
    if uPosFound<>0 then break;
  end;
  TRYFREE(buf);
...
begin
  ...
  if central_pos= 0 then err := UNZ_ERRNO;

  if fseek(fin,central_pos,SEEK_SET)<>0 then err := UNZ_ERRNO;

  {the signature, already checked}
  if unzlocal_getLong(fin,uL)<>UNZ_OK then err := UNZ_ERRNO;

  {number of this disk}
  if unzlocal_getShort(fin,number_disk)<>UNZ_OK then err := UNZ_ERRNO;

  {number of the disk with the start of the central directory}
  if unzlocal_getShort(fin,number_disk_with_CD)<>UNZ_OK then err := UNZ_ERRNO;

  {total number of entries in the central dir on this disk}
  if unzlocal_getShort(fin,us.gi.number_entry)<>UNZ_OK then err := UNZ_ERRNO;

  {total number of entries in the central dir}
  if unzlocal_getShort(fin,number_entry_CD)<>UNZ_OK then err := UNZ_ERRNO;

  if ((number_entry_CD<>us.gi.number_entry) or (number_disk_with_CD<>0) or (number_disk<>0)) then begin
    err := UNZ_BADZIPFILE;
  end;

  {size of the central directory}
  if unzlocal_getLong(fin,us.size_central_dir)<>UNZ_OK then err := UNZ_ERRNO;

  {offset of start of central directory with respect to the starting disk number}
  if unzlocal_getLong(fin,us.offset_central_dir)<>UNZ_OK then err := UNZ_ERRNO;

  {zipfile comment length}
  if unzlocal_getShort(fin,us.gi.size_comment)<>UNZ_OK then err := UNZ_ERRNO;

  if ((central_pos<us.offset_central_dir+us.size_central_dir) and (err=UNZ_OK)) then err := UNZ_BADZIPFILE;
...
begin
  ...
  if s^.pfile_in_zip_read<>nil then unzCloseCurrentFile(afile);
...
begin
  ...
  if fseek(s^.afile, s^.pos_in_central_dir+s^.byte_before_the_zipfile,SEEK_SET)<>0 then err := UNZ_ERRNO;

  {we check the magic}
  if err=UNZ_OK then begin
    if unzlocal_getLong(s^.afile, uMagic)<>UNZ_OK then err := UNZ_ERRNO
    else if uMagic<>CENTRALHEADERMAGIC then err := UNZ_BADZIPFILE;
  end;

  if unzlocal_getShort(s^.afile, file_info.version)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, file_info.version_needed)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, file_info.flag)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, file_info.compression_method)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getLong(s^.afile, file_info.dosDate)<>UNZ_OK then err := UNZ_ERRNO;

  unzlocal_DosDateToTmuDate(file_info.dosDate, file_info.tmu_date);

  if unzlocal_getLong(s^.afile, file_info.crc)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getLong(s^.afile, file_info.compressed_size)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getLong(s^.afile, file_info.uncompressed_size)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, file_info.size_filename)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, file_info.size_file_extra)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, file_info.size_file_comment)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, file_info.disk_num_start)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, file_info.internal_fa)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getLong(s^.afile, file_info.external_fa)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getLong(s^.afile, file_info_internal.offset_curfile)<>UNZ_OK then err := UNZ_ERRNO;

  inc(lSeek, file_info.size_filename);
  if ((err=UNZ_OK) and (szFileName<>nil)) then begin
    if (file_info.size_filename<fileNameBufferSize) then begin
      (szFileName+file_info.size_filename)^:=#0;
      uSizeRead := file_info.size_filename;
    end
    else uSizeRead := fileNameBufferSize;

    if (file_info.size_filename>0) and (fileNameBufferSize>0) then begin
      if fread(szFileName, uInt(uSizeRead),1,s^.afile)<>1 then err := UNZ_ERRNO;
    end;
    dec(lSeek, uSizeRead);
  end;

  if ((err=UNZ_OK) and (extraField<>nil)) then begin
    if (file_info.size_file_extra<extraFieldBufferSize) then uSizeRead := file_info.size_file_extra
    else uSizeRead := extraFieldBufferSize;

    if lSeek<>0 then begin
      if fseek(s^.afile,lSeek,SEEK_CUR)=0 then lSeek := 0 else err := UNZ_ERRNO;
    end;

    if ((file_info.size_file_extra>0) and (extraFieldBufferSize>0)) then begin
      if fread(extraField, uInt(uSizeRead),1, s^.afile)<>1 then err := UNZ_ERRNO;
    end;
    inc(lSeek, file_info.size_file_extra - uSizeRead);
  end
  else inc(lSeek, file_info.size_file_extra);

  if ((err=UNZ_OK) and (szComment<>nil)) then begin
    if (file_info.size_file_comment<commentBufferSize) then begin
    ...
    if ((file_info.size_file_comment>0) and (commentBufferSize>0)) then begin
      if fread(szComment, uInt(uSizeRead),1,s^.afile)<>1 then err := UNZ_ERRNO;
    end;
    inc(lSeek, file_info.size_file_comment - uSizeRead);
  end
  else inc(lSeek, file_info.size_file_comment);

  if ((err=UNZ_OK) and (pfile_info<>nil)) then pfile_info^ := file_info;

  if ((err=UNZ_OK) and (pfile_info_internal<>nil)) then pfile_info_internal^ := file_info_internal;
...
begin
  ...
  s := unz_s_ptr(afile);
  if not s^.current_file_ok then begin
    unzGoToNextFile := UNZ_END_OF_LIST_OF_FILE;
    exit;
  end;
  if s^.num_file+1 = s^.gi.number_entry then begin
    unzGoToNextFile := UNZ_END_OF_LIST_OF_FILE;
    exit;
  end;
...
var
  szCurrentFileName: array[0..UNZ_MAXFILENAMEINZIP+1-1] of char8;
begin
  if afile=nil then begin
  ...
  if strlen(szFileName)>=UNZ_MAXFILENAMEINZIP then begin
    unzLocateFile := UNZ_PARAMERROR;
    exit;
  end;
  ...
  while err=UNZ_OK do begin
    unzGetCurrentFileInfo(afile,nil, szCurrentFileName,sizeof(szCurrentFileName)-1, nil,0,nil,0);
    if unzStringFileNameCompare(szCurrentFileName, szFileName,iCaseSensitivity)=0 then begin
...
begin
  ...
  if err=UNZ_OK then begin
    if unzlocal_getLong(s^.afile, uMagic)<>UNZ_OK then err := UNZ_ERRNO
    else if uMagic<>$04034b50 then err := UNZ_BADZIPFILE;
  end;

  if unzlocal_getShort(s^.afile, uData)<>UNZ_OK then err := UNZ_ERRNO;
{
  else
    if ((err=UNZ_OK) and (uData<>s^.cur_file_info.wVersion)) then
      err := UNZ_BADZIPFILE;
}
  if unzlocal_getShort(s^.afile, uFlags)<>UNZ_OK then err := UNZ_ERRNO;
  if unzlocal_getShort(s^.afile, uData)<>UNZ_OK then err := UNZ_ERRNO
  else if (err=UNZ_OK) and (uData<>s^.cur_file_info.compression_method) then err := UNZ_BADZIPFILE;
...
begin
  ...
  if (unzlocal_CheckCurrentFileCoherencyHeader(s, iSizeVar, offset_local_extrafield, size_local_extrafield)<>UNZ_OK) then begin
    unzOpenCurrentFile := UNZ_BADZIPFILE;
    exit;
  end;

  pfile_in_zip_read_info := file_in_zip_read_info_s_ptr(ALLOC(sizeof(file_in_zip_read_info_s)));
  if pfile_in_zip_read_info=nil then begin
    unzOpenCurrentFile := UNZ_INTERNALERROR;
    exit;
  end;

  pfile_in_zip_read_info^.read_buffer := PChar8(ALLOC(UNZ_BUFSIZE));
  pfile_in_zip_read_info^.offset_local_extrafield := offset_local_extrafield;
  pfile_in_zip_read_info^.size_local_extrafield := size_local_extrafield;
  pfile_in_zip_read_info^.pos_local_extrafield := 0;

  if pfile_in_zip_read_info^.read_buffer=nil then begin
  ...
  if not Store then begin
    ...
    err := inflateInit2(pfile_in_zip_read_info^.stream, -DEF_WBITS);  {*we W0802}

    if err=Z_OK then pfile_in_zip_read_info^.stream_initialised := true;
...
begin
  ...
  if len>pfile_in_zip_read_info^.rest_read_uncompressed then begin
    pfile_in_zip_read_info^.stream.avail_out := uInt(pfile_in_zip_read_info^.rest_read_uncompressed);
  end;

  while pfile_in_zip_read_info^.stream.avail_out>0 do begin
    ...
    if pfile_in_zip_read_info^.compression_method=0 then begin
      ...
      if err=Z_STREAM_END then begin
        if iRead=0 then unzReadCurrentFile := UNZ_EOF
        else unzReadCurrentFile := iRead;
        exit;
      end;
...
begin
  ...
  if pfile_in_zip_read_info^.rest_read_uncompressed=0 then unzeof := 1
  else unzeof := 0;
end;
...
begin
  ...
  if len>size_to_read then read_now := uInt(size_to_read)
  else read_now := uInt(len);

  if read_now=0 then begin
  ...
  if fread(buf,uInt(size_to_read),1, pfile_in_zip_read_info^.afile)<>1 then begin
    unzGetLocalExtrafield := UNZ_ERRNO;
    exit;
  end;
...
begin
  ...
  if pfile_in_zip_read_info^.rest_read_uncompressed=0 then begin
    if pfile_in_zip_read_info^.crc32<>pfile_in_zip_read_info^.crc32_wait then err :=UNZ_CRCERROR;
  end;


  TRYFREE(pfile_in_zip_read_info^.read_buffer);
  pfile_in_zip_read_info^.read_buffer := nil;
  if pfile_in_zip_read_info^.stream_initialised then inflateEnd(pfile_in_zip_read_info^.stream);
...
begin
  ...
  uReadThis := uSizeBuf;
  if uReadThis>s^.gi.size_comment then uReadThis := s^.gi.size_comment;

  if fseek(s^.afile,s^.central_pos+22,SEEK_SET)<>0 then begin
  ...
  if uReadThis>0 then begin
    szComment^ := #0;
    if fread(szComment, uInt(uReadThis), 1,s^.afile)<>1 then begin
      unzGetGlobalComment := UNZ_ERRNO;
      exit;
    end;
  end;

  if ((szComment<>nil) and (uSizeBuf > s^.gi.size_comment)) then (szComment+s^.gi.size_comment)^ := #0;
