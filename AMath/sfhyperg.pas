...
{Common code for special functions: Hypergeometric functions and related}
...
(*************************************************************************
 ...
 DESCRIPTION   :  Common code for hypergeometric functions and related
 ...
 REMARKS       :  - sfc_chu is still beta
                  - In sfc_whitm the sfc_1f1 call may produce overflows
 ...
 -------  --------  -------     ------------------------------------------
 0.01.00  09.11.10  W.Ehrhardt  1F1: Pearson's Taylor method
 0.01.01  10.11.10  we          1F1: Bessel expansion with Buchholz polynomials
 0.01.02  10.11.10  we          1F1: Continued fraction for asymptotic series
 0.01.03  11.11.10  we          1F1: Tricomi version of Bessel expansion
 0.01.03  11.11.10  we          1F1: generalized Laguerre polynomials
 0.01.04  11.11.10  we          1F1: Direct summation for a negative int
 0.01.05  13.11.10  we          1F1: Luke rational approximation
 0.01.06  16.11.10  we          1F1: Range selection roughly from Muller

 0.01.07  26.02.12  we          2F1: Pascal port of Cephes hyp2f1
 0.01.08  27.02.12  we          2F1: Allow negative x < -1
 0.01.09  27.02.12  we          2F1: Allow all x if 2F1 is a polynomial
 0.01.10  28.02.12  we          2F1: Better termination in h2f1_sum,
                                     if d<0, try h2f1_trans only if c > -1
 0.01.10  29.02.12  we          2F1: Use inline TwoSum in h2f1_sum, handle special case x=0
 0.01.11  01.03.12  we          2F1: Fix case d<0, const BIGNEG
 0.01.12  02.03.12  we          2F1: Use Luke code if x<0 and err>ETHRESH

 0.01.13  03.03.12  we          Combine 1F1 and 2F1 in new unit
 0.01.14  04.03.12  we          1F1: Fix b=2a case selection
 0.01.15  05.03.12  we          1F1: Use inline TwoSum in h1f1_sum
 0.01.16  02.04.12  we          1F1: Fix case if b non-positive integer
 0.01.17  18.04.12  we          1F1: use sfc_lngammas
 0.01.18  20.04.12  we          1F1: h1f1_asym
 0.01.19  30.04.12  we          1F1: direct call h1f1_b2a, rearrange non-special cases

 0.01.20  06.05.12  we          experimental: sfc_chu and d9chu
 0.01.21  18.05.12  we          experimental: d9chu returns error code

 0.01.22  22.04.13  we          2F1: improved error estimate, RTE_NoConvergence

 1.18.00  14.05.13  we          2F1: major rewrite (additional linear transformations)
 1.18.01  14.05.13  we          2F1: fix y=0 in h2f1_sum
 1.18.02  16.05.13  we          2F1: removed recurrence on c (to make c-a-b > 0)
 1.18.03  17.05.13  we          2F1: h2f1poly with parameter err
 1.18.04  18.05.13  we          2F1: no Luke for TP5, special x=1 in h2f1poly
 1.18.05  20.05.13  we          1F1: removed h1f1_taylor, h1f1_sum like h2f1_sum
 1.18.06  21.05.13  we          2F1: regularized function sfc_2f1r
 1.18.07  21.05.13  we          1F1: h1f1_sum_laguerre
 1.18.08  23.05.13  we          1F1: h1f1_tri0comi only if 2bx > 4ax
 1.18.09  23.05.13  we          1F1: fix h1f1_acf if x >= Ln_MaxExt
 1.18.10  24.05.13  we          1F1: regularized function sfc_1f1r
 1.18.11  27.05.13  we          CHU: handle a or b negative integer
 1.18.12  28.05.13  we          CHU: special cases a=-1, x=0, a=b
 1.18.13  29.05.13  we          CHU: case b=2a, allow a or a+1-b negative integer in luke
 1.18.14  30.05.13  we          CHU: handle b=a+n+1
 1.18.15  31.05.13  we          CHU: fix some double/overflow cases in dchu

 1.19.00  09.06.13  we          CHU: fix sign in special case a=-1
 1.19.01  12.06.13  we          CHU: implementation of Temme's chu and uabx
 1.19.02  14.06.13  we          CHU: use dchu with Kummer if b<0.0 and t>T_MAXA
 1.19.03  16.06.13  we          Whittaker functions sfc_whitm/sfc_whitw
 1.19.04  27.06.13  we          sfc_0f1
 1.19.05  28.06.13  we          gen_0f1, sfc_0f1r

 1.20.00  27.07.13  we          MAXIT=64 in h1f1_tricomi

 1.24.00  24.03.14  we          CHU: chu_luke only with $ifdef in dchu

 1.27.00  11.07.14  we          sfc_pcfd
 1.27.01  11.07.14  we          sfc_pcfu
 1.27.02  12.07.14  we          sfc_pcfhh

 1.28.00  19.08.14  we          sfc_pcfv

 1.29.00  01.09.14  we          sfc_chu for x<0
 1.29.01  04.09.14  we          fix h1f1_tricomi (division by t=0)

 1.30.00  26.10.14  we          No Kummer transformation in 1F1 if b=0,-1,-2,...
 1.30.01  28.10.14  we          1F1 = 1 + a/b*(exp(x)-1) for very small a,b

 1.31.00  05.01.15  we          Special case b=1 in gen_0f1

 1.34.00  01.08.15  we          Avoid some overflows in h2f1_sum, improved h2f1_trans

 1.36.00  29.09.15  we          sfc_pcfv(a,x) for a<0

 1.40.00  29.06.17  we          Code removed for TP5-TP6, TPW1-D1

 1.49.00  02.08.18  we          Fix tmax update in hyp2f0
 1.49.01  03.08.18  we          sfc_2f0
...
const
  ETHRESH = 5.4e-18; {~  50 eps_x}
  IEPS    = 3.3e-16; {~ 1.5 eps_d}


{---------------------------------------------------------------------------}
function is_negint(x: extended): boolean;
...
{---------------------------------------------------------------------------}
function h1f1_b2a(a,x: extended; var OK: boolean): extended;
...
begin
  ...
  if (x=0.0) or (a=0.0) then h1f1_b2a := 1.0
    ...
    if (x>=ln_MaxExt) or (abs(a) > MAXGAMX-5) then begin
...
{---------------------------------------------------------------------------}
function h1f1_tricomi(a,b,x: extended; var OK: boolean): extended;
  {-Compute M(a,b,x) with Tricomi version of Bessel expansion, b no negative integer}
var
  j: integer;
  d0,d1,d2,f,q,r,s,t,w,y: extended;
const
  MAXIT = 64;
  EPST  = 0.138e-75; {~ eps_x^4, DAMath = 0.243e-62}
begin
  ...
  r := x/y;
  q := sqr(r)/t;
  s := sfc_jv(b-1,y)/t + d2*q*sfc_jv(b+1,y);
  for j:=3 to MAXIT do begin
    ...
    d2 := t/j;
    q  := q*r;
    t  := d2*q*sfc_jv(b+(j-1),y);
    s  := s + t;
    if abs(t)<abs(s)*eps_x then begin
      OK := true;
      h1f1_tricomi := s*f;
      exit;
    end;
...
{---------------------------------------------------------------------------}
function h1f1_luke(a,b,x: extended): extended;
...
var
  z,z3,t,h,e,e0: extended;
  an,a2,a1,a0,bn,b2,b1,b0: extended;
  na,nb,f1,f2,f3: extended;
  n: integer;
begin
  ...
  for n:=3 to NMAX do begin
    ...
    if abs(e-e0)<eps_x*abs(e) then begin
      h1f1_luke := e;
      exit;
    end;
...
{---------------------------------------------------------------------------}
function h1f1_acf(a,b,x: extended): extended;
  {-Compute M(a,b,x) with continued fraction for asymptotic series}
var
  rj, a0, a1, a2,f: extended;
  j,k: integer;
const
  MAXIT = 1000;
begin
  ...
  if x<0 then begin
    ...
    x := -x;
    a := b-a;
    f := sfc_gamma_ratio(b,a)*power(x,a-b);
  end
  else begin
    if x>=Ln_MaxExt then begin
      ...
      f := exp(x)*sfc_gamma_ratio(b,a)*power(x,a-b);
    end;
  end;
  a0 := 1.0;
  a1 := 1.0 + (b-a)*(1.0-a)/x;
  for j:=2 to MAXIT do begin
    rj:= (b-a+j-1)*(j-a)/j;
    a2 := a1 + (a1-a0)*rj/x;
    if (abs(a1-a0)<eps_x*abs(a0)) and (abs(a2-a1)<eps_x*abs(a1)) then begin
      h1f1_acf := f*a2;
      exit;
    end;
    a0 := a1;
...
{---------------------------------------------------------------------------}
function h1f1_sum(a,b,x: extended; var loss: extended): extended;
...
begin
  ...
  repeat
    ...
    if u<>0.0 then begin
      ...
      e := e+y;
      s := w;
      if abs(u)<eps_x*abs(s) then begin
  ...
  if i>IMAX then loss := 1.0
    ...
    loss := maxx(ifac*i,lfac*umax/abs(s));
    loss := eps_x*maxx(1.0,loss);
  end;
end;


{---------------------------------------------------------------------------}
function hyp2f0(a,b,x: extended; var OK: boolean): extended;
  {-Asymptotic sum, OK if convergence, x should be small: 0 < x < 0.02}
var
  an,bn,a0,al,t,tlst,tmax,u,sum: extended;
  n: integer;
const
...
begin
  ...
  al := 1.0;
  sum  := 0.0;
  tlst := 0.5*MaxExtended;
  tmax := 0.0;
  for n:=1 to nmax do begin
    u := an*((bn/n)*x);
    {check for blowup}
    t := abs(u);
    if (t>1.0) and (tmax > MaxExtended/t) then exit;
    a0 := a0*u;
    t  := abs(a0);
    ...
    an := an+1.0;
    bn := bn+1.0;
    if t>tmax then tmax := t; {fix added in V1.49.01}
  end;
end;


{---------------------------------------------------------------------------}
function h1f1_asym(a,b,x: extended; var OK: boolean): extended;
  {- Return M(a,b,x) = Gamma(b)/Gamma(a)*e^x*x^(a-b)*2F0(b-a,1-a,1/x) for x -> INF}
var
  sa,sb: integer;
  f,t,la,lb: extended;
begin
  ...
  if OK then begin
    lb := sfc_lngammas(b,sb);
    la := sfc_lngammas(a,sa);
    t  := (a-b)*ln(x);
    t  := lb - la + x + t;
...
{---------------------------------------------------------------------------}
function h1f1_sum_laguerre(a,b,x: extended): extended;
...
begin
  ...
  r := sfc_laguerre(n,b-1.0,x);
  h1f1_sum_laguerre := n*sfc_beta(b,n)*r;
  exit;
end;
...
var
  ...
  a_ni,OK: boolean;
begin
  if IsNanOrInf(a) or IsNanOrInf(b) or IsNanOrInf(x) then begin
    {$ifopt R+}
  ...
  if (x=0.0) or (a=0.0) then begin
    ...
    {Rel. error is small if |a|<eps_x/2, |b|<eps_x/2. But it can be larger}
    {if a, b have magnitude of about 1.5*eps_d, eg. with cast from double.}

    sfc_1f1 := 1.0 + (a/b)*expm1(x);
    exit;
  end;
  ...
  if is_negint(b) then begin
    ...
    if (b=0.0) or (not a_ni) or (a<b) then begin
      ...
      if a>THRESH_LAGUERRE_A then begin
        ...
        sfc_1f1 := h1f1_sum_laguerre(a,b,x);
      end
      else begin
        sfc_1f1 := h1f1_sum(a,b,x,err);
        {$ifdef debug}
          ...
          if err >= sqrt_epsh then begin
            sfc_write_debug_str('*** sfc_1f1: loss of accuracy');
          end;
        {$endif}
      end;
    end;
  ...
  if x>=50.0 then begin
    r := maxx(1.0,abs(b-a)) * maxx(1.0, abs(1.0-a));
    if r < 0.5*x then begin
      sfc_1f1 := h1f1_asym(a,b,x,OK);
      if OK then exit;
    end;
  end;
  ...
  if isign(x)<>isign(a) then begin
    sfc_1f1 := h1f1_tricomi(a,b,x,OK);
    if OK then exit;
  end;
  if x>0.0 then sfc_1f1 := h1f1_sum(a,b,x,err)
  else begin
    if a<0.0 then begin
...
begin
  ...
  if b_ni then begin
    ...
    if t=0.0 then sfc_1f1r := 0.0
    else begin
      f := sfc_1f1(a-b+1.0, 2.0-b, x);
      sfc_1f1r := f*t;
    end;
  end
  else begin
...
{---------------------------------------------------------------------------}
function h2f1_sum(a,b,c,x: extended; var loss: extended): extended;
...
begin
  ...
  repeat
    ...
    if y=0.0 then begin
      ...
      w := abs(y);
      if w > 1.0 then begin
        if abs(u) >= MaxExtended/w then begin
  ...
  if i>IMAX then loss := 1.0
    ...
    loss := maxx(0.5*i,lfac*umax/abs(s));
    loss := eps_x*maxx(1.0,loss);
  end;
end;
...
{---------------------------------------------------------------------------}
function h2f1_luke(a,b,c,x: extended; var loss: extended): extended;
...
var
  z,z3,t,h,e,e0: extended;
  ab,an,a2,a1,a0,bn,b2,b1,b0: extended;
  na,nb,nc,f1,f2,f3,err: extended;
  n: integer;
begin
  ...
  repeat
    ...
    if (err < eps_x) or (n > NMAX) then begin
      h2f1_luke := e;
      loss := 2.0*(err + eps_x*n);
      exit;
    end;
...
{---------------------------------------------------------------------------}
function h2f1_trans(a,b,c,x: extended; var loss: extended): extended;
  {-Conditionally apply linear transformation and/or call the power series}
var
  p,q,r,s,t,y,d,ax,d1,d2,e,y1,l2: extended;
  r1, l1: extended;
  i,id,aid: longint;
  s1,s2,s3,s4: integer;
const
  ...
  function reflectx: extended;
    {-Compute with s=1-x via HMF[1], 15.3.6}
  begin
    q := h2f1_sum(a, b, 1.0-d, s, l2);
    e := sfc_lngammas(c,s4);
    t := e + sfc_lngammas(d,s1) - (sfc_lngammas(c-a,s2) + sfc_lngammas(c-b,s3));
    q := q*exp(t)*(s1*s2*s3*s4);
    r := power(s,d)*h2f1_sum(c-a, c-b, 1.0+d, s, loss);
    t := e + sfc_lngammas(-d,s1) - (sfc_lngammas(a,s2)+sfc_lngammas(b,s3));
...
begin
  ...
  if x <= 0.75 then begin
    ...
    if abs(d-rint(d)) > IEPS then begin
      ...
      ax := ln(s);
      {sum for t = 0}
      r := sfc_psi(1.0+e) - Eulergamma - sfc_psi(a+d1) - sfc_psi(b+d1) - ax;
      y := r*sfc_rgamma(e+1.0);
      p := (a+d1)*(b+d1)*s*sfc_rgamma(e+2.0);      {Poch for t=1}
      t := 1.0;
      repeat
      ...
      if id=0.0 then begin
        t := sfc_lngammas(c,s1) - (sfc_lngammas(a,s2)+sfc_lngammas(b,s3));
      ...
      p  := sfc_lngammas(c,s4);
      t  := sfc_lngammas(e,s1) - (sfc_lngammas(a+d1,s2)+sfc_lngammas(b+d1,s3));
      y1 := y1*exp(p+t)*(s1*s2*s3*s4);
      q  := sfc_rgamma(a+d2)*sfc_rgamma(b+d2);
      y  := y*q;
      if y<>0.0 then begin
        if odd(aid) then y := -y;
        y := y*exp(p)*s4;
...
{---------------------------------------------------------------------------}
function h2f1_ptprefix(m: longint; b,c,z: extended): extended;
...
{---------------------------------------------------------------------------}
function h2f1poly(a,b,c,x: extended; var err: extended): extended;
...
begin
  ...
  {$ifdef debug}
    if abs(a+m) >= IEPS then begin
      sfc_write_debug_str('a no negative integer');
      {$ifopt R+}
        if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
      {$endif}
      h2f1poly := NaN_x;
...
{---------------------------------------------------------------------------}
function hyp2f1e(a,b,c,x: extended; var err: extended): extended;
  {-Return the Gauss hypergeometric function 2F1(a,b;c;x)}
var
...
begin
  ...
  if abs(s) < IEPS then begin
    ...
    if d<=0.0 then begin
      ...
      t := sfc_rgamma(c-a)*sfc_rgamma(c-b);
      if t<>0.0 then begin
        {d and signgamma(d) >0}
        r:= sfc_lngamma(c)+sfc_lngamma(d);
        t := t*exp(r)*sfc_signgamma(c);
      end;
      hyp2f1e := t;
    end;
    exit;
  ...
  if x<0.0 then begin
    ...
    {0 < x <= 1: use direct sum or linear transformation HMF[1] 15.3.6}
...
var
  err: extended;
begin
  if IsNanOrInf(a) or IsNanOrInf(b) or IsNanOrInf(c) or IsNanOrInf(x) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_2f1 := NaN_x;
  ...
  sfc_2f1 := hyp2f1e(a,b,c,x,err);
  {$ifdef debug}
    if err >= sqrt_epsh then begin
      sfc_write_debug_str('*** sfc_2f1: loss of accuracy');
    end;
  {$endif}
end;
...
begin
  ...
  if is_negint(c) then begin
    ...
    if t=0.0 then sfc_2f1r := 0.0
    else begin
      f := sfc_2f1(a-c+1.0, b-c+1.0, 2.0-c, x);
      sfc_2f1r := f*t;
    end;
  end
  else begin
...
const
  ...
  T_MAXA  = 1700;      { Temmes code underflows for a >~ T_MAXA ~ MAXGAMX}
  T_EPS   = 8.67e-19;  { ~ 8*eps_x}


{---------------------------------------------------------------------------}
function chu_an1(a,x: extended; n: longint): extended;
...
{---------------------------------------------------------------------------}
function u2f0(a,b,x: extended): extended;
...
{-------------------- Functions SLATEC's CHU code --------------------------}

{---------------------------------------------------------------------------}
function d9chu(a,b,z: extended; var Err: integer): extended;
  {-Return Luke's rational approximation for z^a*U(a,b,z)}
  { Err = 0: OK; = 1: loss of precision; = 2: No convergence}
var
  ab,anbn,bp,ct1,ct2,ct3,c2,d1z,eps,g1,g2,g3,sab,sqeps,x2i1,res: extended;
  i,j: integer;
var
  aa,bb: array[1..4] of extended;
const
  MAXIT = 300;
  IMAX  = -IMAX2F0+1;
begin
...
{---------------------------------------------------------------------------}
function chu_luke(a,b,x: extended; var Err: integer): extended;
  {-Check if Luke can be used: If yes and result is OK then Err=0}
var
  t: extended;
begin
  t := maxx(abs(a),1.0)*maxx(abs(1.0+a-b),1.0);
...
{$endif}
  ...
  if n<1 then begin
    ...
    sum := sfc_pochhammer(1.0+a-b,-a)*sum;
  end
  else begin
    ...
    if m<0 then sum := 0.0
      ...
      if b<MAXGAMX then begin
        t := sfc_gamma(b-1.0)*sfc_rgamma(a);
        sum := power(x,1-n)*t*xtoeps*sum;
      end
      else begin
        t := sfc_lngammas(b-1,i);
        t := t - sfc_lngammas(a,m);
        t := t + (1-n)*alnx;
        t := (i*m)*exp(t);
        sum := t*xtoeps*sum;
      end;
  ...
  {factor := sfc_rgamma(1.0+a-b)*power(x,xi);}
  if (xi=0.0) or (x=1.0) then factor := sfc_rgamma(1.0+a-b)
  else begin
    t := sfc_lngammas(1.0+a-b,i);
  ...
  pochai := sfc_pochhammer(a,xi);
  gamri1 := sfc_rgamma(xi+1.0);
  gamrni := sfc_rgamma(aintb+xi);
  b0 := factor*sfc_pochhammer(a,xi-beps);
  b0 := b0*gamrni*sfc_rgamma(xi+1.0-beps);

  if abs(xtoeps-1.0) <= 0.5  then begin
    {x^(-beps) is close to 1, so we must be careful in evaluating the differences}
    pch1ai := sfc_poch1(a+xi, -beps);
    pch1i  := sfc_poch1(xi+1.0-beps, beps);
    t  := pch1ai - sfc_poch1(b+xi, -beps) - pch1i + beps*pch1ai*pch1i;
    ...
    a0 := factor*pochai*sfc_rgamma(b+xi)*gamri1/beps;
    b0 := xtoeps*b0/beps;
    value := sum + a0 - b0;
  ...
  {$ifdef debug}
    sfc_write_debug_str('No convergence in MAXIT terms of the ascending series');
  {$endif}
  if RTE_NoConvergence>0 then RunError(byte(RTE_NoConvergence));
  dchu := value;
end;
...
{---------------------------------------------------------------------------}
procedure tchu_miller(a,b,x: extended; var u,up: extended);
  {-Compute U, U' with Temme's Miller algorithm; a>=0, 0<=b<=1, x>=T_XMILL}
var
  ar,br,cr,c,er,m0,m1,mr,p0,p1,p2,u1,u2,u3,v,w: extended;
  n,r: longint;
  largex: boolean;
...
{$endif}
  ...
  if largex then mr := 1.0
    ...
    v  := exp(-x)*sfc_gamma(a+1.0)/sfc_gamma(b+1.0);
    m0 := v*(b+a*m0);
    m1 := v*m1;
  end;
  ...
  {Warning: Underflow for a > ~ MAXGAMX}
...
{---------------------------------------------------------------------------}
procedure tchu_bessel(a,b,x: extended; var u, up: extended);
  {-Compute U, U' with Temme's BesselK code; 0<=a, 0<=b<=1, 0<x<T_XMILL}
const
  n=9;
var
  d,delta,e,f,p,q,r,s,t,t0,t1,u0,u1,u2,u3,v,w,x2,z: extended;
  bb,bb1,bx: array[0..8] of extended;
  fi: array[-1..8] of extended;
  i: integer;
  nu,j: longint;
begin
...
{$endif}
  ...
  w  := 2.0*exp(0.5*x)/sfc_gamma(1.0+a);
  u2 := w*u0;
  u3 := -w*u1;
  v  := a+1.0-b+x;
...
{---------------------------------------------------------------------------}
procedure tchu(a,b,x: extended; var u,up: extended);
...
{---------------------------------------------------------------------------}
function tuabx(a,b,x: extended): extended;
  {-Compute U(a,b,x) with Temme's method; x > 0}
var
  a1,c,p,q,r,u: extended;
  j,n: longint;
begin
{$ifdef debug}
  if (x<=0.0) then begin
...
{---------------------------------------------------------------------------}
function drchu_alt0(a,b,x: extended): extended;
  {-Compute U(a,b,x) for a<0, frac(a)<>0 with double recursion on a- and b+}
var
  a1,b1,c,p,t,r: extended;
  j,n,m: longint;
begin
{$ifdef debug}
  if (a>=0.0) or (x<=0.0) then begin
...
{---------------------------------------------------------------------------}
function chu_negx(a,b,x: extended; var Err: integer): extended;
...
begin
  ...
  if a=0.0 then begin
    ...
    sfc_chu := sfc_gamma_delta_ratio(1.0-b,a);
    exit;
  end;

  if x<0.0 then begin
    sfc_chu := chu_negx(a,b,x,Err);
    if Err<>0 then begin
      {$ifopt R+}
        if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
      {$endif}
  ...
  if is_negint(a) then begin
    {Tricomi [53], 2.4(6)}
    m := round(-a);
    s := sfc_pochhammer(b,m);
    t := sfc_1f1(a,b,x);
    if odd(m) then sfc_chu := -s*t else sfc_chu := s*t;
    exit;
  end;
  *)
...
var
  a,b: extended;
begin
  if IsNanOrInf(k) or IsNanOrInf(k) or IsNanOrInf(x) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_whitm := NaN_x;
...
var
  a,b: extended;
begin
  if IsNanOrInf(k) or IsNanOrInf(k) or IsNanOrInf(x) or (x<0.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_whitw := NaN_x;
...
{---------------------------------------------------------------------------}
function cf_0f1(b,x: extended; var OK: boolean): extended;
  {-Evaluate continued fraction for 0F1}
var
  a,c,d,f,t,tiny,tol: extended;
  k: integer;
const
  MAXIT = 1000;
begin
...
{---------------------------------------------------------------------------}
function gen_0f1(b,x: extended; reg: boolean): extended;
...
{$endif}
  ...
  gen_0f1 := 1.0;
  if x=0.0 then begin
    if reg then gen_0f1 := sfc_rgamma(b);
    exit;
  end
  else if abs(x)<=XSMALL then begin
    f := cf_0f1(b,x,OK);
    if OK then begin
      if reg then f := f*sfc_rgamma(b);
      gen_0f1 := f;
      exit;
    end;
  end;
  ...
  if not UseCF then begin
    if (abs(b) < MAXGAMX - 10) then begin
      t := power(abs(x), -0.5*c);
      if reg then g := 1.0
      else g := sfc_gamma(b);
      if (t>1.0) and (abs(g) >= MaxExtended/t) then begin
        {Avoid overflow}
        UseCF := true;
      end
      else t := g*t;
      ...
      if abs(t)<ln_MaxExt then t := s*exp(t)
      else begin
    ...
    if not UseCF then begin
      ...
      f := t*f;
      if not IsNanOrInf(f) then begin
        gen_0f1 := f;
        exit;
      end;
      UseCF := true;
  ...
  if UseCF then begin
    ...
    if OK then begin
      f := cf_0f1(b,x,OK);
      if OK and reg then f := f*sfc_rgamma(b);
      gen_0f1 := f;
    end;
    {No convergence}
...
var
  t,f: extended;
begin
  if IsNanOrInf(b) or IsNanOrInf(x) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_0f1r := NaN_x;
  ...
  if (b<=0.0) and (frac(b)=0.0) then begin
    ...
    if x=0.0 then sfc_0f1r := 0.0
    else begin
      b := 1.0-b;
      t := power(x, b);
      if (t<>0.0) and not IsNanOrInf(t) then begin
        f := gen_0f1(1.0+b,x,true);
        if (f<>0.0) and not IsNanOrInf(f) then begin
          sfc_0f1r := t*f;
          exit;
        end;
      end;
...
function sfc_0f1(b,x: extended): extended;
  {-Return the confluent hypergeometric limit function 0F1(;b;x)}
begin
  if IsNanOrInf(b) or IsNanOrInf(x) or ((b<0.0) and (frac(b)=0.0)) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_0f1 := NaN_x;
...
begin
  ...
  if (x=0.0) or (a=0.0) or (b=0.0) then sfc_2f0 := 1.0
  else begin
    z := -1.0/x;
    if z>0.0 then begin
      t := sfc_chu(a, 1.0+a-b, z);
      sfc_2f0 := power(z,a)*t;
    end
...
var
  f,u,w,y,z: extended;
begin
  if IsNanOrInf(v) or IsNanOrInf(x) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_pcfd := NaN_x;
  ...
  if x<0.0 then begin
    if (frac(v)=0.0) and (v>0.0) then begin
      ...
      f := sfc_1f1(0.5-w,1.5, y);
      y := sqrt2*sinPi(w);
      f := y*f;
      y := v*sfc_gamma(w)/sqrtpi;
      u := u - z*y*f
    end;
  end;
  y := expx2(-0.5*z);
...
begin
  {HMF[1], 19.3.7}
  sfc_pcfu := sfc_pcfd(-(a+0.5), x);
end;


{---------------------------------------------------------------------------}
function pcfv_axpos(a,x: extended): extended;
  {-Compute parabolic cylinder V(a,x) with a,x positive, no checks}
var
  k: longint;
  v0,v1,vk,b: extended;
  z,bi0,bk0,bi1,bk1: extended;
const
  ...
  if frac(a)<>0.0 then begin
    ...
    if a>0.0 then begin
      ...
      if x<=eps1 then begin
        ...
        z := sqr(0.5*x);
        sfc_bess_ikv(0.25,z,bi0,bk0);
        sfc_bess_ikv(0.75,z,bi1,bk1);
        b  := sqrt(x);
...
{---------------------------------------------------------------------------}
function pcfv_gen(a,x: extended): extended;
  {-General parabolic cylinder V(a,x), used for a<0}
var
  f,t,v0,v1,u1,u2,s,c: extended;
begin
  f := exp2(0.5*a+0.25);
  {HMF[1], 19.3.6: v0 = V(a, 0)}
  sincospi(0.25-0.5*a,s,c);
  t  := sfc_rgamma(0.75-0.5*a);
  v0 := c*t;
...
var
  g,t: extended;
const
  amax = 3210.0;  {double 302.0} {Max a for V(a,0) without overflow}
begin
  if IsNanOrInf(a) or IsNanOrInf(x) or (frac(2.0*a)<>0) or (abs(a)>amax) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_pcfv := NaN_x;
  ...
  if a<0.0 then begin
    if (frac(a)=0.0) and (a > -MAXGAMX) then begin
      ...
      sfc_pcfv := pcfv_gen(a,x);
    end;
    exit;
  end;
  {Here a >= 0}
  if x>=0.0 then sfc_pcfv := pcfv_axpos(a,x)
  else begin
    {x<0, use reflection formulas}
    if frac(a)=0.0 then begin
      a := a+0.5;
      t := sfc_pcfd(-a, -x)/Pi;
      if t=0.0 then sfc_pcfv := 0.0
      else if a<MAXGAMX then sfc_pcfv := sfc_gamma(a)*t
      else begin
