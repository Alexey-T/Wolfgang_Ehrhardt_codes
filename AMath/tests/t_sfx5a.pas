{Part 5a of regression test for SPECFUNX unit  (c) 2010  W.Ehrhardt}
...
procedure test_legendre_plx;
procedure test_legendre_qlx;
procedure test_spherical_harmonix;
procedure test_legendre_plmx;
procedure test_legendre_qlmx;
procedure test_toroidal_qlmx;
procedure test_toroidal_plmx;
...
begin
  ...
  y := legendre_px(l, x);
  testrel(30, 800, y, f, cnt,failed);     {!!!!!!}
  y := legendre_px(l, -x);
  testrel(31, 800, y, f, cnt,failed);     {!!!!!!}

  x := 0.5;
  f := -0.9585404456573080972e-2;
  y := legendre_px(4000, x);
  testrel(32, 60, y, f, cnt,failed);      {!!!!!!}

  y := legendre_px(3,1.25);
  f := 3.0078125;
...
var
  ...
  P: array[1..400] of extended;
const
  NE = 60;
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','legendre_plmx');
  ...
  for i:=1 to 400 do begin
    x := S[i];
    f := P[i];
    y := legendre_plmx(L[i],M[i],x);
    testrel(i, NE, y, f, cnt,failed);
  end;

  y := legendre_plmx(1,-1,-2);
  f := -sqrt(3)/2;
  testrel(401, NE, y, f, cnt,failed);

  y := legendre_plmx(5,3,1.25);
  f := 289.31396484375;
  ...
  y := legendre_plmx(19,8,0.826751708984375);
  f := 0.3010989900889337580e10;
  testrel(426, NE, y, f, cnt,failed);

  y := legendre_plmx(19,8,1.826751708984375);
  ...
  y := legendre_plmx(19,7,-1.826751708984375);
  ...
  y := legendre_plmx(19,0,1.826751708984375);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','legendre_qlmx');

  y := legendre_qlmx(10,-9,2);
  f := -0.1340548590241211443e-12;
  testrel(1, NE, y, f, cnt,failed);

  y := legendre_qlmx(10,100,2);
  f := 0.1920639297988588242e180;
  testrel(2, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,4,1.5);
  f := 6.226151247203235330;
  testrel(3, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,1,1.5);
  f := -0.6231002914681709265e-2;
  testrel(4, NE, y, f, cnt,failed);

  y := legendre_qlmx(5,1,1.5);
  f := -0.1518597293561739855e-1;
  testrel(5, NE, y, f, cnt,failed);

  y := legendre_qlmx(5,2,1.5);
  ...
  y := legendre_qlmx(6,-5,1.5);
  f := -0.2076350026008685426e-5;
  testrel(8, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,-6,1.5);
  f := 0.2607389964046100887e-5;
  testrel(9, NE, y, f, cnt,failed);

  y := legendre_qlmx(0,2,1.5);
  f := 2.4;
  testrel(10, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,10,1.5);
  f := 186448556.8512;
  testrel(11, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,7,1.5);
  ...
  y := legendre_qlmx(6,4, -2.5);
  f := -0.6664938868986807675e-1;
  testrel(14, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,1, -2.5);
  f := 0.8459507504345526164e-4;
  testrel(15, NE, y, f, cnt,failed);

  y := legendre_qlmx(5,1, -2.5);
  f := -0.3766287608222185512e-3;
  testrel(16, NE, y, f, cnt,failed);

  y := legendre_qlmx(5,2, -2.5);
  f := 0.2691948087605342454e-2;
  testrel(17, NE, y, f, cnt,failed);

  {Wolfram Alpha N[LegendreQ[6,-4,3,-2.5],40]: -3.6733569604204186921288838591245580956435802638622627e-8}
  y := legendre_qlmx(6,-4, -2.5);
  f := -0.3673356960420418692e-7;
  testrel(18, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,-5, -2.5);
  f := 0.1937790855467949965e-7;
  testrel(19, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,-6, -2.5);
  f := -0.2068032380863430234e-7;
  testrel(20, NE, y, f, cnt,failed);

  y := legendre_qlmx(0,2, -2.5);
  f := -0.9523809523809523810;
  testrel(21, NE, y, f, cnt,failed);

  {WA N[LegendreQ[6,10,3,-2.5],40]: -625972.0671942246286269609884770234624462}
  y := legendre_qlmx(6,10, -2.5);
  f := -625972.0671942246286;
  testrel(22, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,7, -2.5);
  ...
  y := legendre_qlmx(6,4,0.5);
  f := -406.1384794042086830;
  testrel(25, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,1,0.5);
  f := -3.353769065702635648;
  testrel(26, NE, y, f, cnt,failed);

  y := legendre_qlmx(5,1,0.5);
  f := -0.9389683027208977621;
  testrel(27, NE, y, f, cnt,failed);

  y := legendre_qlmx(5,2,0.5);
  f := -15.56819951247750953;
  testrel(28, NE, y, f, cnt,failed);

  y := legendre_qlmx(0,2,0.5);
  f := 1.333333333333333333;
  testrel(29, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,10,0.5);
  f := 146800640.0;                 {MPMath}
  testrel(30, NE, y, f, cnt,failed);

  {WA N[LegendreQ[6,100,1,0.5],40]: 3.723930333485983533979756676953715234312e179}
  y := legendre_qlmx(6,100,0.5);
  f := 3.723930333485983534e+179;   {MPMath}
  testrel(31, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,7,0.5);
  f := -126124.0908053710470;       {MPMath}
  testrel(32, NE, y, f, cnt,failed);

  y := legendre_qlmx(6,6,0.5);
  f := 22177.04354339523877;
  testrel(33, NE, y, f, cnt,failed);

  {WA N[LegendreQ[6,-3,1,0.5],40]: -0.002297979741214719555228961100789212263679}
  y := legendre_qlmx(6,-3,0.5);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','spherical_harmonicx');
  ...
  spherical_harmonicx(40, -15, 100, 20,yr,yi);
  fr := -0.7518327450883533303e-2;
  fi :=  0.3401647863191862710;
  testrel( 1, NE, yr, fr, cnt,failed);
  testrel( 2, NE, yi, fi, cnt,failed);

  spherical_harmonicx(55, -42, -10, 40, yr, yi);
  fr := 0.2337815444224362910e-4;
  fi := 0.2186955777030927347e-4;
  testrel( 3, NE2, yr, fr, cnt,failed);
  testrel( 4, NE2, yi, fi, cnt,failed);

  spherical_harmonicx(15, -6, -10, 40, yr, yi);
  fr := -0.1368855380024927641;
  fi :=  0.3972535148099972498;
  testrel( 5, NE2, yr, fr, cnt,failed);
  testrel( 6, NE2, yi, fi, cnt,failed);
  ...
  spherical_harmonicx(19,  14, -0.75, 2.25,yr,yi);
  ...
  spherical_harmonicx(20, -14, -0.75, 2.25,yr,yi);
  ...
  spherical_harmonicx(20,  14, -4, 2.25,yr,yi);
  ...
  spherical_harmonicx(20,  15, -4, 2.25,yr,yi);
  ...
  spherical_harmonicx(39,  15, -0.75, 2.25,yr,yi);
  fr :=  0.1757594233240278197;
  fi := -0.1837126108841860058;
  testrel(39, NE, yr, fr, cnt,failed);
  testrel(40, NE, yi, fi, cnt,failed);

  spherical_harmonicx(39,  15, -0.75, -2.25,yr,yi);
  fr :=  0.1757594233240278197;
  fi :=  0.1837126108841860058;
  testrel(41, NE, yr, fr, cnt,failed);
  testrel(42, NE, yi, fi, cnt,failed);

  spherical_harmonicx(39,  15, +0.75, -2.25,yr,yi);
  fr := -0.1757594233240278197;
  fi := -0.1837126108841860058;
  testrel(43, NE, yr, fr, cnt,failed);
  testrel(44, NE, yi, fi, cnt,failed);

  spherical_harmonicx(39,  15, +0.75, 2.25,yr,yi);
  fr := -0.1757594233240278197;
  fi :=  0.1837126108841860058;
  testrel(45, NE, yr, fr, cnt,failed);
  testrel(46, NE, yi, fi, cnt,failed);

  spherical_harmonicx(40, -15, -0.75, 2.25,yr,yi);
  ...
  spherical_harmonicx(53,  42, -8.75, -2.25,yr,yi);
  fr := -0.8147976618889536160e-3;
  fi :=  0.2099802242493057018e-3;
  testrel(55, NE2, yr, fr, cnt,failed);
  testrel(56, NE2, yi, fi, cnt,failed);
...
begin
  ...
  y := toroidal_qlmx(1, -6, 10);
  f := -0.1249964450032705870e-2;
  testrel(58, NE, y, f, cnt, failed);

  y := toroidal_qlmx(1, -6, 1e20);
  ...
  y := toroidal_qlmx(1, 1000, 1.5);
  f := 0.6145183592555373192e2914;
  testrel(60, NE3, y, f, cnt, failed);

  y := toroidal_qlmx(0, 1000, 1.5);
  f := 0.6146722004130988435e2914;
  testrel(61, NE3, y, f, cnt, failed);

  y := toroidal_qlmx(0, 0, MaxExtended);
  f := 0.2036621834625155804e-2465;
  testrel(62, NE, y, f, cnt, failed);

  y := toroidal_qlmx(0, 1, MaxExtended);
  f := -0.101831091731257790e-2465;
  testrel(63, NE2, y, f, cnt, failed);

  y := toroidal_qlmx(0, 20, MaxExtended);
  f := 0.6211993895066759726e-2448;
  testrel(64, NE2, y, f, cnt, failed);

  y := toroidal_qlmx(1, 0, 1e3000);
  f := 0.5553603672697957809e-4500;
  testrel(65, NE, y, f, cnt, failed);

  y := toroidal_qlmx(1, 1, 1e3000);
  f := -0.8330405509046936713e-4500;
...
begin
  ...
  cnt := 0;
  failed := 0;
  writeln('Function: ','toroidal_plmx');

  {---------------------------------------------}
  l := 0;
  m := 0;

  y := toroidal_plmx(l,m,1.0);
  f := 1.0;
  testrel(1,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,1.0+1/1024.0);
  f := 0.999877963203748251;
  testrel(2,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,1.5);
  f := 0.945006330929758054;
  testrel(3,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,10);
  f := 0.6245209611910859454;
  testrel(4,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,100);
  f := 0.3009174858819926456;
  testrel(5,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,1e6);
  f := 0.7155242359923822686e-2;
  testrel(6,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,1e19);
  f := 0.6523810244569567804e-8;
  testrel(7,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,2e19);
  f := 0.4682801566049467791e-8;
  testrel(8,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,1e300);
  ...
  y := toroidal_plmx(l,m,1e19);
  f := 0.2847050173668708234e10;
  testrel(16,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,2e19);
  f := 0.4026336968358962803e10;
  testrel(17,NE,y,f,cnt,failed);
  ...
  y := toroidal_plmx(l,m,1e19);
  f := -0.3119552613601348490e-8;
  testrel(25,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,2e19);
  f := -0.2240742358815759825e-8;
  testrel(26,NE,y,f,cnt,failed);
  ...
  y := toroidal_plmx(l,m,1e19);
  f := 0.1423525086834354117e10;
  testrel(34,NE,y,f,cnt,failed);

  y := toroidal_plmx(l,m,2e19);
  f := 0.2013168484179481401e10;
  testrel(35,NE,y,f,cnt,failed);
...
{---------------------------------------------------------------------------}
  y := toroidal_plmx(0,0,1e1000);
  f := 0.1037463541855567979e-496;
  testrel(37,NE,y,f,cnt,failed);

  y := toroidal_plmx(1,0,1e1000);
  f := 0.9003163161571060696e500;
  testrel(38,NE,y,f,cnt,failed);

  y := toroidal_plmx(0,1,1e1000);
  f := -0.5182816127697054363e-497;
  testrel(39,NE,y,f,cnt,failed);

  y := toroidal_plmx(1,1,1e1000);
  f := 0.4501581580785530348e500;
  testrel(40,NE,y,f,cnt,failed);
