{Part 3d of regression test for SPECFUNX unit  (c) 2014-2015  W.Ehrhardt}
...
procedure test_levy_cdfx;
procedure test_levy_invx;
procedure test_levy_pdfx;

procedure test_logseries_cdfx;
procedure test_logseries_pmfx;

procedure test_zipf_pmfx;
procedure test_zipf_cdfx;

procedure test_wald_cdfx;
procedure test_wald_invx;
procedure test_wald_pdfx;

procedure test_nakagami_cdfx;
procedure test_nakagami_pdfx;
procedure test_nakagami_invx;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','levy_cdfx');
  {levy_cdf := (m,s,x)->erfc(sqrt(s/(2*(x-m))));}

  a := -1/2;
  b := 2;
  y := levy_cdfx(a, b, -0.25);
...
{$endif}
  ...
  y := levy_invx(a, b, 0.125);
  f := 0.3497888411062026961;
  testrel(6, NE, y, f, cnt,failed);

  y := levy_invx(a, b, 1e-3);
  ...
  {Wolfram Alpha}
  y := levy_invx(1, 2, 0.875);
  f := 81.81901507379606976;
  testrel(15, NE, y, f, cnt,failed);

  y := levy_invx(0, 1, 0.875);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','logseries_cdfx');

  a := 0.5;
  y := logseries_cdfx(a,2);
  f := 0.9016844005556021296;
  testrel(1, NE, y, f, cnt,failed);

  y := logseries_cdfx(a,5);
  f := 1-0.6644352054578320557e-2;
  testrel(2, NE, y, f, cnt,failed);

  y := logseries_cdfx(a,10);
  f := 0.9998812309831727881;
  testrel(3, NE, y, f, cnt,failed);

  y := logseries_cdfx(a,20);
  f := 0.9999999372299021140;
  testrel(4, NE, y, f, cnt,failed);

  y := logseries_cdfx(a,50);
  f := 1-0.24659029992140e-16;
  testrel(5, NE, y, f, cnt,failed);

  {----------------------------}
  a := 0.75;
  y := logseries_cdfx(a,1);
...
{$endif}
  ...
  y := logseries_cdfx(a,1000);
  f := 0.9671296153998283123;
  testrel(16, NE, y, f, cnt,failed);

  y := logseries_cdfx(a,10000);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','logseries_pmfx');

  a := 0.5;
  y := logseries_pmfx(a,2);
  f := 0.1803368801111204259;
  testrel(1, NE, y, f, cnt,failed);

  y := logseries_pmfx(a,5);
  f := 0.9016844005556021296e-2;
  testrel(2, NE, y, f, cnt,failed);

  y := logseries_pmfx(a,10);
  f := 0.1408881875868128327e-3;
  testrel(3, NE, y, f, cnt,failed);

  y := logseries_pmfx(a,20);
  f := 0.6879306034512345349e-7;
  testrel(4, NE, y, f, cnt,failed);

  y := logseries_pmfx(a,50);
  f := 0.2562741203051934149e-16;
  testrel(5, NE, y, f, cnt,failed);

  {----------------------------}
  a := 0.75;
  y := logseries_pmfx(a,1);
  f := 0.5410106403333612778;
  testrel(6, NE, y, f, cnt,failed);
...
const
  NE = 4;
  NE0 = 5;  {Needed for FPC 2.4.2 .. 2.6.4}
  NE1 = 10;
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','wald_cdfx');
  {-------------------------------------}
  a := 1;
  b := 1;

  y := wald_cdfx(a,b,0.0009765625);
  f := 0.2962614197217557007e-223;
  testrel(1, NE0, y, f, cnt,failed);

  y := wald_cdfx(a,b,0.015625);
  f := 0.3356507274327110511e-14;
  testrel(2, NE, y, f, cnt,failed);

  y := wald_cdfx(a,b,0.125);
  f := 0.1206821184832047165e-1;
  testrel(3, NE, y, f, cnt,failed);

  y := wald_cdfx(a,b,0.25);
...
{$endif}
  ...
  a := 2;
  b := 4;
  y := wald_cdfx(a,b,0.00390625);
  f := 0.8041455158779551833e-223;
  testrel(22, NE1, y, f, cnt,failed);

  y := wald_cdfx(a,b,0.015625);
{$ifdef BIT16}
  f := 0.1873692106937386459e-55*0.5;
{$else}
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','wald_invx');
  {-------------------------------------}
  a := 1;
  b := 1;

  y := wald_invx(a,b,1e-10);
  f := 0.2285389221845809094e-1;
  testrel(1, NE, y, f, cnt,failed);

  y := wald_invx(a,b,1e-3);
  f := 0.7921847779047665021e-1;
  testrel(2, NE, y, f, cnt,failed);

  y := wald_invx(a,b,0.125);
  ...
  y := wald_invx(a,b,0.75);
  f := 1.244059755876046385;
  testrel(5, NE, y, f, cnt,failed);

  y := wald_invx(a,b,0.875);
  ...
  y := wald_invx(a,b,0.9921875);      {Needs restart}
  f := 5.325414747748768454;
  testrel(8, NE1, y, f, cnt,failed);

  y := wald_invx(a,b,0.9990234375);   {Needs restart}
...
{$endif}
  ...
  y := wald_invx(2,8,1e-10);          {Needs restart for samll y}
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','wald_pdfx');
  {-------------------------------------}
  a := 1;
  b := 1;

  y := wald_pdfx(a,b,0.0009765625);
  f := 0.1554775506885176576e-217;
  testrel(1, NE, y, f, cnt,failed);

  y := wald_pdfx(a,b,0.015625);
  f := 0.6976830341002265365e-11;
  testrel(2, NE, y, f, cnt,failed);

  y := wald_pdfx(a,b,0.125);
  f := 0.4221999674412029973;
  testrel(3, NE, y, f, cnt,failed);

  y := wald_pdfx(a,b,0.25);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','nakagami_cdfx');

  m := 1/2;
  w := 1;

  y := nakagami_cdfx(m,w,0);
  f := 0;
  testrel(1, NE, y, f, cnt,failed);

  y := nakagami_cdfx(m,w,0.1);
  f := 0.7965567455405796293e-1;
  testrel(2, NE, y, f, cnt,failed);

  y := nakagami_cdfx(m,w,0.5);
  f := 0.38292492254802620728;
  testrel(3, NE, y, f, cnt,failed);

  y := nakagami_cdfx(m,w,1);
  f := 0.68268949213708589717;
  testrel(4, NE, y, f, cnt,failed);

  y := nakagami_cdfx(m,w,1.5);
  f := 0.86638559746228386799;
  testrel(5, NE, y, f, cnt,failed);

  y := nakagami_cdfx(m,w,2);
  f := 1.908999472207283171*0.5;
  testrel(6, NE, y, f, cnt,failed);

  y := nakagami_cdfx(m,w,2.5);
  f := 1.975161338696895459*0.5;
  testrel(7, NE, y, f, cnt,failed);

  y := nakagami_cdfx(m,w,3);
  f := 1.994600407873479622*0.5;
  testrel(8, NE, y, f, cnt,failed);

  y := nakagami_cdfx(m,w,10);
  f := 1.0;
  testrel(9, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','nakagami_invx');

  m := 1/2;
  w := 1;

  y := nakagami_invx(m,w,1/8);
  f := 0.1573106846101706955;
  testrel(1, NE, y, f, cnt,failed);

  y := nakagami_invx(m,w,1/4);
  f := 0.3186393639643751630;
  testrel(2, NE, y, f, cnt,failed);

  y := nakagami_invx(m,w,1/2);
  f := 0.6744897501960817432;
  testrel(3, NE, y, f, cnt,failed);

  y := nakagami_invx(m,w,3/4);
  f := 1.150349380376008178;
  testrel(4, NE1, y, f, cnt,failed);

  y := nakagami_invx(m,w,9/10);
  f := 1.644853626951472715;
  testrel(5, NE, y, f, cnt,failed);

  y := nakagami_invx(m,w,99/100);
...
const
  NE = 2;
  NE1 = 6;  {6 for FPC < 3,  otherwise 4}
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','nakagami_pdfx');

  m := 1/2;
  w := 1;

  y := nakagami_pdfx(m,w,0);
  f := 0;
  testrel(1, NE, y, f, cnt,failed);

  y := nakagami_pdfx(m,w,1e-20);
  f := 0.7978845608028653559;
  testrel(2, NE, y, f, cnt,failed);

  y := nakagami_pdfx(m,w,0.5);
  f := 0.7041306535285989555;
  testrel(3, NE, y, f, cnt,failed);

  y := nakagami_pdfx(m,w,1);
  f := 0.4839414490382866996;
  testrel(4, NE, y, f, cnt,failed);

  y := nakagami_pdfx(m,w,1.5);
  f := 0.2590351913317834552;
  testrel(5, NE, y, f, cnt,failed);

  y := nakagami_pdfx(m,w,2);
  f := 0.1079819330263761039;
  testrel(6, NE, y, f, cnt,failed);

  y := nakagami_pdfx(m,w,2.5);
  f := 0.03505660098713707472;
  testrel(7, NE, y, f, cnt,failed);

  y := nakagami_pdfx(m,w,3);
  f := 0.008863696823876014351;
  testrel(8, NE, y, f, cnt,failed);

  y := nakagami_pdfx(m,w,10);
