{Part 3a of regression test for SPECFUNX unit  (c) 2010-2014  W.Ehrhardt}
...
procedure test_cauchy_cdfx;
procedure test_cauchy_invx;
procedure test_cauchy_pdfx;

procedure test_chi2_cdfx;
procedure test_chi2_invx;
procedure test_chi2_pdfx;

procedure test_chi_distx;

procedure test_exp_cdfx;
procedure test_exp_invx;
procedure test_exp_pdfx;

procedure test_gamma_cdfx;
procedure test_gamma_invx;
procedure test_gamma_pdfx;

procedure test_invgamma_pdfx;
procedure test_invgamma_cdfx;
procedure test_invgamma_invx;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','cauchy_cdfx');

  {statevalf[cdf,cauchy[a,b]](x);}
  a := 5;
  b := 2.0;
  y := cauchy_cdfx(a, b, -70);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','cauchy_invx');

  a := 5;
  b := 2.0;

  y := cauchy_invx(a, b, ldexp(1,-20));
  f := -667539.2144281116036;
  testrel( 1, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, ldexp(1,-10));
  f := -646.8966015954026980;
  testrel( 2, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 0.125);
  f := 0.1715728752538099024;
  testrel( 3, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 0.5);
  f := 5.0;
  testrel( 4, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 0.75);
  f := 7.0;
  testrel( 5, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 0.875);
  f := 9.828427124746190098;
  testrel( 6, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 0.9);
  f := 11.15536707435050681;
  testrel( 7, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 0.99);
  f := 68.64103190754791608;
  testrel( 8, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 0.9990234375);
  f := 656.8966015954026979;
  testrel( 9, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 1-ldexp(1,-20));
  ...
  y := cauchy_invx(a, b, 0.9375);
  f := 1.013669746062924052;
  testrel(17, NE, y, f, cnt,failed);

  y := cauchy_invx(a, b, 0.99);            {.99 not exact}
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','cauchy_pdfx');

  a := 5;
  b := 2.0;
  f := 0.1130964242969588458e-3;
  y := cauchy_pdfx(a, b, -70);
  testrel( 1, NE, y, f, cnt,failed);
  ...
  y := cauchy_pdfx(a, b, 10);
  f := 0.2195240594370970149e-1;
  testrel( 6, NE, y, f, cnt,failed);

  y := cauchy_pdfx(a, b, 1000);
  f := 0.6430314388442978368e-6;
  testrel( 7, NE, y, f, cnt,failed);

  a := -1.5;
  b := 0.5;
  y := cauchy_pdfx(a, b, -100);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','chi2_cdfx');

  {statevalf[cdf,chisquare[nu]](x);}
  nu := 1;

  y := chi2_cdfx(nu, 1e-10);
  f := 0.7978845607895672798e-5;
  testrel( 1, NE, y, f, cnt,failed);

  y := chi2_cdfx(nu, 0.001953125);
  f := 0.3525037386732282597e-1;
  testrel( 2, NE, y, f, cnt,failed);

  y := chi2_cdfx(nu, 0.125);
  ...
  y := chi2_cdfx(1000, 1234.5);
  f := 0.99999951255538488480;
  testrel(50, NE, y, f, cnt,failed);

  y := chi2_cdfx(100000, 101010);
  f := 0.98785020993089822507;
  testrel(51, NE, y, f, cnt,failed);

  y := chi2_cdfx(1200000, 1201234);
  f := 0.78718764724057492688;
  testrel(52, NE, y, f, cnt,failed);

  y := chi2_cdfx(1,MinExtended);
  f := 1.463004216550434528e-2466;
  testrel(53, NE, y, f, cnt,failed);

  y := chi2_cdfx(2,MinExtended);
  f := 0.16810515715560467531e-4000;  {denormal}
  f := f*1e-931;
...
begin
  ...
  y := chi2_invx(nu, 0.001953125);
  f := 0.59921244211799117736e-5;
  testrel( 2, NE, y, f, cnt,failed);

  y := chi2_invx(nu, 0.03125);
  f := 0.15347656753460162514e-2;
  testrel( 3, NE, y, f, cnt,failed);

  y := chi2_invx(nu, 0.125);
  ...
  y := chi2_invx(nu, 0.9990234375);
  f := 10.871483958875362877;
  testrel(11, NE, y, f, cnt,failed);

  y := chi2_invx(nu, 1-ldexp(1,-20)); {0.99999904632568359375}
  ...
  y := chi2_invx(100, 0.9921875);
  f := 137.39138405740706884;
  testrel(49, NE, y, f, cnt,failed);

  {extreme cases}
  y := chi2_invx(10000, 0.9921875);
  f := 10345.121945562315395;
  testrele(50, 1e-17, y, f, cnt,failed);

  y := chi2_invx(120000, 0.9921875);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','chi2_pdfx');

  {statevalf[pdf,chisquare[nu]](x);}

  nu := 1;

  y := chi2_pdfx(nu, 1e-10);
  f := 39894.228038148556392;
  testrel( 1, NE, y, f, cnt,failed);

  y := chi2_pdfx(nu, 0.001953125);
  f := 9.0182221775452522375;
  testrel( 2, NE, y, f, cnt,failed);

  y := chi2_pdfx(nu, 0.125);
  ...
  y := chi2_pdfx(nu, 100.0);
  f := 0.76945986267064193463e-23;
  testrel(11, NE, y, f, cnt,failed);

  y := chi2_pdfx(nu, 1e4);
  f := 0.13443134677817230434e-2173;
  testrel(12, NE, y, f, cnt,failed);
  ...
  y := chi2_pdfx(nu, 1e4);
  f := 0.16848470741544587573e-2171;
  testrel(24, NE, y, f, cnt,failed);
  ...
  y := chi2_pdfx(nu, 1e4);
  f := 0.44810448926057434779e-2166;
  testrel(36, NE, y, f, cnt,failed);
...
{$endif}
  ...
  y := chi2_pdfx(nu, 1e4);
  f := 0.90683337238423921412e-2144;
  testrel(48, NE, y, f, cnt,failed);
  ...
  y := chi2_pdfx(1000, 1001);
  f := 0.89079979170919584589e-2;
  testrel(50, NE, y, f, cnt,failed);

  y := chi2_pdfx(100000, 100000);
  f := 0.89206057130752775749e-3;
  testrel(51, NE, y, f, cnt,failed);

  y := chi2_pdfx(1200000, 1200000);
  f := 0.25751609891599905370e-3;
  testrel(52, NE, y, f, cnt,failed);

  y := chi2_pdfx(1,1e-20);
  f := 3989422804.014326779;
  testrel(53, NE, y, f, cnt,failed);
  ...
  y := chi2_pdfx(3,1e-20);
  f := 0.3989422804014326779e-10;
  testrel(55, NE, y, f, cnt,failed);

  y := chi2_pdfx(440,1e-20);
  f := 0.5716814397580440411e-4865;
  testrel(56, 100, y, f, cnt,failed);   {!!!!!}

  y := chi2_pdfx(448,1e-20);
  f := 0;
  testrel(57, NE, y, f, cnt,failed);

  y := chi2_pdfx(3,MinExtended);
  f := 0.7315021082752172641e-2466;
  testrel(58, NE, y, f, cnt,failed);

  y := chi2_pdfx(4,MinExtended);
  f := 0.8405257857780233766e-4000;     {denormal}
  f := f*1e-932;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','chi_pdfx/cdfx/invx');
...
{$endif}
  ...
  y := chi_pdfx(3,100);
  f := 2.688626935563446087e-2168;
  testrel(22, NE+1, y, f, cnt,failed);  {+1 for FPC242..264}
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','exp_cdfx');

  {statevalf[cdf,exponential[alpha, a]](x);}
  a := 5;
  alpha := 2.0;
  y := exp_cdfx(a, alpha, 5);
  f := 0.0;
  testrel(1, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 5.015625);
  f := 0.3076676552365591815e-1;
  testrel(2, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 5.25);
  f := 0.3934693402873665764;
  testrel(3, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 5.5);
  f := 0.6321205588285576784;
  testrel(4, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 6);
  f := 0.8646647167633873081;
  testrel(5, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 8);
  f := 0.9975212478233336416;
  testrel(6, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 10);
  f := 0.9999546000702375152;
  testrel(7, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 20);
  f := 0.9999999999999064238;
  testrel(8, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 28);
  f := 1.0;
  testrel(9, NE, y, f, cnt,failed);

  a := -1.5;
  alpha := 0.5;
  y := exp_cdfx(a, alpha, -2);
  f := 0.0;
  testrel(10, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, -1.5);
  f := 0.0;
  testrel(11, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, -1);
  f := 0.2211992169285951318;
  testrel(12, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, -0.5);
  f := 0.3934693402873665764;
  testrel(13, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 0);
  ...
  y := exp_cdfx(a, alpha, 50);
  f := 0.9999999999934397998;
  testrel(18, NE, y, f, cnt,failed);

  y := exp_cdfx(a, alpha, 90);
  f := 1.0;
  testrel(19, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','exp_pdfx');

  {statevalf[pdf,exponential[alpha,a]](x);}
  a := 5;
  alpha := 2.0;
  y := exp_pdfx(a, alpha, 2.5);
  f := 0;
  testrel(1, NE, y, f, cnt,failed);

  y := exp_pdfx(a, alpha, 5);
  f := 2.0;
  testrel(2, NE, y, f, cnt,failed);

  y := exp_pdfx(a, alpha, 7.5);
  f := 0.1347589399817093419e-1;
  testrel(3, NE, y, f, cnt,failed);

  y := exp_pdfx(a, alpha, 10);
  f := 0.9079985952496970307e-4;
  testrel(4, NE, y, f, cnt,failed);

  y := exp_pdfx(a, alpha, 100);
  ...
  a := -1.5;
  alpha := 0.5;
  y := exp_pdfx(a, alpha, -2);
  f := 0;
  testrel(6, NE, y, f, cnt,failed);

  y := exp_pdfx(a, alpha, -1.5);
  f := 0.5;
  testrel(7, NE, y, f, cnt,failed);

  y := exp_pdfx(a, alpha, -1);
  f := 0.3894003915357024341;
  testrel(8, NE, y, f, cnt,failed);

  y := exp_pdfx(a, alpha, 0);
  ...
  y := exp_pdfx(a, alpha, 25);
  f := 0.88017315607808464929e-6;
  testrel(13, NE, y, f, cnt,failed);

  y := exp_pdfx(a, alpha, 1000);
...
begin
  ...
  writeln('Function: ','exp_invx');
  a := 5;
  alpha := 2.0;
  y := exp_invx(a, alpha, ldexp(1,-20));
  f := 5.000000476837385577;
  testrel(1, NE, y, f, cnt,failed);

  y := exp_invx(a, alpha, 0.125);
  f := 5.066765696312261312;
  testrel(2, NE, y, f, cnt,failed);

  y := exp_invx(a, alpha, 0.5);
  f := 5.346573590279972655;
  testrel(3, NE, y, f, cnt,failed);

  y := exp_invx(a, alpha, 0.75);
  f := 5.693147180559945309;
  testrel(4, NE, y, f, cnt,failed);

  y := exp_invx(a, alpha, 0.875);
  f := 6.039720770839917964;
  testrel(5, NE, y, f, cnt,failed);

  y := exp_invx(a, alpha, 0.9);
  f := 6.151292546497022842;
  testrel(6, NE, y, f, cnt,failed);

  y := exp_invx(a, alpha, 0.99);
  f := 7.302585092994045684;
  testrel(7, NE, y, f, cnt,failed);

  y := exp_invx(a, alpha, 0.9990234375);
  f := 8.465735902799726547;
  testrel(8, NE, y, f, cnt,failed);

  y := exp_invx(a, alpha, 1-ldexp(1,-20));
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','gamma_pdfx');

  {statevalf[pdf,gamma[a,b](x);}

  a := 1.0; b := 2.0;
  y := gamma_pdfx(a,b, 1e-10);
  f := 0.499999999975;
  testrel( 1, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 0.001953125);
  f := 0.49951195709098783112;
  testrel( 2, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 0.125);
  f := 0.46970653140673789306;
  testrel( 3, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 0.5);
  f := 0.38940039153570243413;
  testrel( 4, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 1.0);
  f := 0.30326532985631671180;
  testrel( 5, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 2.0);
  f := 0.18393972058572116080;
  testrel( 6, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 4.0);
  f := 0.67667641618306345945e-1;
  testrel( 7, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 10.0);
  f := 0.33689734995427335483e-2;
  testrel( 8, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 100.0);
  f := 0.96437492398195889150e-22;
  testrel(09, NE, y, f, cnt,failed);

  y := gamma_pdfx(a,b, 1e4);
  f := 0.16848470741544587573e-2171;
  testrel(10, NE, y, f, cnt,failed);
  ...
  y := gamma_pdfx(a,b, 1e4);
  f := 0.84242353707722937863e-2168;
  testrel(20, NE, y, f, cnt,failed);
  ...
  y := gamma_pdfx(a,b, 1e4);
  f := 0.21060588426930734466e-2164;
  testrel(30, NE, y, f, cnt,failed);
  ...
  y := gamma_pdfx(a,b, 5000.0);
  f := 0.56323604430294449331e-4315;
  testrel(40, NE, y, f, cnt,failed);
  ...
  y := gamma_pdfx(a,b, 1e4);
  f := 0.47311827721447337438e-4328;
  testrel(50, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','gamma_cdfx');

  {statevalf[cdf,gamma[a,b](x);}

  a := 1.0; b := 2.0;
  y := gamma_cdfx(a,b, 1e-10);
  f := 0.49999999998750000000e-10;
  testrel( 1, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 0.001953125);
  f := 0.97608581802433776529e-3;
  testrel( 2, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 0.125);
  f := 0.60586937186524213880e-1;
  testrel( 3, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 0.5);
  f := 0.22119921692859513175;
  testrel( 4, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 1.0);
  f := 0.39346934028736657640;
  testrel( 5, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 2.0);
  f := 0.63212055882855767840;
  testrel( 6, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 4.0);
  f := 0.86466471676338730811;
  testrel( 7, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 10.0);
  f := 0.99326205300091453290;
  testrel( 8, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 100.0);
  f := 1.0;
  testrel(09, NE, y, f, cnt,failed);

  y := gamma_cdfx(a,b, 0.25);
  f := 0.11750309741540459714;
  testrel(10, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','gamma_invx');

  {statevalf[icdf,gamma[a,b](p);}

  a := 1.0; b := 2.0;

  y := gamma_invx(a, b, 1e-6);
  f := 0.20000010000006666672e-5;
  testrel( 1, NE, y, f, cnt,failed);

  y := gamma_invx(a, b, 0.125);
  f := 0.26706278524904524629;
  testrel( 2, NE, y, f, cnt,failed);

  y := gamma_invx(a, b, 0.25);
  f := 0.57536414490356185488;
  testrel( 3, NE, y, f, cnt,failed);

  y := gamma_invx(a, b, 0.5);
  f := 1.3862943611198906188;
  testrel( 4, NE, y, f, cnt,failed);

  y := gamma_invx(a, b, 0.75);
  f := 2.7725887222397812377;
  testrel( 5, NE, y, f, cnt,failed);

  y := gamma_invx(a, b, 0.9);
  f := 4.6051701859880913680;
  testrel( 6, NE, y, f, cnt,failed);

  y := gamma_invx(a, b, 0.99);
  f := 9.2103403719761827361;
  testrel( 7, NE, y, f, cnt,failed);

  y := gamma_invx(a, b, 0.9990234375);
  f := 13.862943611198906188;
  testrel( 8, NE, y, f, cnt,failed);

  y := gamma_invx(a, b, 1-ldexp(1,-20));
  f := 27.725887222397812377;
  testrel( 9, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','invgamma_pdfx');

  {Maple:  ig_pdf := (a,b,x) -> b^a*x^(-a-1)*exp(-b/x)/GAMMA(a);}
  a := 1.0;  b := 2.0;
  y := invgamma_pdfx(a,b,0.03125);
  f := 0.3284604703843610323e-24;
  testrel(1, NE, y, f, cnt,failed);

  y := invgamma_pdfx(a,b,0.125);
  f := 0.1440450236406516666e-4;
  testrel(2, NE, y, f, cnt,failed);

  y := invgamma_pdfx(a,b,1);
  f := 0.2706705664732253838;
  testrel(3, NE, y, f, cnt,failed);

  y := invgamma_pdfx(a,b,10.0);
  f := 0.1637461506155963717e-1;
  testrel(4, NE, y, f, cnt,failed);

  y := invgamma_pdfx(a,b,100.0);
  f := 0.1960397346613510604e-3;
  testrel(5, NE, y, f, cnt,failed);

  y := invgamma_pdfx(a,b,1000);
  ...
  y := invgamma_pdfx(a,b,100.0);
{$ifdef BIT16}
  f := 0.1980099667498336107e-5*0.5;
{$else}
...
{$endif}
  ...
  y := invgamma_pdfx(a,b,5.0);
  f := 0.4534866089799165245e-1;
  testrel(16, NE, y, f, cnt,failed);

  y := invgamma_pdfx(a,b,50.0);
  f := 0.2946161122426586462e-2;
  testrel(17, NE, y, f, cnt,failed);

  y := invgamma_pdfx(a,b,5000.0);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','invgamma_cdfx');

  {Maple: ig_cdf := (a,b,x) -> GAMMA(a,b/x)/GAMMA(a); }
  a := 1.0; b := 2.0;
  y := invgamma_cdfx(a,b,0.03125);
  f := 0.1603810890548637853e-27;
  testrel(1, NE, y, f, cnt,failed);

  y := invgamma_cdfx(a,b,0.125);
  f := 0.1125351747192591145e-6;
  testrel(2, NE, y, f, cnt,failed);

  y := invgamma_cdfx(a,b,1);
  f := 0.1353352832366126919;
  testrel(3, NE, y, f, cnt,failed);

  y := invgamma_cdfx(a,b,10.0);
  f := 0.8187307530779818587;
  testrel(4, NE, y, f, cnt,failed);

  y := invgamma_cdfx(a,b,100.0);
  f := 0.9801986733067553022;
  testrel(5, NE, y, f, cnt,failed);

  y := invgamma_cdfx(a,b,1000);
{$ifdef BIT16}
  f := 3.992007994669332267*0.25;
{$else}
...
{$endif}
  ...
  y := invgamma_cdfx(a,b,5.0);
  f := 0.2059032107320683089;
  testrel(16, NE, y, f, cnt,failed);

  y := invgamma_cdfx(a,b,50.0);
  f := 0.6891565167793516665;
  testrel(17, NE, y, f, cnt,failed);

  y := invgamma_cdfx(a,b,250.0);
  f := 0.8580276569875211913;
  testrel(18, NE, y, f, cnt,failed);

  y := invgamma_cdfx(a,b,5000.0);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','invgamma_invx');

  {Maple: ig_inv := (a,b,y) -> b/stats[statevalf,icdf,gamma[a,1]](1-y);}
  a := 1.0; b := 2.0;
  y := invgamma_invx(a,b,1e-6);
  f := 0.1447648273010839426;
  testrel(1, NE, y, f, cnt,failed);

  y := invgamma_invx(a,b,0.125);
  f := 0.961796693925975605;
  testrel(2, NE, y, f, cnt,failed);

  y := invgamma_invx(a,b,0.5);
  f := 2.885390081777926815;
  testrel(3, NE, y, f, cnt,failed);

  y := invgamma_invx(a,b,0.75);
  f := 6.952118993564413821;
  testrel(4, NE, y, f, cnt,failed);

  y := invgamma_invx(a,b,0.9);
  f := 18.98244316205980605;
  testrel(5, NE, y, f, cnt,failed);

  y := invgamma_invx(a,b,0.9990234375);
