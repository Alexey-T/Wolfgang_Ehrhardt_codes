{Part 9a of regression test for SPECFUNX unit  (c) 2013-2014  W.Ehrhardt}
...
procedure test_hyperg_ux;
procedure test_whittakerx;
procedure test_hyperg_0F1x;
procedure test_hyperg_0F1rx;

procedure test_cylinderdx;
procedure test_cylinderux;
procedure test_cylindervx;
procedure test_hermitehx;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','hyperg_ux');

  y := hyperg_ux(2.5, 1000.5, 300);
  f := 0.1425867116494189890e218;
  testrel(1, NE1, y, f, cnt,failed);

  y := hyperg_ux(-2.5, 29000.5, 24000);
...
*)
  ...
  y := hyperg_ux(0.0001, 10, 5);
  f := 1.0006265020064596364;
  testrel(164, NE, y, f, cnt,failed);

  y := hyperg_ux(0.0001, 10, 10);
  f := 0.9999244381454633265;
  testrel(165, NE, y, f, cnt,failed);

  y := hyperg_ux(8, 10.5, 10);
  f := 2.4370135607662056809e-8;
  testrel(166, NE1, y, f, cnt,failed);

  y := hyperg_ux(10, -2.5, 10);
  f := 6.734690720346560349e-14;
  testrel(167, NE, y, f, cnt,failed);

  y := hyperg_ux(10, 2.5, 10);
  f := 6.787780794037971638e-13;
  testrel(168, NE1, y, f, cnt,failed);

  y := hyperg_ux(10, 2.5, 50);
  f := 2.409872007659608713e-18;
  testrel(169, NE, y, f, cnt,failed);

  y := hyperg_ux(-10.5, 1.1, 99);
  f := 2.546328328942063558e20;
  testrel(170, NE, y, f, cnt,failed);

  y := hyperg_ux(-50.5, 100.1, 10);
  f := -5.5740216266953697786e126;
  testrel(171, NE2, y, f, cnt,failed);

  y := hyperg_ux(-50.5, 100.1, 40);
  f := 5.937463786613929435e91;
  testrel(172, NE, y, f, cnt,failed);

  {-----------------------------------}
  {NEW problem cases:                 }

  {Temme underflow and dchu very inaccurate with extended}
  y := hyperg_ux(1, -2000, 1);
  f := 0.4995003748125624532e-3;
  testrel(173, 8000 , y, f, cnt,failed);    {!!!!!!!!!!!!!!!!}


  {-----------------------------------}
  {Negative x}
  y := hyperg_ux(-3,2.5,-1.5);
  f := -144.0;
  testrel(174, NEN, y, f, cnt,failed);

  y := hyperg_ux(-4,-3,-4.5);
  f := 410.0625;
  testrel(175, NEN, y, f, cnt,failed);

  y := hyperg_ux(-4,-4,-15);
  f := 39489.0;
  testrel(176, NEN, y, f, cnt,failed);

  y := hyperg_ux(-4,-4.5,-15);
  f := 34709.0625;
  testrel(177, NEN, y, f, cnt,failed);

  y := hyperg_ux(-4, 4.5,-15);
  f := 234981.5625;
  testrel(178, NEN, y, f, cnt,failed);

  {GSL}
  y := hyperg_ux(3,4,-2);
  f := -0.125;
  testrel(179, NEN, y, f, cnt,failed);

  y := hyperg_ux(3,6,-0.5);
  f := -296.0;
  testrel(180, NEN, y, f, cnt,failed);

  y := hyperg_ux(1, 2, -0.1);
  f := -10.0;
  testrel(181, NEN, y, f, cnt,failed);

  y := hyperg_ux(-5, -2, -0.1);
  f := -0.02101;
  testrel(182, NEN, y, f, cnt,failed);

  y := hyperg_ux(-2, 0, -0.5);
  f := 1.25;
  testrel(183, NEN, y, f, cnt,failed);

  y := hyperg_ux(-8, 1.5, -0.5);
  f := 827341.0625;
  testrel(184, NEN, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','hyperg_0F1x');

  y := hyperg_0F1x(0.0, 0.0);
  f := 1.0;
  testrel(1, NE, y, f, cnt,failed);
  ...
  y := hyperg_0F1x(-1000.5, -1e8);
  f := 7.503596135205610838e1434;
  testrel(33, NE, y, f, cnt,failed);

  y := hyperg_0F1x(-2000.5, 1e6);
  f := -0.8821614028377686353e728;
...
begin
  ...
  y := hyperg_0F1rx(-2.5, 0);
  f := -1.057855469152043038;
  testrel(3, NE, y, f, cnt,failed);

  y := hyperg_0F1rx(-2.5, -0.0625);
  ...
  y := hyperg_0F1rx(-10, -30);
  f := -0.2627191378120922238e8;
  testrel(10, NE, y, f, cnt,failed);

  y := hyperg_0F1rx(170, 1);
  f := 0.2356251022619302925e-304;
  testrel(11, NE, y, f, cnt,failed);

  y := hyperg_0F1rx(-160.125, 1);
  f := -0.1076810946443946249e285;
{$ifdef FPC}
  testrel(12, NE+1, y, f, cnt,failed);  {FPC271 only}
{$else}
  testrel(12, NE, y, f, cnt,failed);
{$endif}

  y := hyperg_0F1rx(170, 2500);
  f := 0.3227168427177659957e-298;
  testrel(13, NE, y, f, cnt,failed);

  y := hyperg_0F1rx(-160, -120);        {DAMath: power overflow}
  f := -0.3511263902718893603e48;
  testrel(14, NE1, y, f, cnt,failed);

  y := hyperg_0F1rx(-160.125, -0.25);   {DAMath: Jv overflow}
  f := -0.1085249736003132468e285;
  testrel(15, NE, y, f, cnt,failed);

  y := hyperg_0F1rx(-160.125, 0.1875);  {DAMath: Iv overflow}
  f := -0.1082288616245840387e285;
  testrel(16, NE2, y, f, cnt,failed);

  y := hyperg_0F1rx(-1700, -700);
  f := -0.4222215366957806816e81;
  testrel(17, NE3, y, f, cnt,failed);

  y := hyperg_0F1rx(-1701, -800);        {power overflow}
  f := 0.8252189997101624829e179;
  testrel(18, NE2, y, f, cnt,failed);

  {extended}
  y := hyperg_0F1rx(-1800, -30);
  f := -0.1759736908855494601e-2422;
  testrel(19, NE3, y, f, cnt,failed);

  y := hyperg_0F1rx(1750, 1);
  f := 0.8344776461943994681e-4914;
  testrel(20, NE1, y, f, cnt,failed);

  y := hyperg_0F1rx(1750, 1e5);
  f := 2.236964032427701514e-4890;
  testrel(21, NE3, y, f, cnt,failed);

  y := hyperg_0F1rx(-1700.125, 1);
  f := -0.9249988786218418937e4755;
  testrel(22, NE1, y, f, cnt,failed);

  y := hyperg_0F1rx(-1700.125, -0.25);  {Jv overflow}
  f := -0.9256792248389749852e4755;
  testrel(23, NE4, y, f, cnt,failed);   {!!!!!!!!!!!!!!!!!!!!!}

  y := hyperg_0F1rx(-1600.5, 0.1875);   {Iv overflow}
  f := -0.6713279926215629033e4435;
...
begin
  ...
  testrel(2, NE2, y, f, cnt,failed);  {???, cancellation D(.,1) - 1F1 term}
  ...
  y := CylinderDx(20.5, -20);   {N[ParabolicCylinderD[20.5, -20],30]}
  ...
  y := CylinderDx(4.5, -30);  {Double: Overflow for x=-50}
...
begin
  ...
  y := HermiteHx(5.5,0.125);
  f := -22.60805184613228380;
  testrel(14, NE, y, f, cnt,failed);

  y := HermiteHx(5.5,1e-9);
...
begin
  ...
  y := CylinderVx(200.0,x);
  f := 0.1165404358441727419e196;
  testrel(13, NE, y, f, cnt,failed);

  y := CylinderVx(200.5,x);
  f := 0.4498864031728626495e196;
  testrel(14, NE1, y, f, cnt,failed); {FPC}
  ...
  testrel(27, NE1, y, f, cnt,failed);  {FPC}

  y := CylinderVx(200.5,x);
  f := 0.5038731252253890156e266;
  testrel(28, NE, y, f, cnt,failed);

  y := CylinderVx(1.5,1.5);
  f := 2.100496521230687000;
  testrel(29, NE, y, f, cnt,failed);
  ...
  y := CylinderVx(0.5,1e-5);
  f := 0.7978845608228124699;
  testrel(31, NE, y, f, cnt,failed);

  y := CylinderVx(0.5,1.5);
  f := 1.400331014153791334;
  testrel(32, NE, y, f, cnt,failed);
  ...
  {----- negative x -----}
  y := CylinderVx(5/2, -2.0);
  f := 10.844375514192275467;
  testrel(46, NE, y, f, cnt,failed);

  y := CylinderVx(3/2, -2.0);
  f := -4.337750205676910186;
  testrel(47, NE, y, f, cnt,failed);

  y := CylinderVx(1/2, -2.0);
  ...
  testrel(62, NE1, y, f, cnt,failed);

  y := CylinderVx(-2.5, -2.0);
  f := 0.8679380128080086810e-1;
  testrel(63, NE, y, f, cnt,failed);

  y := CylinderVx(-2.5, -0.0625);
  f := 0.4978666915953578549e-1;
  testrel(64, NE, y, f, cnt,failed);

  y := CylinderVx(-2.5, 4.0);
  f := 1.218554345187601585;
  testrel(65, NE, y, f, cnt,failed);

  y := CylinderVx(-2.5, -10.0);
  f := -0.6118449404325429709e8;
  testrel(66, NE, y, f, cnt,failed);

  y := CylinderVx(-20, 0.25);
  ...
  {V(-12.5,x) is odd}
  y := CylinderVx(-12.5, 1.5);
  f := -0.1867855278694266291e-4;
  testrel(71, NE, y, f, cnt,failed);

  y := CylinderVx(-12.5, -2.5);
  ...
  y := CylinderVx(-9.5, -12.5);
  f := 1156953.0123654599948;
  testrel(74, NE2, y, f, cnt,failed);

  y := CylinderVx(-150, -2.0);
  f := -0.3717710476062361776e-131;
  testrel(75, NE, y, f, cnt,failed);

  y := CylinderVx(-150.5, -4.0);
  ...
  testrel(77, NE3, y, f, cnt,failed);

  y := CylinderVx(-200.5, 2.0);
  f := -0.1841171876846235625e-189;
  testrel(78, NE3, y, f, cnt,failed);

  {---- extended only -----}
  y := CylinderVx(1500, -2.0);
  f := 4.579967857537660088e2021;  {alpha}
  testrel(79, NE2, y, f, cnt,failed);

  y := CylinderVx(2000, -2.0);
  f := 6.514870233501521603e2826;  {alpha} {exp(lnGamma...)}
  testrel(80, NE4, y, f, cnt,failed);

  y := CylinderVx(0.5,100);
  f := 0.4346551398877873846e1086;
  testrel(81, NE, y, f, cnt,failed);

  y := CylinderVx(0.5,200);
  f := 7.0268242920537301479e4342;
  testrel(82, NE, y, f, cnt,failed);

  y := CylinderVx(0,100);
  f := 0.4346714430225352993e1085;
  testrel(83, NE, y, f, cnt,failed);

  y := CylinderVx(0,200);
  f := 0.4968761691369382596e4342;
  testrel(84, NE, y, f, cnt,failed);

  y := CylinderVx(1,100);
  f := 0.4346497061890435280e1087;
  testrel(85, NE, y, f, cnt,failed);

  y := CylinderVx(1,200);
  f := 0.19874798318075601957e4345*0.5;
  testrel(86, NE, y, f, cnt,failed);

  y := CylinderVx(3200,0);
  f := 0.17535938227701613346e4913;
  testrel(87, NE1, y, f, cnt,failed);       {FPC}

  y := CylinderVx(-1500, -2.5);
  f := -0.3988929508644486006e-2057;
  testrel(88, NE3, y, f, cnt,failed);

  y := CylinderVx(-1500.5, 2.5);
  f := 0.3506446734115536115e-2058;
  testrel(89, NE3, y, f, cnt,failed);

  {calls pcfv_gen because |a| > MAXGAM}
  y := CylinderVx(-3000, 5);
  f := -0.4139526197939949744e-4565;
