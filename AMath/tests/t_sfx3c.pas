{Part 3c of regression test for SPECFUNX unit  (c) 2011  W.Ehrhardt}
...
procedure test_binomial_pmfx;
procedure test_binomial_cdfx;

procedure test_poisson_pmfx;
procedure test_poisson_cdfx;

procedure test_negbinom_pmfx;
procedure test_negbinom_cdfx;

procedure test_hypergeo_pmfx;
procedure test_hypergeo_cdfx;

procedure test_rayleigh_cdfx;
procedure test_rayleigh_pdfx;
procedure test_rayleigh_invx;

procedure test_maxwell_cdfx;
procedure test_maxwell_pdfx;
procedure test_maxwell_invx;

procedure test_evt1_cdfx;
procedure test_evt1_pdfx;
procedure test_evt1_invx;

procedure test_kumaraswamy_pdfx;
procedure test_kumaraswamy_cdfx;
procedure test_kumaraswamy_invx;

procedure test_kolmogorov_cdfx;
procedure test_kolmogorov_invx;
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','binomial_pmfx');
  ...
  y := binomial_pmfx(p,n,-1);
  f := 0.0;
  testrel(1, NE, y, f, cnt,failed);

  y := binomial_pmfx(p,n,0);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','binomial_cdfx');

  {statevalf[dcdf,binomiald[n,p]](k);}
  p := 0.75;
  n := 40;

  y := binomial_cdfx(p,n,-1);
  f := 0.0;
  testrel(1, NE, y, f, cnt,failed);

  y := binomial_cdfx(p,n,0);
  f := 0.8271806125530276749e-24;
  testrel(2, NE, y, f, cnt,failed);
...
begin
  ...
  y := poisson_pmfx(mu,k);
  testrel(18, 250, y, f, cnt,failed);   {!!!????}

  y := poisson_pmfx(1000000,1000000-5000);
  f := 0.14596440994146676390e-8;
  testrel(19, NE, y, f, cnt,failed);

  y := poisson_pmfx(1000000,1000000-1);
  f := 0.39894224715624402976e-3;
  testrel(20, NE, y, f, cnt,failed);

  y := poisson_pmfx(1000000,1000000);
...
begin
  ...
  y := poisson_cdfx(1000000,1000000-5000);
  f := 0.2814820383896531442e-6;
  testrel(19, NE2, y, f, cnt,failed);

  y := poisson_cdfx(1000000,1000000-1);
  f := 0.4998670192391274088;
  testrel(20, NE2, y, f, cnt,failed);

  y := poisson_cdfx(1000000,1000000);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','negbinom_pmfx');
  ...
  y := negbinom_pmfx(p,r,60);
  f := 0.2566661064111152397e-1;
  testrel(15, NE, y, f, cnt,failed);

  y := negbinom_pmfx(p,r,70);
  f := 0.3365662227431640229e-1;
  testrel(16, NE2, y, f, cnt,failed);

  y := negbinom_pmfx(p,r,80);
  ...
  y := negbinom_pmfx(p,r,2);
  f := 0.46875e-1;
  testrel(24, NE, y, f, cnt,failed);

  y := negbinom_pmfx(p,r,4);
  f := 0.29296875e-2;
  testrel(25, NE, y, f, cnt,failed);

  y := negbinom_pmfx(p,r,6);
  f := 0.18310546875e-3;
  testrel(26, NE, y, f, cnt,failed);

  y := negbinom_pmfx(p,r,8);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','negbinom_cdfx');
  ...
  y := negbinom_cdfx(p,r,-1);
  f := 0.0;
  testrel(1, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,0);
  f := 0.298023223876953125e-7;
  testrel(2, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,5);
  f := 0.5863341687017964432e-4;
  testrel(3, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,10);
  f := 0.1896295783055346033e-2;
  testrel(4, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,20);
  f := 0.6270215545434477773e-1;
  testrel(5, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,30);
  f := 0.3053534137005089327;
  testrel(6, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,40);
  f := 0.6309566199205073919;
  testrel(7, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,50);
  f := 0.8561377403191092259;
  testrel(8, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,100);
  f := 0.9999310806969732651;
  testrel(9, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,250);
  ...
  y := negbinom_cdfx(p,r,60);
  f := 0.2150104419876710213;
  testrel(15, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,70);
  f := 0.5336566222743164023;
  testrel(16, 2*NE, y, f, cnt,failed);         {2* for AMD}

  y := negbinom_cdfx(p,r,80);
  f := 0.8154192651691056915;
  testrel(17, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,90);
  ...
  y := negbinom_cdfx(p,r,2);
  f := 0.984375;
  testrel(24, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,4);
  f := 0.9990234375;
  testrel(25, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,6);
  f := 0.99993896484375;
  testrel(26, NE, y, f, cnt,failed);

  y := negbinom_cdfx(p,r,8);
...
begin
  ...
  cnt := 0;
  failed := 0;
  writeln('Function: ','hypergeo_pmfx');
  ...
  y := hypergeo_pmfx(n1,n2,n,0);
  f := 0.1458070693019822597e-9;
  testrel(1, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,5);
  f := 0.3329662375051472972e-4;
  testrel(2, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,10);
  f := 0.1140119535468728544e-1;
  testrel(3, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,15);
  f := 0.1145256680045404870;
  testrel(4, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,16);
  f := 0.1282808216541822096;
  testrel(5, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,17);
  f := 0.1290489702868419833;
  testrel(6, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,20);
  f := 0.7054335486599437926e-1;
  testrel(7, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,30);
  ...
  y := hypergeo_pmfx(n1,n2,n,33);
  f := 0.1290489702868419833;
  testrel(13, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,35);
  f := 0.1145256680045404870;
  testrel(14, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,40);
  f := 0.1140119535468728544e-1;
  testrel(15, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,50);
  f := 0.1458070693019822597e-9;
  testrel(16, NE, y, f, cnt,failed);
  ...
  y := hypergeo_pmfx(n1,n2,n,21);
  f := 0.1096665608218504728;
  testrel(21, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,22);
  f := 0.1284352525327525682;
  testrel(22, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,23);
  f := 0.1345232262552923270;
  testrel(23, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,24);
  f := 0.1258645484459498172;
  testrel(24, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(n1,n2,n,25);
  ...
  y := hypergeo_pmfx(2598,600,2284,1692);
  f := 0.1612952941179119320e-83;
  testrel(28, NE3, y, f, cnt,failed);

  y := hypergeo_pmfx(2598,600,2284,1812);
  f := 0.2076831972632489092e-5;
  testrel(29, NE2, y, f, cnt,failed);

  y := hypergeo_pmfx(2598,600,2284,1848);
  f := 0.3043400366814929190e-1;
  testrel(30, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(2598,600,2284,1870);
  f := 0.1379967106042925267e-1;
  testrel(31, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(30197,719,24162,23557);
  f := 0.9957825121836800402e-5;
  testrel(32, NE3, y, f, cnt,failed);

  y := hypergeo_pmfx(30197,719,24162,23574);
  f := 0.2023261955389159984e-2;
  testrel(33, NE, y, f, cnt,failed);

  y := hypergeo_pmfx(244,1029923,523360,179);
  f := 0.3252851387472271981e-12;
  testrel(34, 2*NE3, y, f, cnt,failed);        {2* for AMD}

  y := hypergeo_pmfx(244,2063033,523360,7);
  f := 0.4856769030435700293e-21;
  testrel(35, 2*NE3, y, f, cnt,failed);        {2* for AMD}

  y := hypergeo_pmfx(312,1029855,523360,149);
...
begin
  ...
  cnt := 0;
  failed := 0;
  writeln('Function: ','hypergeo_cdfx');
  ...
  y := hypergeo_cdfx(n1,n2,n,0);
  f := 0.1458070693019822597e-9;
  testrel(1, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,5);
  f := 0.4001169492144623170e-4;
  testrel(2, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,10);
  f := 0.1886559586767971603e-1;
  testrel(3, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,15);
  f := 0.3548034277806054734;
  testrel(4, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,16);
  f := 0.4830842494347876830;
  testrel(5, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,17);
  f := 0.6121332197216296663;
  testrel(6, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,20);
  f := 0.8950818719479068637;
  testrel(7, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,30);
  ...
  y := hypergeo_cdfx(n1,n2,n,33);
  f := 0.5169157505652123170;
  testrel(13, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,35);
  f := 0.7597222402239350136;
  testrel(14, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,40);
  f := 0.9925355994870075694;
  testrel(15, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,45);
  ...
  y := hypergeo_cdfx(n1,n2,n,21);
  f := 0.3209331753486768224;
  testrel(21, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,22);
  f := 0.4493684278814293906;
  testrel(22, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,23);
  f := 0.5838916541367217176;
  testrel(23, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,24);
  f := 0.7097562025826715348;
  testrel(24, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(n1,n2,n,25);
  ...
  {Larger values from Boost test data}
  y := hypergeo_cdfx(2598,600,2284,1692);
  f := 1.654453685667568298e-84;
  testrel(28, NE3, y, f, cnt,failed);

  y := hypergeo_cdfx(2598,600,2284,1812);
  f := 5.354493333876393888e-6;
  testrel(29, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(2598,600,2284,1848);
  f := 0.2426930943591920128;
  testrel(30, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(2598,600,2284,1870);
  f := 0.9332289351942023402;
  testrel(31, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(30197,719,24162,23557);
  f := 0.28832290318908713238e-4;
  testrel(33, NE3, y, f, cnt,failed);

  y := hypergeo_cdfx(30197,719,24162,23574);
  f := 0.0087632967578790970761218327;
  testrel(34, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(244,1029923,523360,179);
  f := 0.9999999999998102642;
  testrel(35, NE, y, f, cnt,failed);

  y := hypergeo_cdfx(244,2063033,523360,7);
  f := 0.53100513491698108966e-21;
  testrel(36, 2*NE3, y, f, cnt,failed);        {2* for AMD}

  y := hypergeo_cdfx(312,1029855,523360,149);
...
begin
  ...
  y := rayleigh_cdfx(b, 10.0);
  f := 0.9996645373720974882;
  testrel(14, NE, y, f, cnt,failed);

  y := rayleigh_cdfx(b, 15.0);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Function: ','rayleigh_invx');

  b := 0.75;
  y := rayleigh_invx(b, 1e-8);
  f := 0.1060660174431471730e-3;
  testrel(1, NE2, y, f, cnt,failed);

  y := rayleigh_invx(b, 0.125);
  f := 0.3875858830021908203;
  testrel(2, NE, y, f, cnt,failed);

  y := rayleigh_invx(b, 0.25);
  f := 0.5688957123306990994;
  testrel(3, NE, y, f, cnt,failed);

  y := rayleigh_invx(b, 0.5);
  f := 0.8830575168866060183;
  testrel(4, NE, y, f, cnt,failed);

  y := rayleigh_invx(b, 0.75);
  f := 1.248831916736546635;
  testrel(5, NE2, y, f, cnt,failed);

  y := rayleigh_invx(b, 0.875);
  f := 1.529500485253213452;
  testrel(6, NE, y, f, cnt,failed);

  y := rayleigh_invx(b, 0.9);
  f := 1.609474519717010430;
  testrel(7, NE, y, f, cnt,failed);

  y := rayleigh_invx(b, 0.9990234375);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Test maxwell_pdfx');

  y := maxwell_pdfx(1,0.5);
  f := 0.1760326633821497389;
  testrel(1, NE, y, f, cnt,failed);

  y := maxwell_pdfx(1,1.5);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Test maxwell_cdfx');

  y := maxwell_cdfx(1,0.5);
  f := 0.3085959578372672950e-1;
  testrel(1, NE, y, f, cnt,failed);

  y := maxwell_cdfx(1,1.5);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Test maxwell_invx');

  y := maxwell_invx(1,0.99);
  f := 3.368214175218727377;
  testrel(1, NE, y, f, cnt,failed);

  y := maxwell_invx(1,0.75);
  f := 2.026905260645478921;
  testrel(2, NE, y, f, cnt,failed);

  y := maxwell_invx(1,0.5);
  f := 1.538172254455052334;
  testrel(3, NE, y, f, cnt,failed);

  y := maxwell_invx(1,0.125);
  f := 0.8320803611332766240;
  testrel(4, NE, y, f, cnt,failed);

  y := maxwell_invx(1,0.01);
  f := 0.33886841384100265169;
  testrel(5, NE, y, f, cnt,failed);

  y := maxwell_invx(2,0.9921875);
  f := 6.89293353359828506;
  testrel(6, NE2, y, f, cnt,failed);

  y := maxwell_invx(2,0.875);
  f := 4.791414577804765348;
  testrel(7, NE, y, f, cnt,failed);

  y := maxwell_invx(2,0.375);
  f := 2.648758988497067320;
  testrel(8, NE, y, f, cnt,failed);

  y := maxwell_invx(2,0.25);
  f := 2.202301435358628715;
  testrel(9, NE, y, f, cnt,failed);

  y := maxwell_invx(2,1/64);
  f := 0.7896542852108128080;
  testrel(10, NE, y, f, cnt,failed);

  y := maxwell_invx(5,0.99);
  f := 16.84107087609363689;
  testrel(11, NE, y, f, cnt,failed);

  y := maxwell_invx(5,0.75);
       {10.1345263032273946051084405473;}
  f := 2*5.067263151613697303;
  testrel(12, NE, y, f, cnt,failed);

  y := maxwell_invx(5,0.5);
  f := 7.690861272275261672;
  testrel(13, NE, y, f, cnt,failed);

  y := maxwell_invx(5,0.125);
  f := 4.160401805666383120;
  testrel(14, NE, y, f, cnt,failed);

  y := maxwell_invx(5,0.01);
...
begin
  ...
  y := evt1_pdfx(a,b,11000);
  f := 0.5763669429168183696e-4777;
  testrel(14, NE, y, f, cnt,failed);
...
const
 ...
 begin
  cnt := 0;
  failed := 0;
  writeln('Test evt1_cdfx');

  {boost}
  y := evt1_cdfx(0.5, 1.5, 0.125);
  ...
  y := evt1_cdfx(0.5, 0.25, 0.75);
  f := 0.6922006275553463539;
  testrel(3, NE, y, f, cnt,failed);

  y := evt1_cdfx(0.5, 0.25, 5);
  ...
  y := evt1_cdfx(0,1,12000);
  f := 1.0;
  testrel(13, NE, y, f, cnt,failed);

  y := evt1_cdfx(a,b,-8);
  f := 0.2434419635852852202e-1294;
  testrel(14, NE2, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Test evt1_invx');

  a := 0;
  b := 1;
  y := evt1_invx(a,b,0.99);
  f := 4.600149226776579998;
  testrel(1, NE, y, f, cnt,failed);

  y := evt1_invx(a,b,0.75);
  ...
  y := evt1_invx(a,b,0.125);
  f := -0.7320993680864453644;
  testrel(4, NE, y, f, cnt,failed);

  y := evt1_invx(a,b,0.015625);
  ...
  testrel(10, NE1, y, f, cnt,failed);      {NE1 for AMD}

  y := evt1_invx(a,b,1e-300);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Test kumaraswamy_pdfx');

  a := 2;
  b := 5;

  y := kumaraswamy_pdfx(a,b,0.0009765625);
  f := 0.9765587747150306052e-2;
  testrel(1, NE, y, f, cnt,failed);

  y := kumaraswamy_pdfx(a,b,0.2);
  f := 1.698693120;
  testrel(2, NE, y, f, cnt,failed);

  y := kumaraswamy_pdfx(a,b,0.4);
  f := 1.991485440;
  testrel(3, NE, y, f, cnt,failed);

  y := kumaraswamy_pdfx(a,b,0.6);
  f := 1.006632960;
  testrel(4, NE, y, f, cnt,failed);

  y := kumaraswamy_pdfx(a,b,0.8);
  f := 0.134369280;
  testrel(5, NE, y, f, cnt,failed);

  y := kumaraswamy_pdfx(a,b,0.9990234375);
  f := 0.1450933120938096265e-9;
  testrel(6, NE, y, f, cnt,failed);

  y := kumaraswamy_pdfx(a,b,1);
  f := 0.0;
  testrel(7, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Test kumaraswamy_cdfx');

  a := 2;
  b := 5;

  y := kumaraswamy_cdfx(a,b,0.0009765625);
  f := 0.4768362487092905884e-5;
  testrel(1, NE, y, f, cnt,failed);

  y := kumaraswamy_cdfx(a,b,0.2);
  f := 0.1846273024;
  testrel(2, NE1, y, f, cnt,failed);

  y := kumaraswamy_cdfx(a,b,0.4);
  f := 0.5817880576;
  testrel(3, NE, y, f, cnt,failed);

  y := kumaraswamy_cdfx(a,b,0.6);
  f := 0.8926258176;
  testrel(4, NE, y, f, cnt,failed);

  y := kumaraswamy_cdfx(a,b,0.8);
  f := 0.9939533824;
  testrel(5, NE, y, f, cnt,failed);

  y := kumaraswamy_cdfx(a,b,0.9990234375);
  f := 0.4999999999999858238*2;  {=0.9999999999999716476;}
  testrel(6, NE, y, f, cnt,failed);

  y := kumaraswamy_cdfx(a,b,1);
  f := 1.0;
  testrel(7, NE, y, f, cnt,failed);
...
begin
  cnt := 0;
  failed := 0;
  writeln('Test kumaraswamy_invx');

  a := 2;
  b := 5;

  y := kumaraswamy_invx(a, b, 1.0/1024.0);
  f := 0.1397815576817549282e-1;
  testrel(1, NE, y, f, cnt,failed);

  y := kumaraswamy_invx(a, b, 0.125);
  f := 0.1623355148599093934;
  testrel(2, NE, y, f, cnt,failed);

  y := kumaraswamy_invx(a, b, 0.5);
  f := 0.3597908235403952890;
  testrel(3, NE, y, f, cnt,failed);

  y := kumaraswamy_invx(a, b, 0.75);
  f := 0.4920789740933877362;
  testrel(4, NE, y, f, cnt,failed);

  y := kumaraswamy_invx(a, b, 0.875);
  f := 0.5833061328441120293;
  testrel(5, NE, y, f, cnt,failed);

  y := kumaraswamy_invx(a, b, 0.9);
  f := 0.6074888110243733127;
  testrel(6, NE, y, f, cnt,failed);

  y := kumaraswamy_invx(a, b, 0.99);
  f := 0.7758175232917227355;
  testrel(7, NE, y, f, cnt,failed);

  y := kumaraswamy_invx(a, b, 0.9990234375);
...
begin
  ...
  y := kolmogorov_cdfx(1.1796875);
  f := 0.8763641150488026113;
  testrel(11, NE, y, f, cnt,failed);

  y := kolmogorov_cdfx(1.25);
  f := 0.9121335860583089354;
  testrel(12, NE, y, f, cnt,failed);

  y := kolmogorov_cdfx(1.125);
  f := 0.8409611129029082319;
  testrel(13, NE, y, f, cnt,failed);

  y := kolmogorov_cdfx(1.5);
  f := 0.9777820373834748713;
  testrel(14, NE, y, f, cnt,failed);

  y := kolmogorov_cdfx(1.15);
  f := 0.8580401311219991918;
  testrel(15, NE, y, f, cnt,failed);

  y := kolmogorov_cdfx(1.2);
...
const
  NE = 4;
  NE1 = 8;  {tests cdf(inv(y))=y, y = 1/NTEST .. 1}
begin
  cnt := 0;
  failed := 0;
  writeln('Test kolmogorov_invx');
