{Part 1a of regression test for SPECFUN unit  (c) 2010  W.Ehrhardt}
...
begin
  ...
  {tests 20, 21 not for double}

  y := polygamma(2001,1024);
  f := 0.18525166336666044149e-287;
  testrel(22, NE, y, f, cnt,failed);

  y := polygamma(1800,980);
  ...
  y := polygamma(50, x);
  testrel(70, NEN, y, f, cnt,failed);

  x := -0.5-ldexp(1,-12);
  f := -0.3411319768828644233e79;
  y := polygamma(50, x);
  testrel(71, NEN, y, f, cnt,failed);
  ...
  y := polygamma(10, x);
  testrel(74, NEN2, y, f, cnt,failed);
  ...
  y := polygamma(20, x);
  testrel(76, NEN2, y, f, cnt,failed);
...
begin
  ...
  y := BatemanG(sqrt_epsh);  {extended: 0.5^32}
