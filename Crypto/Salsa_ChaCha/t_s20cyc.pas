{Cycle-Counter test program for Salsa20 stream cipher,  (c) we Jul.2008}
...
begin
  ...
  salsa_xkeysetup(ctx, @key, 128, rounds);
  salsa_ivsetup(ctx, @iv);
  for i:=1 to 5 do salsa_keystream_blocks(ctx, @stream, BLOCKS);
  c1 := MaxLongint;
  c2 := MaxLongint;
  for i:=1 to LOOPS do begin
    ReadTSC(cyc0);
    salsa_keystream_blocks(ctx, @stream, BLOCKS);
    ReadTSC(cyc1);
    salsa_keystream_blocks(ctx, @stream, BLOCKS);
    salsa_keystream_blocks(ctx, @stream, BLOCKS);
    salsa_keystream_blocks(ctx, @stream, BLOCKS);
...
begin
  ...
  for i:=1 to LOOPS do begin
    ReadTSC(cyc0);
    salsa_encrypt_blocks(ctx, @stream, @ct, BLOCKS);
    ReadTSC(cyc1);
    salsa_encrypt_blocks(ctx, @stream, @ct, BLOCKS);
    salsa_encrypt_blocks(ctx, @stream, @ct, BLOCKS);
    salsa_encrypt_blocks(ctx, @stream, @ct, BLOCKS);
...
begin
  ...
  writeln;
  writeln('T_S20CYC - Speed test for Salsa20 stream cipher   (c) 2006-2013 W.Ehrhardt');
  writeln('Selftest passed: ', salsa_selftest);
  writeln;
  writeln('GENERAL');
