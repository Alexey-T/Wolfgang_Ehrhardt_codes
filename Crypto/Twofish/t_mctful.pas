...
begin
  ...
  output('');
  output('Algorithm Name:       TWOFISH');
  output('Principal Submitter:  Bruce Schneier, Counterpane Systems');
  output('');
  TestBits(128);
  TestBits(192);
  TestBits(256);
...
begin
  writeln('T_MCTFUL - Twofish Full Monte Carlo Tests to *.LOG   (c) 2006-2008 W.Ehrhardt');
  HexUpper := true;
  ECBEncr;
  ECBDecr;
  CBCEncr;
