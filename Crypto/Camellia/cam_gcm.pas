...
(*************************************************************************
 ...
 -------  --------  -------     ------------------------------------------
 0.10     08.11.17  W.Ehrhardt  Initial version: from AES_GCM 0.33
**************************************************************************)
...
{$else}
function CAM_GCM_Init(var Key; KeyBits: word; var ctx: TCAM_GCMContext): integer;
...
{---------------------------------------------------------------------------}
{internal/testing}
procedure gf_mul_h(var a: TCAMBlock; {$ifdef CONST}const{$else}var{$endif} ctx: TCAM_GCMContext);
  {-Multiply a by ctx.ghash_h in GF(2^128}

procedure gf_mul(var a: TCAMBlock; {$ifdef CONST}const{$else}var{$endif} b: TCAMBlock);
...
{helper types}
type
  TWA4  = packed array[0..3] of longint;      {CAM block as array of longint}

const
  CTR_POS  = 12;
  BLK_MASK = CAMBLKSIZE-1;
...
{---------------------------------------------------------------------------}
procedure mul_x(var a: TCAMBlock; const b: TCAMBlock);
...
{---------------------------------------------------------------------------}
procedure mul_x(var a: TCAMBlock; {$ifdef CONST}const{$else}var{$endif} b: TCAMBlock);
  {-Multiply a = b*x in GF(2^128)}
var
  x: TWA4;
  t: longint;
...
{Note: At least on my machine inlining CAM_Xorblock in gf_mul is slower for}
...
begin
  ...
  for i:=0 to 15 do begin
    ...
    {$else}
      if c and $80 <> 0 then CAM_Xorblock(r,p[0],r);
      if c and $40 <> 0 then CAM_Xorblock(r,p[1],r);
      if c and $20 <> 0 then CAM_Xorblock(r,p[2],r);
      if c and $10 <> 0 then CAM_Xorblock(r,p[3],r);
      if c and $08 <> 0 then CAM_Xorblock(r,p[4],r);
      if c and $04 <> 0 then CAM_Xorblock(r,p[5],r);
      if c and $02 <> 0 then CAM_Xorblock(r,p[6],r);
      if c and $01 <> 0 then CAM_Xorblock(r,p[7],r);
    {$endif}
  end;
  a := r;
end;
...
{---------------------------------------------------------------------------}
procedure Make4K_Table(var ctx: TCAM_GCMContext);
...
begin
  with ctx do begin
    ...
    while j<256 do begin
      for k:=1 to j-1 do CAM_Xorblock(gf_t4k[k], gf_t4k[j], gf_t4k[j+k]);
      j := j+j;
    end;
  end;
end;
...
begin
  with ctx do begin
    ...
    for i:=14 downto 0 do begin
      p := @gf_t4k[a[i]];
      t := gft_le[x[3] shr 24];
      {preform mul_x8 and xor in pre-computed table entries}
      {$ifndef BIT16}
...
begin
  ...
  {does not hurt, but does not give significant speed improvments.}
  gf_mul(a, ctx.ghash_h);
end;
*)

{---------------------------------------------------------------------------}
function CAM_GCM_Init({$ifdef CONST}const{$else}var{$endif} Key; KeyBits: word; var ctx: TCAM_GCMContext): integer;
  {-Init context, calculate key-dependent GF(2^128) element H=E(K,0) and gf_mul tables}
var
  err: integer;
begin
  ...
  with ctx do begin
    err := CAM_Init(Key, KeyBits, actx);
    if err=0 then begin
      CAM_Encrypt(actx, ghash_h, ghash_h);
      Make4K_Table(ctx);
    end;
  end;
  CAM_GCM_Init := err;
...
{---------------------------------------------------------------------------}
procedure GCM_IncCtr(var x: TCAMBlock);
...
begin
  ...
  if (pIV=nil) and (IV_Len<>0) then begin
    CAM_GCM_Reset_IV := CAM_Err_NIL_Pointer;
    exit;
  end;
  ...
  with ctx do begin
    ...
    if IV_len=CTR_POS then begin
      ...
      while n_pos >= CAMBLKSIZE do begin
        CAM_Xorblock(actx.IV, PCAMBlock(pIV)^, actx.IV);
        inc(Ptr2Inc(pIV), CAMBLKSIZE);
        dec(n_pos, CAMBLKSIZE);
...
begin
  with ctx do begin
    if actx.Flag and $02 <> 0 then begin
      CAM_GCM_Final := CAM_Err_GCM_Auth_After_Final;
      exit;
    end;
    ...
    {$ifdef HAS_INT64}
      ...
      ln := (int64(atx_cnt) + CAMBLKSIZE - 1) div CAMBLKSIZE;
    ...
    CAM_Xorblock(tbuf, txt_ghv, tbuf);
    CAM_Xorblock(aad_ghv, tbuf, aad_ghv);
    gf_mul_h(aad_ghv, ctx);
    {compute E(K,Y0)}
    tbuf := actx.IV;
    x[3] := y0_val;
    CAM_Encrypt(actx, tbuf, tbuf);
    {tag = GHASH(H, AAD, ctp) xor E(K,Y0)}
    CAM_Xorblock(aad_ghv, tbuf, tag);
  end;
  CAM_GCM_Final := 0;
end;
...
{---------------------------------------------------------------------------}
function crypt_data(ptp, ctp: Pointer; ILen: longint; var ctx: TCAM_GCMContext): integer;
...
begin
  ...
  with ctx do begin
    b_pos := actx.bLen;
    if b_pos=0 then b_pos := CAMBLKSIZE
    else begin
      while (cnt < ILen) and (b_pos < CAMBLKSIZE) do begin
    ...
    while cnt + CAMBLKSIZE <= ILen do begin
      GCM_IncCtr(actx.IV);
      CAM_Encrypt(actx, actx.IV, actx.buf);
      CAM_XorBlock(PCAMBlock(ptp)^, actx.buf, PCAMBlock(ctp)^);
      inc(Ptr2Inc(ptp), CAMBLKSIZE);
      inc(Ptr2Inc(ctp), CAMBLKSIZE);
      inc(cnt, CAMBLKSIZE);
    end;
    while cnt < ILen do begin
      if b_pos=CAMBLKSIZE then begin
        GCM_IncCtr(actx.IV);
        CAM_Encrypt(actx, actx.IV, actx.buf);
        b_pos := 0;
      end;
...
{---------------------------------------------------------------------------}
function auth_data(ctp: pointer; ILen: longint; var ctx: TCAM_GCMContext): integer;
...
begin
  ...
  with ctx do begin
    if actx.Flag and $02 <> 0 then begin
      auth_data := CAM_Err_GCM_Auth_After_Final;
      exit;
    end;
    ...
    while cnt + CAMBLKSIZE <= ILen do begin
      gf_mul_h(txt_ghv, ctx);
      CAM_Xorblock(txt_ghv, PCAMBlock(ctp)^, txt_ghv);
      inc(Ptr2Inc(ctp), CAMBLKSIZE);
      inc(cnt, CAMBLKSIZE);
    end;
    while cnt < ILen do begin
...
begin
  ...
  if pAAD=nil then begin
    CAM_GCM_Add_AAD := CAM_Err_NIL_Pointer;
    exit;
  end;

  {$ifdef BIT16}
    if (aLen>$FFFF) or (ofs(pAAD^)+aLen>$FFFF) then begin
      CAM_GCM_Add_AAD := CAM_Err_Invalid_16Bit_Length;
      exit;
    end;
  {$endif}
  ...
  with ctx do begin
    if actx.Flag and $02 <> 0 then begin
      CAM_GCM_Add_AAD := CAM_Err_GCM_Auth_After_Final;
      exit;
    end;
    b_pos := aad_cnt[0] and BLK_MASK;
    if (b_pos=0) and (aad_cnt[0] or aad_cnt[1] <> 0) then gf_mul_h(aad_ghv, ctx);
    while (cnt < aLen) and (b_pos < CAMBLKSIZE) do begin
      aad_ghv[b_pos] := aad_ghv[b_pos] xor pByte(pAAD)^;
      inc(b_pos);
      inc(Ptr2Inc(pAAD));
      inc(cnt);
    end;
    while cnt + CAMBLKSIZE <= aLen do begin
      gf_mul_h(aad_ghv, ctx);
      CAM_Xorblock(aad_ghv, PCAMBlock(pAAD)^, aad_ghv);
      inc(Ptr2Inc(pAAD), CAMBLKSIZE);
      inc(cnt, CAMBLKSIZE);
    end;
...
begin
  ...
  if (ptp=nil) or (ctp=nil) then begin
    CAM_GCM_Encrypt := CAM_Err_NIL_Pointer;
    exit;
  end;
  {$ifdef BIT16}
    if (ofs(ptp^)+ILen>$FFFF) or (ofs(ctp^)+ILen>$FFFF) then begin
      CAM_GCM_Encrypt := CAM_Err_Invalid_16Bit_Length;
      exit;
    end;
  {$endif}
...
begin
  ...
  if (ptp=nil) or (ctp=nil) then begin
    CAM_GCM_Decrypt := CAM_Err_NIL_Pointer;
    exit;
  end;
  {$ifdef BIT16}
    if (ofs(ptp^)+ILen>$FFFF) or (ofs(ctp^)+ILen>$FFFF) then begin
      CAM_GCM_Decrypt := CAM_Err_Invalid_16Bit_Length;
      exit;
    end;
  {$endif}
...
{---------------------------------------------------------------------------}
function Internal_Dec_Veri(var ctx: TCAM_GCMContext; ptag: pointer; tLen: word;
...
begin
  ...
  if cLen>0 then begin
    if (ptp=nil) or (ctp=nil) then begin
      Internal_Dec_Veri := CAM_Err_NIL_Pointer;
      exit;
     end;
    {$ifdef BIT16}
      if (ofs(ptp^)+cLen>$FFFF) or (ofs(ctp^)+cLen>$FFFF) then begin
        Internal_Dec_Veri := CAM_Err_Invalid_16Bit_Length;
        exit;
      end;
    {$endif}
  end;
  ...
  if err=0 then begin
    ...
    if err=0 then begin
      ...
      err := err and CAM_Err_GCM_Verify_Tag;
    end;
...
var
  err: integer;
begin
  err :=  CAM_GCM_Init(Key, KBits, ctx);
  if err=0 then err := CAM_GCM_Reset_IV(pIV, IV_len, ctx);
  if err=0 then err := CAM_GCM_Add_AAD(pAAD, aLen, ctx);
  if err=0 then err := CAM_GCM_Encrypt(ptp, ctp, pLen, ctx);
  if err=0 then err := CAM_GCM_Final(tag, ctx);
  fillchar(ctx, sizeof(ctx), 0);
  CAM_GCM_Enc_Auth := err;
end;
