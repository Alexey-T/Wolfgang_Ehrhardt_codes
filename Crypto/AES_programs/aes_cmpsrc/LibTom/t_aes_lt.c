...
/* AES speed test program, W.Ehrhardt Jan 2005 */
...
int main(int argc, char * argv[])
{
  ...
  if (!strcmp(argv[1],"ctr")) {
    if ((error = ctr_start(find_cipher("rijndael"), IV, key, 16, 0,CTR_COUNTER_BIG_ENDIAN
    , &ctr)) != CRYPT_OK) {
    ...
    if ((error = cfb_start(find_cipher("rijndael"), IV, key, 16, 0, &cfb)) != CRYPT_OK) {
    ...
    /* start up OFB mode */
    if ((error = ofb_start(find_cipher("rijndael"), IV, key, 16, 0, &ofb)) != CRYPT_OK) {
    ...
    /* start up ecb mode */
    if ((error = ecb_start(find_cipher("rijndael"), key, 16, 0, &ecb)) != CRYPT_OK) {
    ...
    /* start up cbc mode */
    if ((error = cbc_start(find_cipher("rijndael"), IV, key, 16, 0, &cbc)) != CRYPT_OK) {
    ...
    if ((oidx = find_cipher("rijndael")) == -1) {
      printf("OMAC: Unable to find RIJNDAEL cipher\n");
      return -1;
    }
