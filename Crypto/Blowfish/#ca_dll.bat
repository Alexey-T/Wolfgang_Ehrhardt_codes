...
rem T_BF_WS with BF_DLL compile batch file for all console compilers
rem pause after each compiler if %1 not empty
rem call self on low env space condition
rem (c) 2003-2009 W.Ehrhardt

if exist BF_dll.dll goto dll_found
echo BF_DLL.DLL not found
goto ende
...
set PCB=call fpc260 -B -Sd -dUSEDLL
del %SRC%.exe >nul
%PCB% %SRC%.pas
if not (%1%)==() pause
echo. >>%LOG%
