...
{.$define intermediate}  {Output intermediate values, valid only for 32/64 bit}
                         {because 16-bit HexStr will truncate long BStrings!  }
...
begin
  ...
  if phash<>nil then with ctx do begin
    ...
    {$ifdef intermediate}
      writeln;
      writeln;
      writeln('---- Begin intermediate values');
      writeln('HMAC-',phash^.HName);
      writeln('Block length: ',bl);
      writeln('key:');
      writeln(' ', HexStr(key,lk));
    {$endif}
    if lk > bl then begin
      ...
      move(kb, hmacbuf, lk);
      {$ifdef intermediate}
        writeln('Block length > key length');
        writeln('K = Hash(key):');
        writeln(' ', HexStr(@hmacbuf,lk));
      {$endif}
    end
    ...
    {$ifdef intermediate}
      writeln('(K,0) xor ipad:');
      writeln(' ', HexStr(@hmacbuf,bl));
    {$endif}
    {start inner hash}
    phash^.HInit(hashctx);
...
begin
  with ctx do if phashd<>nil then begin
    ...
    {$ifdef intermediate}
      writeln('Inner hash (Hash((K,0) xor ipad, text): ');
      writeln(' ', HexStr(@mac,phashd^.HDigestLen));
    {$endif}
    {remove ipad from buf, XOR opad}
    for i:=0 to bl-1 do hmacbuf[i] := hmacbuf[i] xor ($36 xor $5c);
    {$ifdef intermediate}
      writeln('(K,0) xor opad:');
      writeln(' ', HexStr(@hmacbuf,bl));
    {$endif}
    {outer hash}
    ...
    phashd^.HFinal(hashctx, mac);
    {$ifdef intermediate}
      writeln('---- End intermediate values');
      writeln('HMAC:');
      writeln(' ', HexStr(@mac,phashd^.HDigestLen));
    {$endif}
  end;
...
begin
  with ctx do if phashd<>nil then begin
    {$ifdef intermediate}
      write('Final data bits: ''');
      bl := Bdata;
      for i:=1 to bitlen do begin
        if bl and $80 <>0 then write('1') else write('0');
        bl := (bl shl 1) and $ff;
      end;
      writeln(''' = MSB(', HexByte(Bdata),')' );
    {$endif}
    bl := phashd^.HBlockLen;
    {complete inner hash}
