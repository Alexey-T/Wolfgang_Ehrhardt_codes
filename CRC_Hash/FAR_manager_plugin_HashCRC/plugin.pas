(*
   ...
   Changes by W.Ehrhardt marked with //[*we-2006-04-01]
*)
...
type
   ...
   TFarDialogItem = packed record
      ...
      Param : record case Integer of
         0 : (Selected : {$ifdef USE_BOOL} LongBool {$else} Integer {$endif});   //[*we-2006-04-01]
         1 : (History : PChar);
         2 : (Mask : PChar);
      ...
      Flags : DWORD;
      DefaultButton : {$ifdef USE_BOOL} LongBool {$else} Integer {$endif};       //[*we-2006-04-01]
...
type
   ...
   TFarMenuItem = packed record
      Text : array [0..127] of Char;
      Selected :  {$ifdef USE_BOOL} LongBool {$else} Integer {$endif};  //[*we-2006-04-01]
      Checked :   {$ifdef USE_BOOL} LongBool {$else} Integer {$endif};  //[*we-2006-04-01]
      Separator : {$ifdef USE_BOOL} LongBool {$else} Integer {$endif};  //[*we-2006-04-01]
   end;
...
type
   ...
   TPanelInfo = packed record
      ...
      Visible : {$ifdef USE_BOOL} LongBool {$else} Integer {$endif};  //[*we-2006-04-01]
      Focus :   {$ifdef USE_BOOL} LongBool {$else} Integer {$endif};  //[*we-2006-04-01]
      ViewMode : Integer;
...
type
   ...
   TFarApiCmpName = function (
         const Pattern : PChar;
         const aString : PChar;
         SkipPath : {$ifdef USE_BOOL} LongBool {$else} Integer {$endif} //[*we-2006-04-01]
         ) : Integer; {$IFNDEF VP} stdcall; {$ENDIF}
...
type
   ...
   TWindowInfo = packed record
      Pos : Integer;
      WindowType : Integer;
      Modified : {$ifdef USE_BOOL} LongBool {$else} Integer {$endif};  //[*we-2006-04-01]
      Current :  {$ifdef USE_BOOL} LongBool {$else} Integer {$endif};  //[*we-2006-04-01]
...
type
   ...
   TEditorInfo = packed record
      ...
      AnsiMode : {$ifdef USE_BOOL} LongBool {$else} Integer {$endif}; //[*we-2006-04-01]
      TableNum : Integer;
      Options : DWORD;
      TabSize : Integer;
...
type
   ...
   TPluginStartupInfo = packed record
      ...
      FSF : PFarStandardFunctions;  //[*we-2006-04-01]: FSF must be a pointer
...
type
   ...
   TPanelMode = packed record
      ...
      FullScreen :      {$ifdef USE_BOOL} LongBool {$else} Integer {$endif}; //[*we-2006-04-01]
      DetailedStatus :  {$ifdef USE_BOOL} LongBool {$else} Integer {$endif}; //[*we-2006-04-01]
      AlignExtensions : {$ifdef USE_BOOL} LongBool {$else} Integer {$endif}; //[*we-2006-04-01]
      CaseConversion :  {$ifdef USE_BOOL} LongBool {$else} Integer {$endif}; //[*we-2006-04-01]
...
{From Encyclopedia/Custom APi/Addons} //[*we-2006-04-01]
type
   PInitDialogItem = ^TInitDialogItem;
...
type
   PInitDialogItemArray = ^TInitDialogItemArray;
   TInitDialogItemArray = packed array[0..Pred(MaxLongint div SizeOf(TInitDialogItem))] of TInitDialogItem;
