...
(*************************************************************************
 ...
 0.22     21.12.17  we          isaac_rangel/w
 0.23     25.12.17  we          rangel for old non-basm compilers
**************************************************************************)
...
function  isaac_rangew(var ctx: isaac_ctx; range: word): word;
  {-Next random word in range 0..range-1}

function  isaac_rangel(var ctx: isaac_ctx; range: longint): longint;
...
begin
  isaac_next(ctx);
  isaac_rangew := rangew(ctx.nextres, range);
end;
...
begin
  isaac_next(ctx);
  isaac_rangel := rangel(ctx.nextres, range);
end;
