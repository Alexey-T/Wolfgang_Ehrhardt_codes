...
function crc32(crc: uLong; buf: pBytef; len: uInt): uLong;
  ...
  {If buf is NULL, this function returns the required initial value
...
function get_crc_table: puLong;
  {-Returs addr of crc table, can be used by asm versions of crc32()}
...
begin
  ...
  poly := 0;
  for n := 0 to (sizeof(p) div sizeof(byte))-1 do poly := poly or (Long(1) shl (31 - p[n]));

  for n := 0 to 255 do begin
    c := uLong(n);
    for k := 0 to 7 do begin
      if c and 1 <> 0 then c := poly xor (c shr 1) else c := c shr 1;
    end;
    crc_table[n] := c;
  end;
...
begin
  {$ifdef DYNAMIC_CRC_TABLE}
    if crc_table_empty then make_crc_table;
  {$endif}
...
begin
  if buf=Z_NULL then crc32 := 0
    ...
    while len >= 8 do begin
      {DO8(buf)}
      crc := crc_table[(int(crc) xor buf^) and $ff] xor (crc shr 8);  inc(buf);
      crc := crc_table[(int(crc) xor buf^) and $ff] xor (crc shr 8);  inc(buf);
      crc := crc_table[(int(crc) xor buf^) and $ff] xor (crc shr 8);  inc(buf);
      crc := crc_table[(int(crc) xor buf^) and $ff] xor (crc shr 8);  inc(buf);
      crc := crc_table[(int(crc) xor buf^) and $ff] xor (crc shr 8);  inc(buf);
